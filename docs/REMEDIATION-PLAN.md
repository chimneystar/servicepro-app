...
| 6.3 | One action contract; error/loading boundaries per route group; toast primitive | **DONE for the contract and boundaries; toast REJECTED with reasoning.** Boundaries: `app/error.tsx` was the only one in the product — a boundary replaces everything below its layout, so any error on a signed-in screen removed the app shell including the mobile tab bar, which IS the navigation. Added `app/(app)/error.tsx` (renders inside the shell), `app/global-error.tsx` (the root layout’s own failure, which `app/error.tsx` cannot catch), customer-facing boundaries for `/book`, `/p`, `/portal`, and `app/(app)/loading.tsx`. Contract: `reviewManualPayment` returned BARE on every refusal including the authorization check — and because `actionFailureMessage` reads an absent result as SUCCESS, an office user without `can_confirm_manual_payments` pressed “Confirm received”, the page refreshed, and the payment sat there unexplained. `submitPortalRequest` discarded the RPC’s `error` entirely, so a customer’s reschedule request vanished silently. Both now report every path, and both forms moved to client wrappers on the existing `useActionStatus` hook — because `<form action={serverAction}>` in a SERVER component DISCARDS the return value, so fixing the actions alone would have looked complete and changed nothing a user sees. `beginHelcimOnboarding` was left alone: every path already ends in a `redirect()`, which is visible feedback. **Toast REJECTED:** the five remaining callable actions already return `{ok,…}` and their callers already render the message inline, next to the control that failed. A toast would move that feedback further from the thing it describes and, unwired, would be exactly the exists-but-doesn’t-work failure this branch exists to remove. | **DONE / REJECTED** — 13 probes across boundaries and contract, each proven both ways |
...
| 6.9 | **A deleted job type stayed bookable by customers.** `booking_services.job_type_id` is `on delete set null`, so deleting a job type orphaned its bookable service instead of withdrawing it. The owner’s settings screen lists BY JOB TYPE, so the orphan was invisible to them; `public_booking_info_v2`, `/api/booking/[org]/slots` and `/submit` never mention job_types, so customers could still see and book it — and the owner had no way to find or remove it. Fixed by `db/043_booking_orphan_services.sql`: a BEFORE DELETE trigger deactivates the service, which closes all three read paths at once because each already filters on `active`. Deactivates rather than cascade-deletes, so the configured duration, price and translations survive a deletion the owner may not have meant. Existing orphans backfilled. **A correction to this row’s first version:** I originally claimed the orphan produced `key="null"` and a non-UUID POST in `BookingSettingsForm`. That was wrong — `rows` is built from job types, not services, so an orphan simply never appears. The real defect is worse and in the opposite direction: invisible to the owner, still live for the customer. The privacy-export half of this row was fixed by 6.1 (the `organization_id` guard). | **DONE** — 3 probes, proven both ways: removing the trigger leaves the service offered |
| 6.10 | **A valid VAPID key pair was rejected about 1 time in 256, leaving push silently unavailable.** `checkVapidKeys` required the private key to be exactly 32 bytes. A P-256 private key is a 256-bit INTEGER, and a big-endian integer need not occupy its full width: when the high byte is zero — measured at 78 in 20,000 — a generator emitting the minimal encoding produces 31 bytes. A correctly generated pair was then reported as `private_key_length` and push stayed off until somebody happened to regenerate. Surfaced by `tests/push.test.mjs` failing about 1 run in 50, which is exactly the shape of defect that gets “fixed” by padding the test until it stops complaining. Found by the 6.2 agent, which explicitly refused to do that. Fixed by left-padding the scalar in both `checkVapidKeys` and `publicKeyFromPrivate`; a length that cannot be a scalar (over 32, or empty) is still refused by length. The new probe GENERATES until it finds a short scalar, so the case runs every time rather than once in fifty. | **DONE** — proven both ways: restoring the exact-32 check refuses a valid pair and names the 31-byte scalar |
| 6.7 | Consolidate overlapping tables (line items, assignment models, permission systems) | **REJECTED, with a guard shipped instead** — measured against a real database, `estimate_items` and `invoice_items` are ALREADY identical: 11 columns each, differing only in their parent key, with no type or nullability difference on any shared column. Migration 002 added `cost_minor` and `image_path` to both. The drift consolidation exists to prevent has not occurred in the life of this product. Against that: merging them means rewriting 14 files and migrating live invoice line items — the money — with no production access and no environment to rehearse in, and production is already on a different schema (102 tables vs 97 at audit time) so we cannot predict the starting state. Every other item on this branch fixed something WRONG; this would restructure something merely duplicated, and do it to financial records. So the obligation it carried — “remember to change both” — becomes a mechanism: `tests/line-item-parity.test.mjs`, 3 probes proven both ways (a column added to one side trips the parity check; a nullability change trips the type check; the clean tree is silent). **Deliberately still open:** the two assignment models (`jobs.assigned_to`, written in 18 places, vs `job_assignments`, 4) and the three permission mechanisms (`profiles.role`, `profile_capabilities`, `profile_payment_permissions`). Both are genuine confusion, but authorization is where this branch already found a live hole, and restructuring it without a live environment to test against trades a known cost for an unknown risk. Recommended post-launch, in that order. |
