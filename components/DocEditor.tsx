...
export default function DocEditor({
  ...
  const [rows, setRows] = useState<Row[]>(
    initial.items.length
      ? initial.items
      : [{ title: "", desc: "", qty: "1", price: "", cost: "", taxable: true, image_path: "" }],
  );
  ...
  function addFromCatalog(id: string) {
    ...
    setRows((rs) => [
      ...rs,
      {
        ...
        desc: it.description ?? "",
        qty: "1",
        price: (it.price_minor / 100).toFixed(2),
        cost: (it.cost_minor / 100).toFixed(2),
  ...
  const subtotal = rows.reduce(
    (s, r) =>
      s + Math.round((parseFloat(r.qty || "0") || 0) * (parseFloat(r.price || "0") || 0) * 100),
    0,
  );
  ...
  return (
    <form action={formAction} style={{ maxWidth: 640 }}>
      ...
      */}
      ...
      {catalog.length > 0 && (
        <select
          aria-label={he ? "שימוש חוזר בפריט שמור" : "Reuse a saved item"}
        ...
        <div key={i} style={itemCard}>
          <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
            ...
            <button
              ...
              style={xBtn}
              aria-label={he ? "הסרת פריט זה" : "Remove this item"}
            >
              ✕
            </button>
          </div>
          ...
          >
            ...
            {r.image_path && (
              <button
                type="button"
                onClick={() => update(i, { image_path: "" })}
                style={{ ...xBtn, background: "#eef2f8", color: "#5c6675" }}
              >
                remove
              </button>
            )}
