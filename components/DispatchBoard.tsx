...
export default function DispatchBoard({
  ...
  const columns = useMemo(
    () => [{ id: "", full_name: he ? "לא משובצות" : "Unassigned", role: "" }, ...techs],
    [he, techs],
  );
  ...
  function move(jobId: string, profileId: string | null) {
    ...
    setJobs((rows) =>
      rows.map((job) => (job.id === jobId ? { ...job, assigned_to: profileId } : job)),
    );
    ...
    setAssignments((rows) =>
      rows.filter((row) => !(row.job_id === jobId && row.is_lead && row.profile_id !== profileId)),
    );
    setNotice(null);
    startTransition(async () => {
      ...
      if (!result.ok) {
        setJobs(before);
        setAssignments(beforeAssignments);
  ...
  function add(jobId: string, profileId: string) {
    if (
      !profileId ||
      assignments.some((row) => row.job_id === jobId && row.profile_id === profileId)
    )
      return;
    setNotice(null);
    startTransition(async () => {
      ...
      if (result.ok)
        setAssignments((rows) => [
          ...rows,
          { job_id: jobId, profile_id: profileId, is_lead: false },
        ]);
  ...
  function remove(jobId: string, profileId: string) {
    ...
    startTransition(async () => {
      ...
      if (result.ok)
        setAssignments((rows) =>
          rows.filter((row) => !(row.job_id === jobId && row.profile_id === profileId)),
        );
      else setNotice(result.error || failed);
    });
  }
  ...
  return (
    <div className="dispatch-page">
      <header className="dispatch-heading">
        <div>
          <span>{he ? "מרכז שיבוץ" : "Dispatch center"}</span>
          <h1>{dayLabel}</h1>
          <p>
            {he
              ? "גררו עבודה לטכנאי, או הוסיפו עוד אנשי צוות לעבודה."
              : "Drag a job to a technician, or add more team members to the job."}
          </p>
        </div>
        <form>
          <input type="date" name="date" defaultValue={date} aria-label={he ? "תאריך" : "Date"} />
          <button type="submit">{he ? "מעבר" : "Go"}</button>
        </form>
      </header>
        ...
        {columns.map((column) => {
          ...
          return (
            <section
              ...
              onDrop={(event) => move(event.dataTransfer.getData("text/job-id"), column.id || null)}
            >
              <header>
                <div className="dispatch-avatar">
                  {column.id ? column.full_name.slice(0, 1).toUpperCase() : "?"}
                </div>
                <strong>{column.full_name || (he ? "ללא שם" : "Unnamed")}</strong>
                <b>{rows.length}</b>
              </header>
              <div className="dispatch-stack">
                {rows.map((job) => {
                  const extra = assignments.filter(
                    (row) =>
                      row.job_id === job.id && row.profile_id && row.profile_id !== job.assigned_to,
                  );
                  return (
                    <article
                      key={job.id}
                    ...
                    >
                      ...
                      <Link href={`/jobs/${job.id}`}>
                        ...
                        <p>
                          {job.customers?.name || (he ? "לקוח" : "Customer")}
                          {job.job_city ? ` · ${job.job_city}` : ""}
                        </p>
                      </Link>
                      {extra.length > 0 && (
                        <div className="dispatch-assignees">
                          {extra.map((row) => (
                            <button
                              type="button"
                              key={row.profile_id!}
                              onClick={() => remove(job.id, row.profile_id!)}
                              title={he ? "הסרה" : "Remove"}
                            >
                              {techs.find((tech) => tech.id === row.profile_id)?.full_name || "?"} ×
                            </button>
                          ))}
                        </div>
                      )}
                  ...
                  drag stays, as the faster way for a mouse. */}
                      <select
                        ...
                        onChange={(event) => move(job.id, event.target.value || null)}
                        aria-label={
                          he ? `שיוך "${job.service}" לטכנאי` : `Assign "${job.service}" to`
                        }
                      >
                        <option value="">{he ? "לא משובצת" : "Unassigned"}</option>
                        {techs.map((tech) => (
                      ...
                      <select
                        value=""
                        onChange={(event) => add(job.id, event.target.value)}
                        aria-label={he ? "הוספת טכנאי" : "Add technician"}
                      >
                        <option value="">{he ? "+ הוספת טכנאי" : "+ Add technician"}</option>
                        {techs
                          .filter(
                            (tech) =>
                              tech.id !== job.assigned_to &&
                              !extra.some((row) => row.profile_id === tech.id),
                          )
                          .map((tech) => (
                            <option key={tech.id} value={tech.id}>
                              {tech.full_name}
                ...
                {rows.length === 0 && (
                  <div className="dispatch-dropzone">
                    {he ? "אפשר לגרור לכאן עבודה" : "Drop a job here"}
                  </div>
                )}
              </div>
            </section>
