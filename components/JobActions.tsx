  ...
  return (
    <div>
      <label style={{ display: "block" }}>
        <span style={{ fontSize: "0.8125rem", fontWeight: 700, color: "#334155", display: "block", marginBottom: 6 }}>{he ? "סטטוס" : "Status"}</span>
      ...
      {canInvoice && <button type="button" onClick={makeInvoice} disabled={pending} style={{ width: "100%", background: "#15803d", color: "#fff", border: "none", borderRadius: 12, padding: 14, fontWeight: 800, fontSize: "0.9375rem", cursor: "pointer" }}>{he ? "יצירת חשבונית מהעבודה" : "Create invoice from job"}</button>}
      {ok && <div role="status" style={{ marginTop: 10, color: "#15803d", fontSize: "0.8125rem", fontWeight: 600 }}>✓ {ok}</div>}
