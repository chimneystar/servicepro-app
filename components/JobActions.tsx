...
export default function JobActions({ jobId, stage, stages, canInvoice }: { jobId: string; stage: string; stages: Stage[]; canInvoice: boolean }) {
  ...
  return (
    <div>
      <label style={{ fontSize: "0.8125rem", fontWeight: 700, color: "#334155", display: "block", marginBottom: 6 }}>{he ? "סטטוס" : "Status"}</label>
      ...
      {canInvoice && <button onClick={makeInvoice} disabled={pending} style={{ width: "100%", background: "#15803d", color: "#fff", border: "none", borderRadius: 12, padding: 14, fontWeight: 800, fontSize: "0.9375rem", cursor: "pointer" }}>{he ? "יצירת חשבונית מהעבודה" : "Create invoice from job"}</button>}
