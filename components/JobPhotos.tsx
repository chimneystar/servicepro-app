...
export default function JobPhotos({
    ...
    try {
      for (const file of files) {
        ...
        if ((!file.type.startsWith("image/") && !isVideo) || file.size > limit)
          throw new Error(
            he ? "הקובץ לא נתמך או גדול מדי." : "That file type is not supported or is too large.",
          );
  ...
  async function saveAnnotation(blob: Blob, parent: Photo) {
    ...
    setError(null);
    try {
      const path = `${orgId}/${jobId}/${crypto.randomUUID()}-annotated.png`;
      ...
      const result = await recordPhoto(
        jobId,
        path,
        he ? "תמונה עם סימון" : "Annotated photo",
        "image",
        parent.id,
      );
      ...
      setError(err?.message ?? (he ? "שמירת הסימון נכשלה." : "Annotation could not be saved."));
    } finally {
      setBusy(false);
    }
  }
  ...
  async function toggleVisible(photo: Photo) {
    ...
    if (!result.ok)
      setError(
        result.error ?? (he ? "לא הצלחנו לעדכן את התמונה." : "We couldn't update that photo."),
      );
    router.refresh();
    setBusy(false);
  }
        ...
        {photos.map((p) => (
          ...
          >
            ...
            {p.mediaType === "image" && p.url && (
              <button type="button" onClick={() => setEditing(p)} style={annotateBtn}>
                {he ? "סימון" : "Mark up"}
              </button>
            )}
            ...
            <button
              ...
              title={
                p.customerVisible
                  ? he
                    ? "מוצג בדוח ללקוח — לחצו כדי להסתיר"
                    : "Shown on the customer report — click to hide"
                  : he
                    ? "מוסתר מהלקוח — לחצו כדי להציג"
                    : "Hidden from the customer — click to show"
              }
              aria-label={
                p.customerVisible
                  ? he
                    ? "הסתרת התמונה מהלקוח"
                    : "Hide this photo from the customer"
                  : he
                    ? "הצגת התמונה ללקוח"
                    : "Show this photo to the customer"
              }
            ...
            <button
              ...
              style={delBtn}
              title="Delete"
              aria-label={he ? "מחיקת התמונה" : "Delete photo"}
            >
              ✕
            </button>
          </div>
...
function PhotoAnnotator({
  ...
  he: boolean;
  onCancel: () => void;
  onSave: (blob: Blob, photo: Photo) => void;
}) {
  ...
  function start(event: React.PointerEvent<HTMLCanvasElement>) {
    ...
    if (ctx) {
      ...
      ctx.lineWidth = Math.max(4, event.currentTarget.width / 140);
      ctx.lineCap = "round";
      ctx.lineJoin = "round";
    }
  }
  ...
  return (
    <div className="photo-annotator" role="dialog" aria-modal="true" aria-labelledby={titleId}>
      <div>
        <header>
          <strong id={titleId}>{he ? "סימון על התמונה" : "Mark up photo"}</strong>
          <small>
            {he
              ? "ציירו באדום על האזור שחשוב להראות."
              : "Draw in red over the area you want to call out."}
          </small>
        </header>
        <canvas
          ref={canvasRef}
