  ...
  return (
    <div>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 14 }}>
        <div style={{ background: "#fdf1dc", borderRadius: 12, padding: "12px 14px" }}><div style={{ fontSize: 14, color: "#b45309", fontWeight: 700 }}>Balance due</div><div style={{ fontSize: 20, fontWeight: 800, color: "#b45309" }}>{money(totalDue, currency)}</div></div>
        <div style={{ background: "#e6f6ec", borderRadius: 12, padding: "12px 14px" }}><div style={{ fontSize: 14, color: "#15803d", fontWeight: 700 }}>Paid</div><div style={{ fontSize: 20, fontWeight: 800, color: "#15803d" }}>{money(totalPaid, currency)}</div></div>
      </div>
            ...
            <div key={inv.id} style={{ background: "#fff", border: "1px solid #e2e8f0", borderRadius: 12, padding: 14 }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                <div>
                  ...
                  <div style={{ fontSize: 14, color: "#5c6675" }}>{money(inv.paid_minor, currency)} paid of {money(inv.total_minor, currency)}</div>
                </div>
              ...
              {(inv.payments ?? []).length > 0 && (
                <div style={{ marginTop: 8, display: "grid", gap: 4 }}>
                  {(inv.payments ?? []).map((p, k) => (
                    <div key={k} style={{ fontSize: 14, color: "#5c6675", display: "flex", justifyContent: "space-between", gap: 8 }}>
