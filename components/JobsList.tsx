...
export default function JobsList({
  ...
  const shown = useMemo(() => {
    ...
    return jobs.filter((j) => {
      ...
      if (
        s &&
        ![j.customer, j.service, j.address, ...(j.tags ?? [])].some((f) =>
  ...
  const fmt = (d: string, t: string | null) => {
    ...
    return `${x.toLocaleDateString("en-US", { weekday: "short", month: "short", day: "numeric" })}${t ? " · " + t.slice(0, 5) : ""}`;
  };
  ...
  return (
    <div>
      {truncated && (
        ...
        >
          <span>
            Showing the {loadedCount} most recent of {totalCount} jobs. The counts and search below
            cover only these.
          </span>
          {loadMoreHref && (
            <Link
      ...
      <div style={{ display: "flex", gap: 8, marginBottom: 12, flexWrap: "wrap" }}>
        <div style={{ position: "relative", flex: 1, minWidth: 200 }}>
          ...
          <input
            ...
            placeholder="Search client, service, address, tag…"
            aria-label="Search client, service, address, tag…"
            style={{
              width: "100%",
        ...
        {allTags.length > 0 && (
          <select
            ...
            aria-label={he ? "סינון לפי תגית" : "Filter by tag"}
            style={{
              border: "1px solid #e2e8f0",
              borderRadius: 12,
              padding: "11px 12px",
      ...
      <div className="rlist">
          ...
          return (
            ...
            >
              <div className="rmain">
                ...
                <div className="rsub">
                  {fmt(j.scheduled_date, j.start_time)}
                  {j.tech ? ` · ${j.tech}` : ""}
                  {j.address ? ` · ${j.address}` : ""}
                </div>
                {(j.tags ?? []).length > 0 && (
              ...
              <div className="rend">
                <span
                  className="pill"
                  style={{ background: colorOf(j.stage) + "22", color: colorOf(j.stage) }}
                >
                  {j.stage}
                </span>
                <b style={{ fontSize: "0.875rem" }}>
                  {cur}
                  {(j.price_minor / 100).toLocaleString("en-US", {
        ...
        {shown.length === 0 && (
          <div className="rempty">No jobs match. Try a different status, tag, or search.</div>
        )}
      </div>
    </div>
  );
