    ...
    <div>
      ...
      <div style={{ display: "flex", gap: 8, marginBottom: 12, flexWrap: "wrap" }}>
        <div style={{ position: "relative", flex: 1, minWidth: 200 }}>
          ...
          <input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Search client, service, address, tag…" style={{ width: "100%", border: "1px solid #e2e8f0", borderRadius: 12, padding: "11px 12px 11px 38px", fontSize: "1rem", outline: "none" }} />
        </div>
        {allTags.length > 0 && (
          <select value={tag} onChange={(e) => setTag(e.target.value)} style={{ border: "1px solid #e2e8f0", borderRadius: 12, padding: "11px 12px", fontSize: "0.875rem", background: "#fff", fontWeight: 600 }}>
...
function TabBtn({ label, n, on, onClick, color }: { label: string; n: number; on: boolean; onClick: () => void; color: string }) {
  return (
    <button onClick={onClick} style={{ border: "none", background: "transparent", padding: "8px 12px 10px", cursor: "pointer", whiteSpace: "nowrap", fontSize: "0.875rem", fontWeight: 700, color: on ? color : "#5c6675", borderBottom: on ? `3px solid ${color}` : "3px solid transparent", marginBottom: -3 }}>
