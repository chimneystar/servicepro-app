...
export type CalJob = {
  ...
  date: string; start: string | null; end: string | null; tech: string;
};
...
function statusColor(s: string): [string, string] {
  const m: Record<string, string> = { scheduled: "#2563eb|#fff", in_progress: "#b45309|#fff", done: "#15803d|#fff", cancelled: "#94a3b8|#fff" };
  ...
  return (
    <div style={{ minWidth: days === 7 ? 680 : undefined }}>
        ...
        {cols.map((d) => {
          ...
          return (
            <div key={iso(d)} style={{ textAlign: "center", padding: "8px 0", borderInlineStart: "1px solid #eef1f6" }}>
              <div style={{ fontSize: "0.8125rem", color: "#5c6675", fontWeight: 700 }}>{he ? He[DAYS[d.getDay()]] : DAYS[d.getDay()]}</div>
              <div style={{ display: "inline-flex", alignItems: "center", justifyContent: "center", width: 28, height: 28, borderRadius: "50%", fontWeight: 800, fontSize: "0.9375rem", marginTop: 2, background: isToday ? "#2563eb" : "transparent", color: isToday ? "#fff" : "#0b1524" }}>{d.getDate()}</div>
            </div>
          );
        })}
      </div>
      <div style={{ display: "grid", gridTemplateColumns: `56px repeat(${days},1fr)`, maxHeight: 560, overflowY: "auto", position: "relative" }}>
        <div>
          {hours.map((h) => <div key={h} style={{ height: HOUR, fontSize: "0.8125rem", color: "#94a3b8", textAlign: "end", paddingInlineEnd: 6, paddingTop: 2 }}>{fmtHour(h, he)}</div>)}
        </div>
        {cols.map((d) => {
          const dayJobs = jobs.filter((j) => j.date === iso(d));
          return (
            <div key={iso(d)} style={{ position: "relative", borderInlineStart: "1px solid #eef1f6" }}>
              {hours.map((h) => <div key={h} style={{ height: HOUR, borderTop: "1px solid #f1f4f9" }} />)}
              {dayJobs.map((j) => {
                ...
                return (
                  <a key={j.id} href={`/jobs/${j.id}`} title={`${j.title} · ${j.service}`} style={{ position: "absolute", top: Math.max(0, top), left: 3, right: 3, height: Math.max(24, h - 3), background: bg, color: "#fff", borderRadius: 7, padding: "4px 7px", fontSize: "0.8125rem", overflow: "hidden", boxShadow: "0 2px 6px rgba(0,0,0,.15)", textDecoration: "none", display: "block" }}>
  ...
  return (
    <div style={{ minWidth: 620 }}>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(7,1fr)", borderBottom: "1px solid #eef1f6" }}>
        {DAYS.map((d) => <div key={d} style={{ textAlign: "center", padding: "8px 0", fontSize: "0.8125rem", fontWeight: 700, color: "#5c6675" }}>{he ? He[d] : d}</div>)}
      </div>
        ...
        {cells.map((d) => {
          ...
          return (
            <div key={iso(d)} style={{ minHeight: 92, borderInlineStart: "1px solid #f1f4f9", borderTop: "1px solid #f1f4f9", padding: 5, background: inMonth ? "#fff" : "#fafbfd" }}>
              <div style={{ display: "inline-flex", alignItems: "center", justifyContent: "center", width: 22, height: 22, borderRadius: "50%", fontSize: "0.75rem", fontWeight: 700, background: isToday ? "#2563eb" : "transparent", color: isToday ? "#fff" : inMonth ? "#0b1524" : "#b6bfcc" }}>{d.getDate()}</div>
              {dayJobs.slice(0, 3).map((j) => {
                const bg = typeColors[j.service] || statusColor(j.status)[0];
                return <a key={j.id} href={`/jobs/${j.id}`} style={{ display: "block", marginTop: 3, background: bg, color: "#fff", borderRadius: 5, padding: "2px 6px", fontSize: "0.8125rem", fontWeight: 700, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis", textDecoration: "none" }}>{(j.start ?? "").slice(0, 5)} {j.title}</a>;
              })}
              {dayJobs.length > 3 && <div style={{ fontSize: "0.8125rem", color: "#5c6675", marginTop: 2 }}>+{dayJobs.length - 3} {he ? "נוספות" : "more"}</div>}
            </div>
          );
        })}
      </div>
...
const notice: React.CSSProperties = { background: "#eef2f8", border: "1px solid #e2e8f0", color: "#334155", padding: "8px 14px", fontSize: "0.8125rem", fontWeight: 600 };
const navBtn: React.CSSProperties ={ width: 30, height: 30, borderRadius: 8, border: "1px solid #e2e8f0", background: "#fff", cursor: "pointer", fontSize: "1rem", color: "#334155" };
const btnGhost: React.CSSProperties = { padding: "6px 12px", borderRadius: 8, border: "1px solid #e2e8f0", background: "#fff", cursor: "pointer", fontWeight: 700, fontSize: "0.8125rem" };
const seg: React.CSSProperties = { border: "none", background: "transparent", padding: "6px 14px", borderRadius: 8, fontWeight: 700, fontSize: "0.8125rem", color: "#5c6675", cursor: "pointer" };
