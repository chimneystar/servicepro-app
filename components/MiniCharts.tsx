...
export function Donut({ segments, size = 128, thickness = 18, centerTop, centerSub }: {
  segments: { value: number; color: string }[]; size?: number; thickness?: number; centerTop?: string; centerSub?: string;
}) {
    ...
    <div style={{ position: "relative", width: size, height: size }}>
      ...
      {(centerTop || centerSub) && (
        <div style={{ position: "absolute", inset: 0, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center" }}>
          ...
          {centerSub && <div style={{ fontSize: "0.8125rem", color: "#5c6675", fontWeight: 600 }}>{centerSub}</div>}
        </div>
      )}
    </div>
  );
...
export function Bars({ data, color = "#2563eb", height = 120 }: { data: { label: string; value: number }[]; color?: string; height?: number }) {
    ...
    <div style={{ display: "flex", alignItems: "flex-end", gap: 8, height, paddingTop: 6 }}>
      {data.map((d, i) => (
        <div key={i} style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", gap: 6, minWidth: 0 }}>
          ...
          <div style={{ fontSize: "0.8125rem", color: "#5c6675", fontWeight: 700, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis", maxWidth: "100%" }}>{d.label}</div>
        </div>
      ))}
    </div>
  );
