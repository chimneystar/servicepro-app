...
export default function JobTypesEditor({ locale, types, currency }: { locale: Locale; types: JobType[]; currency: string }) {
  ...
  return (
    <div className="settings-section">
      ...
      {types.map((tp) => (
        <div key={tp.id} style={{ display: "flex", alignItems: "center", gap: 10, padding: "10px 0", borderTop: "1px solid #f1f4f9" }}>
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            <b>{tp.name}</b>
            <div style={{ fontSize: 12, color: "#5c6675" }}>{tp.duration_min} {he ? "דק׳" : "min"}{tp.default_price_minor ? ` · ${cur}${(tp.default_price_minor / 100).toFixed(2)}` : ""}</div>
          </div>
      ...
      {types.length === 0 && <div style={{ color: "#5c6675", fontSize: 13, padding: 8 }}>{he ? "עוד לא הוגדרו סוגי עבודה." : "No job types yet."}</div>}

      {editing !== undefined && (
        <Modal onClose={() => setEditing(undefined)} labelledBy={titleId} width={420}>
          <form action={formAction}>
            <h3 id={titleId} style={{ fontSize: 17, fontWeight: 800, marginBottom: 12 }}>{editing ? (he ? "עריכת סוג עבודה" : "Edit job type") : (he ? "סוג עבודה חדש" : "New job type")}</h3>
