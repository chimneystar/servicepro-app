  ...
  return (
    <div className="settings-section">
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
        <h3 style={{ fontSize: "0.9375rem", fontWeight: 800 }}>{he ? "סוגי עבודות" : "Job types"}</h3>
      ...
      {types.map((tp) => (
        <div key={tp.id} style={{ display: "flex", alignItems: "center", gap: 10, padding: "10px 0", borderTop: "1px solid #f1f4f9" }}>
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            <b>{tp.name}</b>
            <div style={{ fontSize: "0.75rem", color: "#5c6675" }}>{tp.duration_min} {he ? "דק׳" : "min"}{tp.default_price_minor ? ` · ${cur}${(tp.default_price_minor / 100).toFixed(2)}` : ""}</div>
          </div>
          <button type="button" onClick={() => open(tp)} style={mini} aria-label={he ? "עריכה" : "Edit"}>✎</button>
          <button type="button" onClick={() => del(tp.id)} style={{ ...mini, background: "#fff0f0", color: "#b93545" }} aria-label={he ? "מחיקה" : "Delete"}>×</button>
        </div>
      ))}
      {types.length === 0 && <div style={{ color: "#5c6675", fontSize: "0.8125rem", padding: 8 }}>{he ? "עוד לא הוגדרו סוגי עבודה." : "No job types yet."}</div>}

      {editing !== undefined && (
        <Modal onClose={() => setEditing(undefined)} labelledBy={titleId} width={420}>
          <form action={formAction}>
            <h3 id={titleId} style={{ fontSize: "1.0625rem", fontWeight: 800, marginBottom: 12 }}>{editing ? (he ? "עריכת סוג עבודה" : "Edit job type") : (he ? "סוג עבודה חדש" : "New job type")}</h3>
            {editing && <input type="hidden" name="id" value={editing.id} />}
            <input type="hidden" name="color" value={color} />
            <label style={{ display: "block" }}><L>{he ? "שם" : "Name"}</L><input name="name" defaultValue={editing?.name ?? ""} style={inp} required /></label>
            <L>{he ? "צבע" : "Color"}</L>
            ...
            <div style={two}>
              <div><label style={{ display: "block" }}><L>{he ? "משך בדקות" : "Duration (min)"}</L><input name="duration" type="number" defaultValue={editing?.duration_min ?? 60} style={inp} /></label></div>
              <div><label style={{ display: "block" }}><L>{he ? "מחיר ברירת מחדל" : "Default price"}</L><input name="price" type="number" step="0.01" defaultValue={editing ? (editing.default_price_minor / 100).toFixed(2) : ""} style={inp} placeholder="0.00" /></label></div>
            </div>
