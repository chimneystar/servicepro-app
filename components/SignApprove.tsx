  ...
  return (
    <div style={{ border: "1px solid #e2e8f0", borderRadius: 14, padding: 16 }}>
      <label style={{ display: "block" }}>
        <span style={{ fontSize: "0.8125rem", fontWeight: 700, color: "#334155", display: "block", marginBottom: 6 }}>{t(locale, "doc.your_name")}</span>
      ...
      <button type="button" onClick={approve} disabled={busy} style={{ width: "100%", background: "#15803d", color: "#fff", border: "none", borderRadius: 12, padding: 15, fontSize: "1rem", fontWeight: 800, cursor: "pointer", marginTop: 12 }}>
        {busy ? "…" : <><span aria-hidden="true">✓</span> {t(locale, "doc.approve")}</>}
      </button>
    </div>
  );
}
