...
export default function JobFieldTools({
  jobId,
  onMyWayAt,
  startedAt,
  completedAt,
  ...
  return (
    ...
    >
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginBottom: 10 }}>
        <button
          type="button"
          onClick={() => run(() => setOnMyWay(jobId))}
          disabled={pending || done}
          style={{ ...b, background: onMyWayAt ? "#1e40af" : "#2563eb" }}
        >
        ...
        {!clockedIn ? (
          ...
          <button
            type="button"
            onClick={() => run(() => clockOut(jobId))}
            disabled={pending}
            style={{ ...b, background: "#b45309" }}
          >
            ⏸️ Clock out
          </button>
        )}
      ...
      {!done ? (
        <button
          ...
          disabled={pending}
          style={{ ...b, width: "100%", background: "#fff", color: "#15803d" }}
        >
          ✅ Complete job {startedAt ? "" : ""}
        </button>
      ) : (
        <div
          style={{
...
function SignModal({ jobId, onClose }: { jobId: string; onClose: () => void }) {
  ...
  return (
    <Modal onClose={onClose} labelledBy={titleId} width={560}>
      <h3
        id={titleId}
        style={{ fontSize: "1.125rem", fontWeight: 800, marginBottom: 4, color: "#0b1524" }}
      >
        Complete job
      </h3>
      ...
      <div style={{ ...lbl, marginTop: 10 }} id={`${titleId}-sig`}>
        Signature
      </div>
      <canvas
        ref={canvasRef}
      ...
      <div style={{ display: "flex", gap: 8, marginTop: 12 }}>
        <button
          ...
          onClick={save}
          disabled={pending}
          style={{ ...b, background: "#15803d", color: "#fff", flex: 1 }}
        >
        ...
        <button
          onClick={clear}
          type="button"
          style={{ ...b, background: "#eef2f8", color: "#2563eb" }}
        >
          Clear
        </button>
        <button
          onClick={onClose}
          type="button"
          style={{ ...b, background: "#e2e9f4", color: "#2563eb" }}
        >
          Cancel
        </button>
      </div>
