...
export default function MessageComposer({ phone }: { phone: string }) {
  ...
  return (
    <div style={{ position: "sticky", bottom: 0, background: "#eef3fb", paddingTop: 8, display: "flex", gap: 8 }}>
      <input value={text} onChange={(e) => setText(e.target.value)} onKeyDown={(e) => e.key === "Enter" && send()} placeholder="Type a message…" style={{ flex: 1, border: "1px solid #e2e8f0", borderRadius: 22, padding: "12px 16px", fontSize: "1rem", outline: "none", background: "#fff" }} />
      <button onClick={send} disabled={pending} style={{ background: "#2563eb", color: "#fff", border: "none", borderRadius: "50%", width: 46, height: 46, fontSize: "1.125rem", cursor: "pointer", flexShrink: 0 }}>➤</button>
    </div>
  );
}
