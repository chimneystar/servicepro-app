...
export default function JobAddressForm({ jobId, jobAddress, jobCity }: { jobId: string; jobAddress: string | null; jobCity: string | null }) {
  ...
  return (
    <form action={submit} style={{ background: "#f8fbff", border: "1px solid #e2e8f0", borderRadius: 12, padding: 14, marginTop: 10 }}>
      <div style={{ fontSize: 14, fontWeight: 700, color: "#5c6675", marginBottom: 8 }}>{he ? "כתובת העבודה. אם משאירים ריק, תוצג כתובת הלקוח." : "Job address. Leave blank to use the customer address."}</div>
...
const link: React.CSSProperties = { background: "#eef2f8", color: "#2563eb", border: "none", borderRadius: 9, padding: "8px 12px", fontWeight: 700, fontSize: 14, cursor: "pointer", marginTop: 6 };
