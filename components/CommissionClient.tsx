  ...
  return (
    <div>
      <div style={{ display: "flex", gap: 12, alignItems: "center", flexWrap: "wrap", marginBottom: 14 }}>
        <label style={{ fontSize: "0.8125rem", fontWeight: 700, color: "#334155" }}>Credit-card fee %
          <input value={ccFee} onChange={(e) => setCcFee(e.target.value)} type="number" step="0.01" style={{ width: 80, marginInlineStart: 8, border: "1px solid #e2e8f0", borderRadius: 8, padding: "7px 10px", fontSize: "0.9375rem" }} />
        </label>
        <button type="button" onClick={exportCsv} style={{ background: "#2563eb", color: "#fff", border: "none", borderRadius: 10, padding: "9px 14px", fontWeight: 700, fontSize: "0.875rem", cursor: "pointer" }}><span aria-hidden="true">⬇</span> Export CSV</button>
        <div style={{ marginInlineStart: "auto", fontWeight: 800, fontSize: "1rem" }}>Total payout: <span style={{ color: "#15803d" }}>{m(totalCommission)}</span></div>
      </div>
          ...
          return (
            <div key={r.profileId} style={{ background: "#fff", border: "1px solid #e2e8f0", borderRadius: 14, padding: 16 }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
                ...
                <div style={{ fontWeight: 800, color: "#15803d", fontSize: "1rem" }}>{m(c.commission)}</div>
              </div>
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(90px,1fr))", gap: 8, margin: "10px 0", fontSize: "0.8125rem" }}>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                <label style={{ display: "flex", alignItems: "center", gap: 8 }}>
                  <span style={{ fontSize: "0.8125rem", color: "#5c6675", fontWeight: 600 }}>Commission</span>
                  <input type="number" value={pcts[r.profileId] ?? 0} disabled={!canEditPct} onChange={(e) => setPcts({ ...pcts, [r.profileId]: parseInt(e.target.value, 10) || 0 })} style={{ width: 70, border: "1px solid #e2e8f0", borderRadius: 8, padding: "7px 10px", fontSize: "0.9375rem" }} />
                </label>
                <span style={{ fontSize: "0.8125rem", color: "#5c6675" }}>%</span>
                {canEditPct && <button type="button" onClick={() => savePct(r.profileId)} disabled={pending} style={{ background: "#eef2f8", color: "#2563eb", border: "none", borderRadius: 8, padding: "7px 12px", fontWeight: 700, fontSize: "0.8125rem", cursor: "pointer" }}>{saved === r.profileId ? <><span aria-hidden="true">✓</span> Saved</> : "Save %"}</button>}
              </div>
            </div>
          );
        })}
      ...
      <p style={{ color: "#5c6675", fontSize: "0.75rem", marginTop: 12 }}>Net = job revenue − job costs (entered on each job) − credit-card fees. Commission = net × the technician’s %. Only jobs in a “Done” status count.</p>
    </div>
  );
}

function KV({ label, v, strong }: { label: string; v: string; strong?: boolean }) {
  return <div style={{ background: "#f8fafc", borderRadius: 8, padding: "7px 10px" }}><div style={{ color: "#94a3b8", fontWeight: 700, fontSize: "0.8125rem" }}>{label}</div><b style={{ color: strong ? "#15803d" : "#0b1524" }}>{v}</b></div>;
}
