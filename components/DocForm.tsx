      ...
      {open && (
        <div style={overlay} onClick={(e) => e.target === e.currentTarget && setOpen(false)}>
          <form action={formAction} style={modal}>
            ...
            <button type="button" onClick={() => setRows([...rows, blankRow()])} style={{ ...btn, background: "#e2e9f4", color: "#2563eb", padding: "8px 12px", fontSize: 14, marginTop: 4 }}>➕ {t(locale, "doc.add_item")}</button>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginTop: 14 }}>
              ...
              <div style={{ alignSelf: "end", textAlign: "end", fontSize: 14, color: "#5c6675", paddingBottom: 10 }}>Items subtotal ≈ {sym}{(previewSubtotal / 100).toFixed(2)}</div>
            </div>
            <label style={lbl}>{t(locale, "form.notes")}</label>
            <textarea name="notes" rows={2} style={inp} />
            <div style={{ fontSize: 14, color: "#5c6675", marginTop: 8 }}>ℹ️ Tax & total are calculated on save (only taxable items are taxed). New items are saved to your library for reuse.</div>
...
const lbl: React.CSSProperties = { fontSize: 14, fontWeight: 700, color: "#334155", display: "block", margin: "6px 0 6px" };
const miniLbl: React.CSSProperties = { fontSize: 14, color: "#5c6675", fontWeight: 700, display: "block", marginBottom: 3 };
...
const photoBtn: React.CSSProperties = { background: "#e0ebff", color: "#2563eb", borderRadius: 8, padding: "7px 12px", fontWeight: 700, fontSize: 14, cursor: "pointer" };
