  ...
  return (
    <div style={{ background: "#fff", border: "1px solid #e2e8f0", borderRadius: 12, padding: 14, marginTop: 12 }}>
      ...
      <div style={{ display: "flex", gap: 8 }}>
        <input value={text} onChange={(e) => setText(e.target.value)} onKeyDown={(e) => e.key === "Enter" && add(text)} placeholder={he ? "הוספת תגית…" : "Add a tag…"} aria-label={he ? "הוספת תגית…" : "Add a tag…"} style={{ flex: 1, border: "1px solid #e2e8f0", borderRadius: 10, padding: "9px 12px", fontSize: 16, outline: "none" }} />
      ...
      <div style={{ display: "flex", flexWrap: "wrap", gap: 6, marginTop: 8 }}>
        {(he ? SUGGESTED.he : SUGGESTED.en).filter((s) => !list.includes(s)).map((s) => (
          <button key={s} type="button" onClick={() => add(s)} disabled={pending} style={{ background: "#f1f5fb", color: "#5c6675", border: "none", borderRadius: 20, padding: "4px 10px", fontSize: 12, fontWeight: 600, cursor: "pointer" }}>+ {s}</button>
        ))}
      </div>
