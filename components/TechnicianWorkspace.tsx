...
export type TechJob = {
  ...
  customers: { name: string; phone: string; address: string | null; city: string | null } | null;
};
...
export default function TechnicianWorkspace({
  ...
  const [queued, setQueued] = useState(() =>
    typeof window === "undefined" ? 0 : readOutbox().length,
  );
  ...
  useEffect(() => {
    ...
    try {
      localStorage.setItem(
        "servicepro:tech-snapshot",
        JSON.stringify({ savedAt: new Date().toISOString(), jobs }),
      );
    } catch {}
    return () => {
  ...
  async function enableNotifications() {
    if (!("serviceWorker" in navigator) || !("PushManager" in window))
      return setNotice(
        he ? "המכשיר הזה לא תומך בהתראות." : "This device does not support push notifications.",
      );
    if (!vapidPublicKey)
      return setNotice(
        he
          ? "ההתראות מוכנות באפליקציה וממתינות למפתח השליחה של העסק."
          : "Push is installed and waiting for the business delivery key.",
      );
    const permission = await Notification.requestPermission();
    if (permission !== "granted")
      return setNotice(
        he
          ? "ההתראות לא הופעלו. אפשר לשנות זאת בהגדרות המכשיר."
          : "Notifications were not enabled. You can change this in device settings.",
      );
    const registration = await navigator.serviceWorker.ready;
    const subscription = await registration.pushManager.subscribe({
      userVisibleOnly: true,
      applicationServerKey: vapidBytes(vapidPublicKey),
    });
    const payload = subscription.toJSON();
    const response = await fetch("/api/devices/push", {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ ...payload, deviceName: navigator.userAgent, locale }),
    });
    if (!response.ok)
      return setNotice(
        he ? "לא הצלחנו לשמור את ההתראות." : "We couldn't save notification access.",
      );
      ...
      setNotice(
        result.sent
          ? he
            ...
            : "Job updated."
          : he
            ? "השינוי נשמר ויישלח כשהחיבור יחזור."
            : "Change saved and will send when you're back online.",
      );
      if (result.sent) location.reload();
    } else
      setNotice(
        he
          ? "השינוי נשמר במכשיר ויישלח כשהחיבור יחזור."
          : "Change saved on this device and will send when you're back online.",
      );
  }
  ...
  async function toggleLocation() {
    if (sharingLocation) {
      ...
      await fetch("/api/devices/location", { method: "DELETE" });
      setNotice(he ? "שיתוף המיקום הופסק." : "Location sharing is off.");
      return;
    }
    if (!("geolocation" in navigator)) {
      setNotice(he ? "המכשיר הזה לא תומך בשיתוף מיקום." : "This device cannot share location.");
      return;
    }
    locationWatch.current = navigator.geolocation.watchPosition(
      async (position) => {
        const now = Date.now();
        if (now - lastLocationSent.current < 60000) return;
        ...
        if (response.ok) {
          ...
          setNotice(
            he
              ? "המיקום משותף כל עוד יום העבודה פתוח."
              : "Location is shared while My workday stays open.",
          );
        }
      },
      () =>
        setNotice(he ? "לא התקבלה הרשאה לשיתוף מיקום." : "Location permission was not granted."),
  ...
  const list = (rows: TechJob[]) => (
    <div className="tech-job-list">
      {rows.map((job, index) => {
        ...
        const address = [job.job_address || customer?.address, job.job_city || customer?.city]
          .filter(Boolean)
          .join(", ");
        return (
          <article className="tech-job-card" key={job.id}>
            <Link href={`/jobs/${job.id}`}>
              <span className="tech-stop">{index + 1}</span>
              <div>
                <small>
                ...
                <p>
                  {customer?.name || (he ? "לקוח" : "Customer")}
                  {address ? ` · ${address}` : ""}
                </p>
              </div>
  ...
  return (
    <div className="tech-workspace">
      <header className="tech-hero">
        <div>
          <span>{he ? "יום העבודה שלי" : "My workday"}</span>
          <h1>
            {todayJobs.length
              ? he
                ? `${todayJobs.length} עבודות להיום`
          ...
          <p>
            {online
              ...
              : he
                ? "אין חיבור · השינויים ימתינו לסנכרון"
                : "Offline · changes will wait to sync"}
          </p>
        </div>
        <i className={online ? "online" : "offline"}>{online ? "●" : "○"}</i>
      </header>
      <div className="tech-quick-actions">
        <Link href="/route">{he ? "המסלול שלי" : "My route"}</Link>
        <button type="button" onClick={enableNotifications}>
          {he ? "התראות" : "Notifications"}
        </button>
        <button
          type="button"
          className={sharingLocation ? "location-on" : ""}
      ...
      {(notice || queued > 0) && (
        <div className="tech-notice" role="status">
          {notice || `${queued} ${he ? "עדכונים ממתינים לסנכרון" : "updates waiting to sync"}`}
        </div>
      )}
      <section>
        <h2>{he ? "היום" : "Today"}</h2>
        {todayJobs.length ? (
          ...
          <div className="tech-empty">
            {he ? "כשתשובץ עבודה היא תופיע כאן." : "Assigned jobs will appear here."}
          </div>
        )}
      </section>
      {laterJobs.length > 0 && (
        <section>
          <h2>{he ? "בהמשך" : "Coming up"}</h2>
          {list(laterJobs)}
        </section>
      )}
    </div>
