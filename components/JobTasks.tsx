    ...
    <div>
      ...
      <div style={{ display: "flex", gap: 8, marginBottom: 12 }}>
        <input
          ...
          placeholder={he ? "הוספת משימה…" : "Add a task…"}
      ...
      <div style={{ display: "grid", gap: 8 }}>
        {tasks.map((t) => (
          <div key={t.id} style={row}>
            <input
              ...
              onChange={() =>
                run(
                  () => toggleJobTask(t.id, !t.done, jobId),
                  () => router.refresh(),
                )
              }
            ...
            <button
              ...
              onClick={() =>
                run(
                  () => deleteJobTask(t.id, jobId),
                  () => router.refresh(),
                )
              }
              disabled={pending}
              style={xBtn}
              aria-label={he ? `מחיקת המשימה "${t.title}"` : `Delete task "${t.title}"`}
            >
              🗑️
            </button>
          </div>
        ...
        {tasks.length === 0 && (
          <div className="rempty">{he ? "עוד אין משימות." : "No tasks yet."}</div>
        )}
      </div>
