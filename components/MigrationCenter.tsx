...
export default function MigrationCenter({ locale, batches, canRollback }: { locale: Locale; batches: Batch[]; canRollback: boolean }) {
  ...
  return <div className="migration-page">
    ...
    <section className="migration-flow"><div className="migration-steps"><b className="on">1</b><span>{he ? "מקור" : "Source"}</span><b className={filename ? "on" : ""}>2</b><span>{he ? "קובץ" : "File"}</span><b className={rows.length ? "on" : ""}>3</b><span>{he ? "בדיקה וייבוא" : "Review & import"}</span></div><div className="migration-source-grid">{(["workiz","housecall_pro","spreadsheet"] as Source[]).map((item) => <button type="button" className={source === item ? "active" : ""} onClick={() => setSource(item)} key={item}>{item === "housecall_pro" ? "Housecall Pro" : item === "workiz" ? "Workiz" : (he ? "גיליון נתונים" : "Spreadsheet")}</button>)}</div><label className="migration-file"><input type="file" accept=".csv,text/csv" onChange={(event) => choose(event.target.files?.[0])} /><strong>{filename || (he ? "בחירת קובץ CSV" : "Choose a CSV file")}</strong><small>{he ? "הקובץ נשאר במכשיר עד שתאשרו את הייבוא." : "The file stays on this device until you approve the import."}</small></label>{rows.length > 0 && <><div className="migration-preview"><header><strong>{he ? "תצוגה מקדימה" : "Preview"}</strong><b>{rows.length} {he ? "לקוחות תקינים" : "valid customers"}</b></header>{rows.slice(0,6).map((row,index) => <div key={`${row.name}-${index}`}><strong>{row.name}</strong><span>{[row.phone,row.email,row.city].filter(Boolean).join(" · ")}</span></div>)}</div><button type="button" className="migration-import" disabled={pending} onClick={run}>{pending ? (he ? "מייבאים…" : "Importing…") : (he ? `ייבוא ${rows.length} לקוחות` : `Import ${rows.length} customers`)}</button></>}{notice && <p className="migration-notice" role="status">{notice}</p>}</section>
