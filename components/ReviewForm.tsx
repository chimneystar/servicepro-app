  ...
  return (
    <div style={{ background: "#fff", border: "1px solid #e2e8f0", borderRadius: 14, padding: 16, marginBottom: 16 }}>
      ...
      <div style={{ display: "flex", gap: 4, marginBottom: 10 }}>
        {[1, 2, 3, 4, 5].map((n) => (
          <button key={n} type="button" onClick={() => setRating(n)} aria-label={`Rate ${n} star${n === 1 ? "" : "s"}`} style={{ background: "none", border: "none", fontSize: "1.625rem", cursor: "pointer", color: n <= rating ? "#eab308" : "#d7dce6", lineHeight: 1 }}>★</button>
        ))}
      </div>
