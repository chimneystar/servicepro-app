...
const META: Record<string, { en: [string, string]; he: [string, string]; icon: string }> = {
  booked: {
    en: ["When a job is booked", "Sent right after you create the appointment"],
    he: ["כשקובעים עבודה", "נשלחת מיד אחרי יצירת העבודה ביומן"],
    icon: "📅",
  },
  day_before: {
    en: ["Day before the job", "Sent the day before the scheduled date"],
    he: ["יום לפני העבודה", "נשלחת יום לפני המועד שקבעתם"],
    icon: "⏰",
  },
  on_the_way: {
    en: ["Technician on the way", "Sent when you mark the technician as on the way"],
    he: ["הטכנאי בדרך", "נשלחת כשמסמנים שהטכנאי יצא לדרך"],
    icon: "🚚",
  },
  completed: {
    en: ["After the job is done", "Sent when the job is marked complete"],
    he: ["אחרי שהעבודה הושלמה", "נשלחת כשמסמנים שהעבודה הסתיימה"],
    icon: "✓",
  },
};
...
export default function MessageTemplatesEditor({
  ...
  return (
    <div>
      ...
      >
        {he ? "אפשר לערוך כל הודעה. השדות " : "Customize every message. The fields "}
        <b>{"{name}"}</b>, <b>{"{service}"}</b>, <b>{"{date}"}</b>, <b>{"{time}"}</b>,{" "}
        <b>{"{business}"}</b>
        {he
          ? " יתמלאו אוטומטית בפרטי הלקוח והעבודה."
          : " are filled automatically with the customer and job details."}
      </div>
      {ORDER.map((trig) => (
        <TemplateRow
          key={trig}
          locale={locale}
          trigger={trig}
          tpl={byTrigger.get(trig) ?? { trigger: trig, enabled: false, body: "" }}
        />
      ))}
    </div>
  );
  ...
  return (
    <form
      ...
      style={{
        ...
        padding: 16,
        marginBottom: 12,
        boxShadow: "0 6px 18px rgba(15,42,94,.05)",
      }}
    >
