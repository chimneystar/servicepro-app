...
export default function JobItems({
  ...
  function del(id: string) {
    setErr(null);
    start(async () => {
      const r = await deleteJobItem(id, jobId);
      if (!r.ok) setErr(r.error ?? (he ? "לא הצלחנו למחוק" : "Could not delete"));
      else router.refresh();
    });
  }
  function makeInvoice() {
    if (!confirm(he ? "ליצור חשבונית מהפריטים האלה?" : "Create an invoice from these items?"))
      return;
    setErr(null);
    // A refused conversion used to do nothing at all: no redirect, no message.
    start(async () => {
      const r = await createInvoiceFromJob(jobId);
      if (!r.ok) setErr(r.error ?? (he ? "לא הצלחנו ליצור חשבונית" : "Could not create invoice"));
    ...
    <div>
      <div className="rlist">
        {items.map((it) => (
          <div className="ritem" key={it.id}>
            ...
            <div className="rend">
              <b>{money(lineTotal(it), currency)}</b>
            </div>
            {canEdit && (
              <button
                type="button"
                ...
                disabled={pending}
                style={xBtn}
                aria-label={he ? `מחיקת "${it.description}"` : `Delete "${it.description}"`}
              >
                🗑️
              </button>
            )}
        ...
        {items.length === 0 && (
          <div className="rempty">{he ? "עוד אין פריטים." : "No items yet."}</div>
        )}
      </div>
      {err && !adding && (
      ...
      {canEdit && items.length > 0 && (
        <button
          ...
          onClick={makeInvoice}
          disabled={pending}
          style={{ ...btn, background: "#e6f6ec", color: "#15803d", marginInlineStart: 8 }}
        >
          {he ? "יצירת חשבונית מהפריטים" : "Create invoice from items"}
        </button>
      )}
