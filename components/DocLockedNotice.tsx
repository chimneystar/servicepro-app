...
export default function DocLockedNotice({ kind, id, number, reason }: {
  ...
  return (
    <div style={{ maxWidth: 640 }}>
      <Link href={`${base}/${id}`} style={{ color: "#2563eb", fontWeight: 700, fontSize: 14, textDecoration: "none" }}>‹ Back</Link>
      <h1 style={{ fontSize: 24, fontWeight: 800, margin: "8px 0 14px" }}>{label} #{number} is locked</h1>
      <div style={{ background: "#fdf1dc", border: "1px solid #f5d99b", borderRadius: 12, padding: "14px 16px", color: "#7c4a03", fontSize: 14, lineHeight: 1.6 }}>
        {reason}
      </div>
      <p style={{ fontSize: 13, color: "#5c6675", marginTop: 12, lineHeight: 1.6 }}>
      ...
      <Link href={`${base}/${id}`} style={{ display: "inline-block", marginTop: 14, background: "#2563eb", color: "#fff", borderRadius: 10, padding: "10px 16px", fontWeight: 700, fontSize: 14, textDecoration: "none" }}>
