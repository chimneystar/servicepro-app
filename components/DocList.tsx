    ...
    <div className="rlist">
      ...
      {rows.length > 0 && (
        <div style={{ position: "relative" }}>
          ...
          <input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Search by number, customer, or status…" aria-label="Search by number, customer, or status…" style={{ width: "100%", border: "1px solid #e2e8f0", borderRadius: 12, padding: "11px 38px", fontSize: 16, outline: "none" }} />
        </div>
      )}
      {visible.map((r) => {
