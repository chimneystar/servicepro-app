...
export async function runReminders(): Promise<{ appointments: number; overdue: number }> {
  ...
  const { data: jobs } = await admin.from("jobs")
    .select("id, service, scheduled_date, start_time, organization_id, customers(name, phone)")
  ...
  for (const j of jobs ?? []) {
    ...
    try { const sid = await sendSms(cust.phone, body); await admin.from("sms_messages").insert({ organization_id: j.organization_id, job_id: j.id, to_phone: cust.phone, body, provider: "twilio", provider_message_id: sid, status: "sent", sent_at: new Date().toISOString() }); appointments++; } catch { /* skip */ }
  }

  // --- Overdue invoice nudges (unpaid > 14 days, at most weekly) ---
  const { data: invs } = await admin.from("invoices")
    .select("id, number, issue_date, organization_id, customers(name, phone)")
  ...
  for (const inv of invs ?? []) {
    ...
    try { const sid = await sendSms(cust.phone, body); await admin.from("sms_messages").insert({ organization_id: inv.organization_id, to_phone: cust.phone, body, provider: "twilio", provider_message_id: sid, status: "sent", sent_at: new Date().toISOString() }); overdue++; } catch { /* skip */ }
  }
