...
export const BULK_ACTIONS = Object.freeze([
  ...
  "invoices.statement",
  "customers.statement",
  "customers.opt_out_sms",
  "customers.opt_out_email",
]);
...
export function summarizeBulk(report, locale = "en") {
  ...
  if (report.skippedCount > 0) {
    ...
    parts.push(
      he
        ? `${report.skippedCount} דולגו — ${detail}`
        : `${report.skippedCount} skipped — ${detail}`,
    );
  }
  if (report.failedCount > 0) {
    const detail = report.failed
      ...
      .join("; ");
    parts.push(
      he ? `${report.failedCount} נכשלו — ${detail}` : `${report.failedCount} FAILED — ${detail}`,
    );
  }
...
export function selectionError(result, locale = "en") {
  ...
  if (result?.reason === "empty_selection")
    return he ? "לא נבחרו שורות" : "Select at least one row first";
  ...
  if (result?.reason === "invalid_id")
    return he ? "הבחירה אינה תקינה" : "That selection is not valid";
