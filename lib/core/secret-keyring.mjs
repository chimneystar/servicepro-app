...
export function parseKeyring(env = {}) {
  ...
  const activeVersion = Number.isInteger(parsedVersion) && parsedVersion >= 1 ? parsedVersion : null;
  if (activeVersion === null) {
    errors.push(`PAYMENT_SECRETS_KEY_VERSION must be a whole number >= 1 (got "${env.PAYMENT_SECRETS_KEY_VERSION}")`);
  }
  ...
  for (const entry of String(env.PAYMENT_SECRETS_KEYS ?? "").split(/[,\s]+/).filter(Boolean)) {
    ...
    if (separator < 1 || !Number.isInteger(version) || version < 1) {
      errors.push("PAYMENT_SECRETS_KEYS entries must look like <version>:<key>, e.g. 1:base64:AAAA...");
      continue;
    }
...
export function encryptWithKeyring(plaintext, keyring) {
  ...
  return {
    payload: [ENVELOPE_VERSION, iv.toString("base64url"), tag.toString("base64url"), encrypted.toString("base64url")].join(":"),
...
export function planRotation(rows, keyring) {
  ...
  const plan = { activeVersion, toRotate: [], alreadyCurrent: [], unreadable: [], missingVersions: [] };
  const missing = new Set();

  for (const row of rows ?? []) {
    const version = Number(row?.key_version ?? 1);
    const id = row?.organization_id ?? row?.id ?? null;
    if (version === activeVersion) { plan.alreadyCurrent.push(id); continue; }
