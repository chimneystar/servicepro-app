...
export function documentLock(kind, doc = {}) {
  ...
  if (k === "estimate" && (status === "approved" || status === "rejected"))
    return at(LOCK_CODES.DECIDED);
 ...
 *
 * The `@returns` is a discriminated union, and it is load-bearing: without it
 * TypeScript infers `version: number | undefined` for the success case, so
 * `.eq("version", version.version)` — the whole optimistic-concurrency check —
 * does not compile against the typed client. The union says what the function
 * has always guaranteed: `version` is present exactly when `ok` is true.
 *
 * @returns {{ ok: true, version: number }
 *   | { ok: false, code: "version_missing" | "stale_write", error: string }}
 */
export function assertVersionMatch(kind, loaded, current, { he = false } = {}) {
  ...
  const noun =
    k === "invoice" ? { en: "invoice", he: "החשבונית" } : { en: "estimate", he: "הצעת המחיר" };
  return {
    ok: false,
    code: "stale_write",
    error: he
...
export function validateVoid(kind, doc = {}, { reason = "", collectedMinor = 0, he = false } = {}) {
  ...
  if (doc?.deleted_at) {
    return {
      ok: false,
      code: "deleted",
      error: he ? "המסמך נמחק." : "This document has been deleted.",
    };
  }
 ...
 */
export function invoiceOutstandingMinor(
  invoice = {},
  { collectedMinor = 0, creditNotes = [] } = {},
) {
 ...
 *
 * A discriminated `@returns`, for the same reason as assertVersionMatch above:
 * `amountMinor` and `reason` are what the caller writes into `credit_notes`,
 * and inferred as `number | undefined` / `string | undefined` they cannot be.
 * The union states the guarantee the function already makes.
 *
 * @returns {{ ok: true, amountMinor: number, reason: string }
 *   | { ok: false, code: string, error: string }}
 */
export function validateCreditNote(
  invoice = {},
  creditNotes = [],
  { amountMinor = 0, reason = "", he = false } = {},
) {
  if (!invoice || invoice.deleted_at) {
    return {
      ok: false,
      code: "not_found",
      error: he ? "החשבונית לא נמצאה." : "That invoice was not found.",
    };
  }
  ...
  if (!Number.isFinite(amount) || !Number.isInteger(amount)) {
    return {
      ok: false,
      code: "invalid_amount",
      error: he ? "צריך להזין סכום זיכוי." : "Enter a credit amount.",
    };
  }
  if (amount <= 0) {
    return {
...
export function validateCreditNoteCancel(note = {}, { reason = "", he = false } = {}) {
  if (!note || !note.id) {
    return {
      ok: false,
      code: "not_found",
      error: he ? "הזיכוי לא נמצא." : "That credit note was not found.",
    };
  }
  if (String(note.status ?? ACTIVE_CREDIT_STATUS) !== ACTIVE_CREDIT_STATUS) {
    return {
      ...
      error: he ? "הזיכוי כבר בוטל." : "That credit note is already cancelled.",
    };
  }
  const problem = reasonProblem(reason, he);
  if (problem) return { ok: false, code: "reason_required", error: problem };
...
export function validateReopen(
  kind,
  doc = {},
  { reason = "", collectedMinor = 0, he = false } = {},
) {
  ...
  if (!lock.locked) {
    return {
      ...
      code: "not_locked",
      error: he
        ? "הצעת המחיר אינה נעולה — אפשר לערוך אותה ישירות."
        : "This estimate is not locked; you can edit it directly.",
    };
  }
