...
export function planMigrations(input) {
  ...
  if (allFilenames) {
    ...
    const classified = classifyFiles(files.map((f) => f.filename), nonMigrations);
    problems.push(...classified.problems);
    excluded = classified.excluded;
  }
...
export function renderSequenceTable({ migrations, excluded = [], descriptions = {} }) {
  ...
  migrations.forEach((m, i) => {
    const desc = descriptions[m.version] ?? "**MISSING — add a description to db/migrations.manifest.json**";
    lines.push(`| ${i + 1} | \`${m.version}\` | \`${m.filename}\` | ${desc} |`);
  });

  if (excluded.length) {
    lines.push("", "### `.sql` files in `db/` that are NOT migrations", "", "| File | Why |", "|---|---|");
