...
export function addMonthsISO(iso, months) {
  ...
  if (!Number.isFinite(step))
    throw new TypeError(`addMonthsISO: months must be a number, got ${JSON.stringify(months)}`);
...
export function recurringJobKey(planId, dueDate) {
  ...
  if (!id)
    throw new TypeError(`recurringJobKey: a plan id is required, got ${JSON.stringify(planId)}`);
