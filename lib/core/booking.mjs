...
function zoneOffsetMs(instantMs, timeZone) {
  ...
  return (
    Date.UTC(parts.year, parts.month - 1, parts.day, parts.hour, parts.minute, parts.second) - whole
  );
}
...
export function buildBookingSlots(input) {
  ...
  const daysAhead =
    civilDayNumber(target.year, target.month, target.day) -
...
export function evaluateServiceArea(postalCode, city, areas) {
  ...
  const hit = evaluable.some((area) => {
    ...
    if (area.area_type === "zip")
      return values.some((value) => value.trim().toLowerCase() === postal);
