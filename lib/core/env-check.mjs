...
export function checkEnv(env) {
  ...
  if (present(env, "PAYMENT_SECRETS_KEY_VERSION")) {
    ...
    if (!Number.isInteger(version) || version < 1) {
      fatal.push(`PAYMENT_SECRETS_KEY_VERSION must be a whole number >= 1 (got "${env.PAYMENT_SECRETS_KEY_VERSION}").`);
    }
  }
  if (present(env, "PAYMENT_SECRETS_KEYS")) {
    for (const entry of String(env.PAYMENT_SECRETS_KEYS).split(/[,\s]+/).filter(Boolean)) {
      ...
      if (keyByteLength(entry.slice(separator + 1).replace(/^(hex|base64):/, "")) !== 32) {
        fatal.push(`PAYMENT_SECRETS_KEYS version ${version} must decode to 32 bytes. Stored tokens on that version would be unreadable.`);
      }
    }
  }
