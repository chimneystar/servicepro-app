...
export const CAPABILITIES = [
  {
    name: "Server-side data access (webhooks, cron, health)",
    vars: ["SUPABASE_SERVICE_ROLE_KEY"],
    required: true,
  },
  {
    name: "Scheduled automation (reminders, reconciliation, data retention)",
    vars: ["CRON_SECRET"],
    required: false,
  },
  {
    name: "Card and ACH payments (Helcim)",
    vars: [
      "HELCIM_PARTNER_TOKEN",
      "HELCIM_CONNECTED_WEBHOOK_VERIFIER",
      "HELCIM_PAYMENT_WEBHOOK_VERIFIER",
      "PAYMENT_SECRETS_KEY",
    ],
    required: false,
  },
  ...
  {
    name: "Push notifications to technicians (Web Push / VAPID)",
...
export function checkEnv(env) {
  ...
  for (const capability of CAPABILITIES) {
    ...
    if (capability.required) {
      fatal.push(`${capability.name} — missing: ${missing.join(", ")}`);
      continue;
    }
    if (missing.length < capability.vars.length) {
      warnings.push(
        `${capability.name} is PARTIALLY configured — missing: ${missing.join(", ")}. It will fail at the moment it is used.`,
      );
    } else {
  ...
  for (const [name, expected, shape] of vapid) {
    ...
    if (bytes !== expected) {
      warnings.push(
        `${name} must be ${shape} (got ${bytes ?? "undecodable"} bytes). Push notifications would be refused by the push service on every send.`,
      );
    }
  }
  if (present(env, "VAPID_SUBJECT") && !/^(mailto:|https:\/\/)/.test(env.VAPID_SUBJECT.trim())) {
    warnings.push(
      "VAPID_SUBJECT must be a mailto: or https: address. Push notifications would be refused on every send.",
    );
  }
  ...
  if (present(env, "PAYMENT_SECRETS_KEY")) {
    ...
    if (bytes !== 32) {
      fatal.push(
        `PAYMENT_SECRETS_KEY must decode to 32 bytes (got ${bytes ?? "undecodable"}). Card payments would fail at first use.`,
      );
    }
  }

  // The rotation keyring (ledger 6b.9). A malformed retired key is only
  // discovered when a rotation tries to read a row it cannot decrypt, by which
  // point the operator has already changed PAYMENT_SECRETS_KEY.
  if (present(env, "PAYMENT_SECRETS_KEY_VERSION")) {
    const version = Number(String(env.PAYMENT_SECRETS_KEY_VERSION).trim());
  ...
  if (present(env, "PAYMENT_SECRETS_KEYS")) {
    for (const entry of String(env.PAYMENT_SECRETS_KEYS)
      ...
      if (separator < 1 || !Number.isInteger(version) || version < 1) {
        fatal.push("PAYMENT_SECRETS_KEYS entries must look like <version>:<key>, e.g. 1:hex:abc…");
        continue;
      }
      if (keyByteLength(entry.slice(separator + 1).replace(/^(hex|base64):/, "")) !== 32) {
        fatal.push(
          `PAYMENT_SECRETS_KEYS version ${version} must decode to 32 bytes. Stored tokens on that version would be unreadable.`,
        );
      }
    }
  }
