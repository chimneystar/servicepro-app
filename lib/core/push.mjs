...
import { createECDH, createHmac, createCipheriv, createPrivateKey, randomBytes, sign as signWith, timingSafeEqual } from "node:crypto";
...
export function checkVapidKeys(publicKey, privateKey) {
  ...
  if (priv.length !== 32) return { ok: false, reason: "private_key_length" };
  ...
  if (derived.length !== pub.length || !timingSafeEqual(derived, pub)) return { ok: false, reason: "key_pair_mismatch" };
...
export function vapidStatus(env) {
  ...
  if (!/^(mailto:|https:\/\/)/.test(subject)) return { available: false, reason: "invalid_subject" };
...
export function pushUnavailableMessage(reason, locale = "en") {
  const en = {
    not_configured: "Push delivery is not configured — the business has no notification keys yet, so nothing can be delivered.",
    missing_public_key: "Push delivery is incomplete: NEXT_PUBLIC_VAPID_PUBLIC_KEY is missing. Nothing will be delivered.",
    missing_private_key: "Push delivery is incomplete: VAPID_PRIVATE_KEY is missing. Nothing will be delivered.",
    missing_subject: "Push delivery is incomplete: VAPID_SUBJECT is missing. Nothing will be delivered.",
    invalid_subject: "Push delivery is misconfigured: VAPID_SUBJECT must be a mailto: or https: address.",
    private_key_length: "Push delivery is misconfigured: VAPID_PRIVATE_KEY must be a 32-byte base64url key.",
    public_key_format: "Push delivery is misconfigured: NEXT_PUBLIC_VAPID_PUBLIC_KEY must be a 65-byte uncompressed base64url key.",
    private_key_invalid: "Push delivery is misconfigured: VAPID_PRIVATE_KEY is not a valid P-256 key.",
    key_pair_mismatch: "Push delivery is misconfigured: the VAPID public and private keys are not a pair. Every send would be refused.",
    no_devices: "No device has enabled notifications for this person yet.",
  };
  const he = {
    not_configured: "משלוח ההתראות אינו מוגדר — לעסק אין עדיין מפתחות התראות, ולכן שום התראה לא תישלח.",
    missing_public_key: "הגדרת ההתראות חסרה: NEXT_PUBLIC_VAPID_PUBLIC_KEY לא הוגדר. שום התראה לא תישלח.",
    ...
    private_key_length: "הגדרת ההתראות שגויה: VAPID_PRIVATE_KEY חייב להיות מפתח base64url באורך 32 בתים.",
    public_key_format: "הגדרת ההתראות שגויה: NEXT_PUBLIC_VAPID_PUBLIC_KEY חייב להיות מפתח base64url באורך 65 בתים.",
  ...
  return table[reason] ?? (locale === "he" ? "משלוח ההתראות אינו זמין." : "Push delivery is unavailable.");
}
 ...
 */
export function buildVapidAuthorization({ endpoint, subject, publicKey, privateKey, now = Date.now(), ttlSeconds = 12 * 3600 }) {
 ...
 */
export function encryptPushPayload({ payload, p256dh, auth, ephemeralPrivateKey = null, salt = randomBytes(16) }) {
...
export function jobAssignedNotification({ locale = "en", service, customerName, scheduledDate, startTime, jobId }) {
  const he = locale === "he";
