...
export function staffNotification(input) {
  ...
  if (type === "job_assigned" || type === "job_unassigned") {
    ...
    return {
      title: locale === "he"
        ? (assigned ? "עבודה חדשה שויכה אליך" : "עבודה הוסרה ממך")
        : (assigned ? "New job assigned to you" : "A job was taken off your list"),
  ...
  return {
    ...
    body: [
      customer,
      invoice ? (locale === "he" ? `חשבונית #${invoice}` : `Invoice #${invoice}`) : "",
    ].filter(Boolean).join(" · ") || (locale === "he" ? "תשלום חדש נרשם" : "A new payment was recorded"),
