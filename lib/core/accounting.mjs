...
const COLUMNS = {
  quickbooks: {
    invoices: ["InvoiceNo", "Customer", "InvoiceDate", "DueDate", "Item(Product/Service)", "ItemDescription", "ItemQuantity", "ItemRate", "ItemAmount", "Taxable", "TaxAmount", "TotalAmount", "PrivateNote"],
    payments: ["RefNumber", "Customer", "PaymentDate", "PaymentMethod", "InvoiceNo", "Amount", "PrivateNote"],
    expenses: ["RefNumber", "Payee", "ExpenseDate", "Category", "Amount", "Memo"],
  },
  xero: {
    invoices: ["*ContactName", "*InvoiceNumber", "*InvoiceDate", "*DueDate", "Description", "*Quantity", "*UnitAmount", "*AccountCode", "*TaxType", "TaxAmount", "Reference"],
    payments: ["*ContactName", "*InvoiceNumber", "*Date", "*Amount", "Reference", "*AccountCode"],
    expenses: ["*ContactName", "*InvoiceNumber", "*InvoiceDate", "*DueDate", "Description", "*Quantity", "*UnitAmount", "*AccountCode", "Reference"],
  },
};
...
export function mapRow(target, kind, row) {
  ...
  const ref = externalRef(kind === "invoices" ? "invoice" : kind === "payments" ? "payment" : "expense", row?.id);
  ...
  if (kind === "invoices") {
    ...
    if (target === "quickbooks") {
      Object.assign(out, {
        InvoiceNo: number, Customer: contact, InvoiceDate: date, DueDate: due,
      ...
      Object.assign(out, {
        "*ContactName": contact, "*InvoiceNumber": number, "*InvoiceDate": date, "*DueDate": due,
  ...
  if (kind === "payments") {
    ...
    const amount = decimalFromMinor(finite(row?.base_amount_minor ?? row?.amount_minor) - finite(row?.refunded_minor));
    if (target === "quickbooks") {
      Object.assign(out, {
        RefNumber: ref, Customer: contact, PaymentDate: date,
        PaymentMethod: String(row?.method ?? ""), InvoiceNo: String(row?.invoice_number ?? ""),
        Amount: amount, PrivateNote: ref,
      });
    } else {
      Object.assign(out, {
        "*ContactName": contact, "*InvoiceNumber": String(row?.invoice_number ?? ""),
        "*Date": date, "*Amount": amount, Reference: ref,
  ...
  if (target === "quickbooks") {
    Object.assign(out, {
      RefNumber: ref, Payee: payee, ExpenseDate: date,
      Category: String(row?.category ?? ""), Amount: decimalFromMinor(row?.amount_minor), Memo: ref,
    });
  } else {
    Object.assign(out, {
      "*ContactName": payee || "Sundry", "*InvoiceNumber": ref,
      "*InvoiceDate": date, "*DueDate": date,
      Description: String(row?.category ?? ""), "*Quantity": "1.000",
      "*UnitAmount": decimalFromMinor(row?.amount_minor),
      "*AccountCode": String(row?.account_code ?? "400"), Reference: ref,
    });
  }
  return out;
}
...
export function reconcile(local, remote) {
  const byRef = (rows) => {
    ...
    for (const row of rows ?? []) {
      ...
      if (!ref) continue;
      map.set(ref, { ref, amountMinor: Math.trunc(finite(row?.amountMinor)), date: String(row?.date ?? "").slice(0, 10) });
    }
    return map;
  };
  const l = byRef(local);
  ...
  for (const [ref, row] of l) {
    ...
    if (!other) { missingRemote.push(row); continue; }
    if (other.amountMinor !== row.amountMinor) {
      amountMismatch.push({ ref, localMinor: row.amountMinor, remoteMinor: other.amountMinor, differenceMinor: row.amountMinor - other.amountMinor });
      continue;
    }
    matched.push(row);
  }
  ...
  return {
    matched, missingRemote, missingLocal, amountMismatch,
    ...
    balanced: missingRemote.length === 0 && missingLocal.length === 0 && amountMismatch.length === 0,
...
export const ACCOUNTING_SYNC_STATUS = Object.freeze({
  ...
  reason: "No developer credentials for either provider exist in this environment, so no API call has ever been made. Nothing here claims to have talked to an accounting system.",
});
