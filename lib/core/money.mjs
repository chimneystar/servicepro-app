 ...
 *
 * @param {{items:{qtyMilli:number,unitPriceMinor:number,taxable?:boolean}[], discountMinor?:number, taxRateBps?:number}} doc
...
export function computeDocument(doc) {
  ...
  const tax = divRoundHalfUp(taxBase * taxRateBps, 10000);
  const total = subtotal - discount + tax;

  if (!Number.isSafeInteger(total)) throw new Error("amount too large");
  return { subtotalMinor: subtotal, discountMinor: discount, taxableMinor: taxBase, taxMinor: tax, totalMinor: total };
}
