...
export function resolveTaxJurisdictions(jurisdictions, options = {}) {
  ...
  for (const raw of jurisdictions ?? []) {
    ...
    if (!rule.active) { skipped.push({ rule, reason: "inactive" }); continue; }
    if (rule.effectiveFrom && onDate < rule.effectiveFrom) { skipped.push({ rule, reason: "not_yet_effective" }); continue; }
    if (rule.effectiveTo && onDate > rule.effectiveTo) { skipped.push({ rule, reason: "expired" }); continue; }
    if (rule.appliesTo !== "all") { skipped.push({ rule, reason: "unsupported_scope" }); continue; }
    applied.push(rule);
  }
