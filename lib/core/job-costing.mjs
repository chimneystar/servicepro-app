...
export function minutesByTechnician(entries) {
  ...
  for (const entry of entries ?? []) {
    if (entry?.started_at && !entry?.ended_at) { openEntries += 1; continue; }
...
export function jobLabour({ entries, rates, onDate }) {
  ...
  for (const [profileId, mins] of byTech) {
    ...
    if (rate === null) { unpriced.push({ profileId, minutes: mins }); continue; }
...
export function labourInvoiceLine({ minutes, costMinor, locale = "en" }) {
  ...
  return {
    description: locale === "he" ? `עבודה (${hours} שעות) — כלול במחיר` : `Labour (${hours} h) — included in the price`,
    qty_milli: 1000,
    unit_price_minor: 0,
    cost_minor: cost,
    taxable: false,
