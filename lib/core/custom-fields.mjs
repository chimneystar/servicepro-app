...
export function normalizeDefinitionInput(input) {
  ...
  if (!ENTITY_TYPES.includes(entityType)) throw new CustomFieldError("A field must belong to customers or to jobs.");
  ...
  let options = [];
  if (fieldType === "choice") {
    const raw = Array.isArray(input.options) ? input.options : String(input.options ?? "").split("\n");
    const seen = new Set();
    for (const entry of raw) {
      const option = String(entry).trim();
      if (!option || seen.has(option)) continue;
      if (option.length > 80) throw new CustomFieldError("Each choice is limited to 80 characters.");
      seen.add(option);
      options.push(option);
    }
    if (options.length === 0) throw new CustomFieldError("A choice field needs at least one option.");
 ...
 *
 * @returns {{present: boolean, value: any}} `present: false` means "no value" —
...
export function coerceFieldValue(definition, raw) {
  ...
  if (type === "checkbox") {
    const truthy = ["1", "on", "true", "yes"].includes(String(raw ?? "").trim().toLowerCase()) || raw === true;
  ...
  if (type === "text") {
    if (text.length > 2000) throw new CustomFieldError(`${label} is limited to 2000 characters.`, id);
  ...
  if (type === "number") {
    ...
    if (!/^-?\d{1,15}(\.\d{1,6})?$/.test(text)) throw new CustomFieldError(`${label} must be a number.`, id);
  ...
  if (!options.map(String).includes(text)) throw new CustomFieldError(`${label} must be one of the configured options.`, id);
  return { present: true, value: text };
}
 ...
 *
 * @returns {{writes: {definitionId: string, value: any}[], deletes: string[], errors: string[]}}
 */
export function collectFieldValues(definitions, submitted) {
  ...
  for (const definition of definitions ?? []) {
    ...
    const raw = submitted instanceof Map ? submitted.get(definition.id) : submitted?.[definition.id];
    let coerced;
    try {
    ...
    const missing = !coerced.present || (definition.field_type === "checkbox" && coerced.value === false);
    if (required && missing) {
...
export function assertEntityReference({ definition, entityType, entity, organizationId }) {
  if (!ENTITY_TYPES.includes(entityType)) throw new CustomFieldError(`Unknown entity type: ${entityType}`);
  ...
  if ((definition.entity_type ?? definition.entityType) !== entityType)
    throw new CustomFieldError(`That field is defined for ${definition.entity_type ?? definition.entityType}s, not ${entityType}s.`, definition.id ?? null);
