 ...
 *
 * @type {readonly ["customer", "job", "estimate", "invoice"]}
 */
 ...
 *
 * The annotation is what lets `app/(app)/trash/actions.ts` call
 * `supabase.from(tableForKind(kind))`: the typed client needs a literal table
 * name, and an inferred `string` is exactly the hole that let a query name a
 * table which did not exist.
 *
 * @type {{ readonly customer: "customers", readonly job: "jobs",
 *   readonly estimate: "estimates", readonly invoice: "invoices" }}
 */
 ...
 *
 * @type {readonly ["owner", "office"]}
 */
export const RESTORE_ROLES = ["owner", "office"];

/**
 * @param {unknown} kind
 * @returns {kind is "customer" | "job" | "estimate" | "invoice"}
 */
...
/**
 * @param {unknown} kind
 * @returns {"customers" | "jobs" | "estimates" | "invoices" | null}
 */
