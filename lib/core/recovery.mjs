...
/** kind -> table name. Verified against db/schema.sql. */
...
function parentBlocker(code, label, parent) {
  if (parent == null) {
    return { code, label, reason: "missing", message: `The ${label} this record belongs to no longer exists.` };
  }
  if (parent.deleted) {
    return {
      code,
...
export function restoreFailureMessage(error) {
  ...
  if (code === "42501" || /permission denied|row-level security|insufficient_privilege/i.test(text)) {
