 ...
 *
 * The JSDoc annotation is load-bearing, not decoration. This array is passed
 * straight to `.in("normalized_status", COLLECTED_STATUSES)` from four call
 * sites; without it TypeScript infers `string[]`, which the generated column
 * type (a union of the eleven values `payments.normalized_status` allows)
 * rejects. Writing the values here rather than inferring them means a typo —
 * or a status renamed in a migration — fails the build instead of silently
 * matching no rows and reporting revenue of zero.
 *
 * @type {readonly ["settled", "partially_refunded"]}
 */
