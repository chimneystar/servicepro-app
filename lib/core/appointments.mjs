...
export function canRespond(job, { maxResponses = 10 } = {}) {
  ...
  if (job.status === "done" || job.status === "cancelled")
    return { ok: false, error: "appointment_closed" };
  if (finite(job.customer_response_count) >= maxResponses)
    return { ok: false, error: "too_many_responses" };
  return { ok: true };
}
...
export function describeArrival(appointment, { locale = "en", now = Date.now() } = {}) {
  ...
  switch (state) {
    case "completed":
      return he ? "העבודה הושלמה. תודה!" : "This visit is complete. Thank you!";
    case "cancelled":
      return he ? "הביקור בוטל." : "This visit has been cancelled.";
    case "arrived":
      return he ? `${who} הגיע.` : `${who} has arrived.`;
    case "due":
      return he
        ? `${who} בדרך והיה אמור להגיע. אם עוד לא הגיע, התקשרו אלינו.`
        : `${who} is on the way and is due now. If they have not arrived, please call us.`;
    case "on_the_way": {
      const mins = minutesUntilArrival(appointment, now);
      if (mins === null) return he ? `${who} בדרך אליכם.` : `${who} is on the way.`;
      return he
        ? `${who} בדרך אליכם — עוד כ-${Math.max(1, mins)} דקות.`
        : `${who} is on the way — about ${Math.max(1, mins)} minutes away.`;
    }
    default:
      return he
        ? "הביקור מתוכנן. נעדכן אתכם כשהטכנאי יצא לדרך."
        : "Your visit is booked. We will tell you the moment your technician sets off.";
  }
}
