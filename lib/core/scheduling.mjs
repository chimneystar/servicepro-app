...
/**
 * The four job statuses, in the order the `job_status` Postgres enum declares
 * them (db/001_schema.sql).
 *
 * The annotation is what lets `lib/job-status.ts` narrow a `string` from the
 * client to the enum before writing it to `jobs.status`. Without it the array
 * infers as `string[]` and the guard below teaches TypeScript nothing.
 *
 * @type {readonly ["scheduled", "in_progress", "done", "cancelled"]}
 */
...
export function validateInterval(startMin, endMin, { maxDurationMin = 24 * 60 } = {}) {
  ...
  if (endMin <= startMin) return { ok: false, error: "שעת הסיום חייבת להיות אחרי שעת ההתחלה" };
  if (endMin - startMin > maxDurationMin) return { ok: false, error: "משך העבודה ארוך מדי" };
  return { ok: true };
}
...
export function canBook(candidate, existing, opts) {
  ...
  if (conflicts.length > 0) return { ok: false, error: "הטכנאי כבר משובץ בזמן הזה", conflicts };
  return { ok: true };
}
