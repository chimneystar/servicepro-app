...
export function isDigestDue(schedule, todayISO) {
  ...
  if (String(schedule.last_period_key ?? "") === period.key) return { due: false, reason: "already_sent", period };
  if (schedule.starts_on && day(schedule.starts_on) > period.end) return { due: false, reason: "not_started", period };
 ...
 */
export function renderDigest({ totals, counts = {}, period, orgName = "", locale = "en", format, reportUrl = "" }) {
  ...
  const heading = he ? `סיכום ${orgName} · ${period.label}` : `${orgName} summary · ${period.label}`;
  const body = [
    heading,
    "",
