...
export function daysBetween(fromISO, toISO) {
  ...
  if (!Number.isFinite(a) || !Number.isFinite(b)) throw new TypeError(`invalid date range: ${fromISO}..${toISO}`);
...
export function isChargeable(invoice) {
  return CHARGEABLE_INVOICE_STATUSES.includes(String(invoice?.status ?? ""))
    && !invoice?.deleted_at;
}
...
export function buildStatement({ invoices, payments, asOf, since = null }) {
  ...
  const charges = (invoices ?? []).filter(isChargeable).filter((i) => dayISO(i.issue_date) <= asOfDay);
  ...
  for (const invoice of charges) {
    ...
    if (inWindow(day)) {
      lines.push({
        date: day, kind: "invoice", reference: String(invoice.number ?? ""),
        invoiceId: String(invoice.id), description: `Invoice #${invoice.number ?? ""}`.trim(),
        chargeMinor: amount, creditMinor: 0,
      });
    } else {
      openingMinor += amount;
    }
  ...
  lines.sort((a, b) => (a.date < b.date ? -1 : a.date > b.date ? 1 : a.kind === "invoice" ? -1 : 1));
  ...
  let oldestDays = 0;
  for (const invoice of charges) {
    const paidForInvoice = collectedMinor(credits.filter((p) => String(p.invoice_id ?? "") === String(invoice.id)));
    ...
    open.push({
      invoiceId: String(invoice.id), number: invoice.number ?? null,
      issueDate: dayISO(invoice.issue_date), ageDays: age, outstandingMinor: outstanding,
    });
  }
  open.sort((a, b) => b.ageDays - a.ageDays);

  return {
    asOf: asOfDay, since: sinceDay,
    openingMinor, chargesMinor, paymentsMinor, balanceMinor,
    lines, aging, openInvoices: open, oldestDays,
...
export function nextDunningStage(invoice, sentStages = []) {
  ...
  const highestSent = DUNNING_LADDER
    .filter((rung) => sent.has(rung.stage))
    .reduce((max, rung) => Math.max(max, rung.severity), 0);
...
export function dunningKey(invoiceId, stage) {
  ...
  if (!DUNNING_STAGES.includes(String(stage))) throw new TypeError(`unknown dunning stage: ${stage}`);
  return `${id}:${stage}`;
}
 ...
 */
export function dunningMessage({ stage, locale = "en", firstName = "", businessName = "", invoiceNumber = "", amountLabel = "", balanceLabel = "", link = "" }) {
  ...
  const where = link ? (he ? ` ניתן לצפות ולשלם כאן: ${link}` : ` You can view and pay it here: ${link}`) : "";
  ...
  return { channel: rung.channel, subject: `${subjects[stage]} · ${businessName}`.trim(), body: bodies[stage] };
}

/** The covering note that goes out with a statement. */
export function statementMessage({ locale = "en", firstName = "", businessName = "", balanceLabel = "", asOf = "", link = "" }) {
  const he = locale === "he";
  const name = String(firstName ?? "").trim();
  const hi = he ? (name ? `שלום ${name},` : "שלום,") : (name ? `Hi ${name},` : "Hello,");
  const where = link ? (he ? ` פירוט מלא: ${link}` : ` The full statement is here: ${link}`) : "";
  return {
    subject: he ? `דף חשבון ל-${asOf} · ${businessName}` : `Account statement as of ${asOf} · ${businessName}`,
    body: he
