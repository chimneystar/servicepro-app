...
export function validateAutomationRule(input) {
  ...
  if (!SUPPORTED_AUTOMATIONS[triggerType].includes(actionType))
  ...
  if (
    ...
    input?.overdueDays !== undefined &&
    input?.overdueDays !== ""
  ) {
    const parsed = Number(input.overdueDays);
    if (!Number.isInteger(parsed) || parsed < 1 || parsed > 365)
      return { ok: false, reason: "invalid_overdue_days" };
    overdueDays = parsed;
  }
...
export function automationRefusalMessage(reason, he = false) {
  const messages = {
    unknown_trigger: ["That trigger isn't one this app can watch for.", "הטריגר הזה לא נתמך."],
    unknown_action: ["That action isn't one this app can perform.", "הפעולה הזאת לא נתמכת."],
