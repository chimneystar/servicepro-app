...
/**
 * The literal tuple type is what lets a TypeScript caller write a checked status
 * into `purchase_orders.status`, whose CHECK constraint the generated database
 * types express as this same union. Annotation only — no runtime effect.
 *
 * @type {readonly ["draft", "ordered", "partially_received", "received", "cancelled"]}
 */
...
export function validateReceipt(line, qtyMilli, poStatus = "ordered") {
  ...
  if (poStatus === "received")
    return { ok: false, error: "This purchase order is already fully received." };
