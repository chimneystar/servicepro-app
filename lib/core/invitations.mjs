...
export function invitationEmail({ locale = "en", businessName, inviterName = "", role = "tech", acceptUrl }) {
  const he = locale === "he";
  ...
  const subject = he ? `הוזמנתם להצטרף ל${businessName || "עסק"} ב-ServicePro` : `You're invited to join ${businessName || "a business"} on ServicePro`;
...
export function describeInviteDelivery(invite, locale = "en") {
  ...
  if (status === "failed") {
    return { tone: "error", text: he ? `שליחת ההזמנה נכשלה${invite?.delivery_error ? ` — ${invite.delivery_error}` : ""}` : `Invitation email failed${invite?.delivery_error ? ` — ${invite.delivery_error}` : ""}` };
  }
