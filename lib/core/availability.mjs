...
export function rowWindow(row) {
  ...
  if (start === null || end === null || end <= start) return { start: 0, end: 24 * 60, allDay: true };
...
export function describeUnavailable(result, { locale = "en", name = "" } = {}) {
  ...
  if (result.reason === "closed") {
    return he ? "העסק סגור בתאריך הזה. שנו את התאריך או הסירו את יום הסגירה." : "The business is closed on that date. Change the date, or remove the closure.";
  }
  const who = name || (he ? "הטכנאי" : "That technician");
  if (result.reason === "away_partial" && result.window) {
    const from = String(Math.floor(result.window.start / 60)).padStart(2, "0") + ":" + String(result.window.start % 60).padStart(2, "0");
    const to = String(Math.floor(result.window.end / 60)).padStart(2, "0") + ":" + String(result.window.end % 60).padStart(2, "0");
    return he ? `${who} בחופשה בין ${from} ל-${to} בתאריך הזה.` : `${who} is off between ${from} and ${to} on that date.`;
  }
