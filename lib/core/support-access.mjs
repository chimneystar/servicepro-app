...
export function evaluateSupportSession(session, options = {}) {
  ...
  if (adminUserId && session.admin_user_id !== adminUserId)
    return { granted: false, reason: "wrong_admin" };
  if (organizationId && session.organization_id !== organizationId)
    return { granted: false, reason: "wrong_organization" };
...
export function selectGrantingSession(sessions, options = {}) {
  ...
  for (const row of rows) {
    ...
    if (!verdict.granted) {
      ...
      if (bestRefusal.reason === "no_session" && verdict.reason !== "no_session")
        bestRefusal = verdict;
      continue;
    }
...
export function supportAccessMessage(reason, locale = "en") {
  const en = {
    no_session:
      "No support session grants access to this business. Open a reason-bound session from a support case first.",
    revoked: "That support session has been revoked. Access stopped the moment it was revoked.",
    expired:
      "That support session has expired. Open a new one from the case if access is still needed.",
    ...
    insufficient_level:
      "That support session is read-only. A guided-write session is required for this action.",
