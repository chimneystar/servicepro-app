 ...
 *
 * The literal tuple type is what lets a TypeScript caller write a checked tier
 * into `estimate_options.tier`, whose CHECK constraint the generated database
 * types express as this same union. Annotation only — no runtime effect.
 *
 * @type {readonly ["good", "better", "best"]}
 */
...
/**
 * @param {unknown} value
 * @returns {value is "good" | "better" | "best"} Declared as a type guard so a
 * TypeScript caller that has already run this check can write the tier into the
 * CHECK-constrained column. Annotation only — the test itself is unchanged.
 */
