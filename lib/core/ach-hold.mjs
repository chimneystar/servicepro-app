...
export function isAchPayment(payment) {
  return String(payment?.method ?? "").trim().toUpperCase() === "ACH";
}
 ...
 */
export function depositReleaseDecision({ holdEnabled, requiredMinor, payments, overridden = false }) {
  const required = Math.max(0, finite(requiredMinor));
  const settled = settledMinor(payments);
  const pending = pendingAchMinor(payments);

  if (required <= 0) return { released: true, reason: "no_deposit_required", settledMinor: settled, pendingMinor: pending };
  if (settled >= required) return { released: true, reason: "settled", settledMinor: settled, pendingMinor: pending };

  if (settled + pending >= required) {
    if (!holdEnabled) return { released: true, reason: "hold_disabled", settledMinor: settled, pendingMinor: pending };
    if (overridden) return { released: true, reason: "overridden", settledMinor: settled, pendingMinor: pending };
    return { released: false, reason: "awaiting_settlement", settledMinor: settled, pendingMinor: pending };
  }

  return { released: false, reason: "deposit_unpaid", settledMinor: settled, pendingMinor: pending };
}
