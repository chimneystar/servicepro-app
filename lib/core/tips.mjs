...
export function tipMinorFromPercent(balanceMinor, percent) {
  ...
  if (!Number.isInteger(pct) || pct < 0 || pct > MAX_TIP_PERCENT) throw new Error("percent out of range");
...
export function resolveTip({ enabled, balanceMinor, percent, customMinor }) {
  ...
  const asked = percent !== undefined && percent !== null && percent !== ""
...
export function paymentTipParts(chargedMinor, tipMinor) {
  ...
  const tip = Number(tipMinor);
  if (!Number.isInteger(charged) || charged < 0) throw new Error("charged must be a non-negative integer");
  if (!Number.isInteger(tip) || tip < 0 || tip > charged) throw new Error("tip must be within the charged amount");
