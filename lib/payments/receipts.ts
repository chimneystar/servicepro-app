...
export async function sendPaymentReceipt(paymentId: string) {
  ...
  const { data: payment } = await admin
    .from("payments")
    .select(
      "id, organization_id, invoice_id, estimate_id, base_amount_minor, surcharge_minor, tip_minor, currency, method, normalized_status",
    )
    .eq("id", paymentId)
    .maybeSingle();
  if (!payment || !["settled", "partially_refunded"].includes(payment.normalized_status))
    return { email: false, sms: false };

  const [{ data: settings }, { data: organization }] = await Promise.all([
    admin
      .from("payment_settings")
      .select("receipt_email_enabled, receipt_sms_enabled")
  ...
  const { data: document } = payment.invoice_id
    ? await admin
        .from("invoices")
        .select("number, public_token, customer_id")
  ...
  const totalMinor =
    Number(payment.base_amount_minor) + Number(payment.surcharge_minor ?? 0) + tipMinor;
  ...
  const tipLine =
    tipMinor > 0
      ? he
        ? `<p>כולל טיפ בסך ${html(format(tipMinor))}.</p>`
        : `<p>Includes a ${html(format(tipMinor))} tip.</p>`
      : "";
  const tipSms =
    tipMinor > 0
      ? he
        ? ` כולל טיפ ${format(tipMinor)}.`
        : ` Includes a ${format(tipMinor)} tip.`
      : "";
  const label = he ? (documentType === "invoice" ? "חשבונית" : "הצעת מחיר") : documentType;
  const firstName =
    String(customer.name ?? "")
      .trim()
      .split(/\s+/)[0] || (he ? "שלום" : "there");
  const link =
    document.public_token && appUrl()
      ? `${appUrl().replace(/\/$/, "")}/p/${document.public_token}`
      : "";
  const subject = he
    ? `קבלה מ־${business} — ${label} #${document.number}`
    : `Receipt from ${business} — ${label} #${document.number}`;
  const emailBody = he
    ...
    if (notificationId) {
      try {
        ...
        await Promise.all([
          ...
          admin.from("email_messages").insert({
            organization_id: payment.organization_id,
            related_type: "payment_receipt",
