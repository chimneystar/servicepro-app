...
import {
  deliveryPlan, notificationKey, notificationOutcome, paymentNotificationRecipients,
  staffEmailEligibility, staffNotification,
...
export async function resolveStaffEmail(
  ...
  try {
    ...
    if (error) { console.error(`[notify] could not resolve the login email for ${profile.id}: ${error.message}`); return null; }
    return data?.user?.email ?? null;
  } catch (cause: unknown) {
    console.error(`[notify] could not resolve the login email for ${profile.id}: ${staffError(cause)}`);
    return null;
  }
}
...
export async function staffContact(
  admin: ReturnType<typeof createAdminClient>,
  profile: { id: string; active?: boolean; notify_email?: string | null; notify_email_opt_in?: unknown },
  ...
  const preflight = staffEmailEligibility({ ...profile, email: "placeholder@example.com" }) as { ok: boolean; reason?: string };
  if (!preflight.ok) return preflight;
  const email = await resolveStaffEmail(admin, profile);
  return staffEmailEligibility({ ...profile, email }) as { ok: boolean; to?: string; reason?: string };
}
...
export async function notifyStaff(input: StaffNotifyInput): Promise<StaffNotifyResult> {
  ...
  try {
    dedupeKey = notificationKey({
      type: input.type, relatedType: input.relatedType,
      relatedId: input.relatedId, profileId: input.profileId,
    }) as string;
  } catch (cause: unknown) {
    console.error(`[notify] refusing to send a notification with an incomplete key: ${staffError(cause)}`);
    return { ok: false, status: "failed", reached: [], reason: "bad_key" };
  }

  const { data: profile } = await admin.from("profiles")
    .select("id, organization_id, full_name, active, notify_email, notify_email_opt_in, notify_push_opt_in")
    .eq("id", input.profileId).eq("organization_id", input.organizationId).maybeSingle();
  if (!profile) return { ok: false, status: "failed", reached: [], reason: "profile_not_found" };

  const content = staffNotification({ type: input.type, locale: "en", ...input.vars }) as { title: string; body: string; url: string };
  ...
  const { data: claimed, error: claimError } = await admin.from("staff_notifications").insert({
    organization_id: input.organizationId, profile_id: input.profileId, dedupe_key: dedupeKey,
    type: input.type, title: content.title, body: content.body, url: content.url,
    related_type: input.relatedType, related_id: input.relatedId, delivery_status: "pending",
  ...
  try {
    ...
    const push = input.pushResult
      ? { delivered: input.pushResult.delivered, reason: input.pushResult.reason ?? "", message: input.pushResult.message ?? "" }
      : pushWanted
      ? await sendPushToProfile({
          organizationId: input.organizationId, profileId: input.profileId,
          eventType: input.type, relatedType: input.relatedType, relatedId: input.relatedId,
          notification: (deviceLocale) => staffNotification({ type: input.type, locale: deviceLocale, ...input.vars }) as { title: string; body: string; url: string },
        })
      : { ok: false, delivered: 0, removed: 0, failed: 0, reason: pushStatus.reason || "push_opt_out", message: pushStatus.message };
    ...
    const plan = deliveryPlan({
      ...
      emailAvailable: providers.email(), emailEligible: eligibility.ok, urgent: input.urgent === true,
    ...
    if (plan.emailFallback && eligibility.ok && eligibility.to) {
      ...
      const html = `<p>${escapeHtml(content.title)}</p><p>${escapeHtml(content.body)}</p>`
        + (link ? `<p><a href="${escapeHtml(link)}">Open in ServicePro</a></p>` : "");
      try {
        const id = await sendEmail(eligibility.to, content.title, html);
        emailSent = true;
        await admin.from("email_messages").insert({
          organization_id: input.organizationId, related_type: `staff_${input.type}`, related_id: input.relatedId,
          to_email: eligibility.to, subject: content.title, provider: "resend",
        ...
        await admin.from("email_messages").insert({
          organization_id: input.organizationId, related_type: `staff_${input.type}`, related_id: input.relatedId,
          to_email: eligibility.to, subject: content.title, provider: "resend", status: "failed", error: emailError,
        });
    ...
    return { ok: outcome.ok, status: outcome.status as StaffNotifyResult["status"], reached: outcome.reached, reason: String(reason) };
  } catch (cause: unknown) {
    ...
    console.error(`[notify] released the ${input.type} claim for ${input.profileId}: ${staffError(cause)}`);
...
export async function notifyJobAssignedStaff(input: {
  organizationId: string; jobId: string; profileId: string; unassigned?: boolean;
  pushResult?: { available: boolean; delivered: number; reason?: string; message?: string };
  ...
  try {
    ...
    const { data: job } = await admin.from("jobs")
      ...
      .eq("id", input.jobId).eq("organization_id", input.organizationId).maybeSingle();
    ...
    return await notifyStaff({
      ...
      relatedType: "job", relatedId: input.jobId,
      pushResult: input.pushResult,
      vars: {
        service: job?.service ?? "", customerName: customer?.name ?? "",
        scheduledDate: job?.scheduled_date ?? "", startTime: job?.start_time ?? "", jobId: input.jobId,
      },
    });
  } catch (cause: unknown) {
...
export async function notifyPaymentReceived(input: {
  organizationId: string; paymentId: string; amountLabel: string;
  customerName?: string; invoiceNumber?: string | number | null; invoiceId?: string | null;
}): Promise<{ notified: number; results: StaffNotifyResult[] }> {
  try {
    const admin = createAdminClient();
    const { data: team } = await admin.from("profiles")
      .select("id, role, active").eq("organization_id", input.organizationId).limit(200);
    const ids = (team ?? []).map((row: { id: string }) => row.id);
    const { data: capabilities } = ids.length
      ? await admin.from("profile_capabilities").select("profile_id, can_manage_payments").in("profile_id", ids)
      : { data: [] as { profile_id: string; can_manage_payments: boolean }[] };
    const canPay = new Map(((capabilities ?? []) as { profile_id: string; can_manage_payments: boolean }[])
      .map((row) => [String(row.profile_id), row.can_manage_payments === true]));

    const recipients = paymentNotificationRecipients(
      (team ?? []).map((row: { id: string; role: string; active: boolean }) => ({
        ...row, can_manage_payments: canPay.get(String(row.id)) === true,
      })),
    ) as string[];

    const results: StaffNotifyResult[] = [];
    for (const profileId of recipients) {
      results.push(await notifyStaff({
        organizationId: input.organizationId, profileId, type: "payment_received",
        relatedType: "payment", relatedId: input.paymentId,
        vars: {
          amountLabel: input.amountLabel, customerName: input.customerName ?? "",
