...
export async function loadStatement(
  ...
  const asOf =
    options.asOf && /^\d{4}-\d{2}-\d{2}$/.test(options.asOf)
      ? options.asOf
  ...
  const { data: org } = await supabase
    .from("organizations")
    .select("name, currency, phone, email, address, city, logo_url, accent_color")
    .single();
...
export async function sendStatement(input: {
  ...
  const record = async (
    ...
    const { error } = await supabase.from("customer_statements").insert({
      ...
      charges_minor: statement.chargesMinor,
      payments_minor: statement.paymentsMinor,
      balance_minor: statement.balanceMinor,
    ...
    if (error)
      console.error(
        `[statements] could not record the statement for ${input.customerId}: ${error.message}`,
      );
  };
  ...
  const detail = statement.openInvoices
    ...
    .map(
      (row) =>
        `#${row.number ?? ""} — ${formatMoney(row.outstandingMinor, { currency })} (${row.ageDays}d)`,
    )
    .join("\n");
 ...
 */
export async function loadStatementForCron(
