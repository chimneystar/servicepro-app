...
const fieldLabels: Record<string, [string, string]> = {
  ...
  status: ["Status", "סטטוס"],
  stage: ["Job stage", "שלב העבודה"],
  scheduled_date: ["Date", "תאריך"],
  start_time: ["Start time", "שעת התחלה"],
  end_time: ["End time", "שעת סיום"],
  assigned_to: ["Technician", "טכנאי"],
  price_minor: ["Price", "מחיר"],
  notes: ["Job notes", "הערות עבודה"],
  job_address: ["Service address", "כתובת העבודה"],
  job_city: ["City", "עיר"],
  tags: ["Tags", "תגיות"],
  on_my_way_at: ["On my way", "בדרך ללקוח"],
  started_at: ["Work started", "העבודה התחילה"],
  completed_at: ["Work completed", "העבודה הושלמה"],
  parent_job_id: ["Original job", "עבודה מקורית"],
};
...
function changedFields(
  ...
  return Object.keys(newData)
    .filter(
      (key) => !ignored.has(key) && JSON.stringify(oldData?.[key]) !== JSON.stringify(newData[key]),
    )
    .map((key) => fieldLabels[key]?.[locale === "he" ? 1 : 0] ?? key.replaceAll("_", " "))
    .slice(0, 5);
}
...
export async function loadJobHistory(
  ...
  const actorIds = [
    ...new Set(
      [
        ...audit.map((row) => row.actor),
        ...actions.flatMap((row) => [row.created_by, row.completed_by, row.assigned_to]),
        ...calls.map((row) => row.handled_by),
        ...(warranty ? [warranty.created_by] : []),
  ...
  const entries: JobTimelineEntry[] = audit.map((row) => {
    ...
    return {
      id: `audit-${row.id}`,
      kind: "change",
      tone: row.action === "DELETE" ? "coral" : "blue",
      title,
      detail: fields.length
        ? he
          ? `מה השתנה: ${fields.join(", ")}`
          : `Changed: ${fields.join(", ")}`
        : null,
      actor: who(row.actor),
      at: row.at,
  ...
  actions.forEach((row) => {
    ...
    entries.push({
      id: `action-${row.id}`,
      kind: isFollowUp ? "follow_up" : "note",
      tone: isFollowUp && row.status === "open" ? "yellow" : "navy",
      title: row.title,
      detail:
        row.body ||
        (row.due_at
          ? `${he ? "לביצוע עד" : "Due"} ${new Intl.DateTimeFormat(he ? "he-IL" : "en-US", { dateStyle: "medium", timeStyle: "short" }).format(new Date(row.due_at))}`
          : null),
      actor: who(row.created_by),
      at: row.created_at,
      status: row.status,
      actionId: row.id,
      canComplete:
        row.status === "open" && (!row.assigned_to || row.assigned_to === currentProfileId),
    });
    if (row.completed_at)
      entries.push({
        ...
        kind: "follow_up",
        tone: "navy",
        title: he ? `הושלם: ${row.title}` : `Completed: ${row.title}`,
        actor: who(row.completed_by),
        at: row.completed_at,
        status: "done",
      });
  });
  calls.forEach((row) => {
    ...
    entries.push({
      id: `call-${row.id}`,
      kind: "call",
      tone: missed ? "coral" : "blue",
      title: `${direction} · ${formatUsPhone(otherNumber)}`,
      detail: [
        row.reason,
        row.outcome,
        duration ? `${he ? "משך" : "Duration"} ${duration}` : null,
        row.notes,
      ]
        .filter(Boolean)
        .join(" · "),
      actor: who(row.handled_by),
      at: row.started_at,
      status: row.needs_follow_up ? "follow_up" : row.status,
    });
  });
  if (warranty)
    entries.push({
      ...
      kind: "warranty",
      tone: "yellow",
      title: he ? "נוספה אחריות לעבודה" : "Warranty added",
      detail: `${warranty.starts_on}${warranty.expires_on ? ` → ${warranty.expires_on}` : ` · ${he ? "ללא תאריך סיום" : "No end date"}`}`,
      actor: who(warranty.created_by),
  ...
  callbacks.forEach((row) => {
    entries.push({
      id: `callback-${row.id}`,
      kind: "callback",
      tone: row.priority === "urgent" ? "coral" : "yellow",
      title: he ? "דווחה חזרה במסגרת אחריות" : "Warranty callback reported",
      detail: row.issue,
      actor: who(row.created_by),
      at: row.reported_at,
      status: row.status,
    });
    if (row.scheduled_for && row.callback_job_id)
      entries.push({
        id: `callback-scheduled-${row.id}`,
        kind: "callback",
        tone: "blue",
        title: he ? "נקבע ביקור חוזר" : "Return visit scheduled",
        detail: row.scheduled_for,
        at: `${row.scheduled_for}T12:00:00.000Z`,
        status: "scheduled",
      });
    if (row.resolved_at)
      entries.push({
        id: `callback-resolved-${row.id}`,
        kind: "callback",
        tone: "navy",
        title:
          row.status === "denied"
