...
test("the comment stripping these guards rely on actually works", () => {
  ...
  assert.ok(
    !/worst available outcome|Storing a reusable card/i.test(stripped),
...
test("saving the settings form does not silently switch the stored preference off", () => {
  ...
  assert.ok(
    !/save_methods_enabled: enabled\(formData/.test(actions),
    "the form must not be the source for a control the form does not submit",
  );
  assert.ok(
    /save_methods_enabled: current\?\.save_methods_enabled/.test(actions),
...
test("nothing in the product pretends to store a card token", () => {
  ...
  for (const file of [
    ...
    "lib/payments/deposits.ts",
    "lib/payments/booking-deposit.ts",
    "components/CustomerPaymentOptions.tsx",
  ]) {
    const src = read(file);
    assert.ok(
      !/card_token|cardToken|saved_payment_method|customerCode/.test(src),
