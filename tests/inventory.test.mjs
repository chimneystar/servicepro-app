...
test("sign is decided by the kind, not by the caller remembering a minus", () => {
  ...
  assert.equal(signedQtyMilli("adjustment", -5_000), -5_000, "only an adjustment carries its own sign");
...
test("a consumption within stock is permitted (the guard is not a cry-wolf)", () => {
  const r = validateMovement({ kind: "consumption", qtyMilli: 2_000, availableMilli: 5_000, reason: "Fitted on job" });
...
test("consuming the exact remaining stock is permitted", () => {
  const r = validateMovement({ kind: "consumption", qtyMilli: 5_000, availableMilli: 5_000, reason: "Fitted on job" });
  assert.equal(r.ok, true, r.error);
...
test("consuming more than exists is REFUSED by default — the second of two technicians loses", () => {
  ...
  const r = validateMovement({ kind: "consumption", qtyMilli: 1_000, availableMilli: 0, reason: "Fitted on job" });
...
test("but an acknowledged over-consumption IS recorded — refusing outright loses the truth", () => {
  ...
  const r = validateMovement({
    kind: "consumption", qtyMilli: 1_000, availableMilli: 0,
    allowNegative: true, reason: "Fitted on site, stock count was wrong",
  });
  assert.equal(r.ok, true, r.error);
  assert.equal(r.afterMilli, -1_000);
  assert.equal(isOversold({ quantity_milli: r.afterMilli }), true, "and the item is flagged for a count");
  assert.equal(isOversold({ quantity_milli: 0 }), false);
});

test("the override needs a real reason, not a keystroke", () => {
  const bare = validateMovement({ kind: "consumption", qtyMilli: 1_000, availableMilli: 0, allowNegative: true, reason: "x" });
  assert.equal(bare.ok, false);
  assert.equal(bare.code, "no_reason");
  // The same movement with an explanation is accepted.
  assert.equal(validateMovement({ kind: "consumption", qtyMilli: 1_000, availableMilli: 0, allowNegative: true, reason: "Van stock" }).ok, true);
});

test("every movement must say why", () => {
  assert.equal(validateMovement({ kind: "receipt", qtyMilli: 1_000, reason: "  " }).ok, false);
  assert.equal(validateMovement({ kind: "receipt", qtyMilli: 1_000, reason: "Delivery 8812" }).ok, true);
});
...
test("a receipt is never blocked by a negative balance — putting stock back always works", () => {
  const r = validateMovement({ kind: "receipt", qtyMilli: 4_000, availableMilli: -2_000, reason: "PO-1234" });
  assert.equal(r.ok, true, r.error);
...
test("the PO lifecycle advances, and cannot go backwards", () => {
  assert.deepEqual(PO_STATUSES, ["draft", "ordered", "partially_received", "received", "cancelled"]);
  ...
  assert.equal(canTransitionPurchaseOrder("draft", "received"), false, "stock cannot arrive before it is ordered");
  assert.equal(canTransitionPurchaseOrder("ordered", "ordered"), true, "a no-op is not a transition");
...
test("the comment stripping used by these guards actually works", () => {
  ...
  assert.ok(/create table if not exists public\.inventory_movements/.test(sql), "SQL must survive stripping");
...
test("stock is an append-only ledger with a derived cache, not a mutable counter", () => {
  ...
  assert.ok(/grant select, insert on public\.inventory_movements to authenticated/.test(sql),
...
test("the last unit cannot be consumed twice: the guard locks the item row", () => {
  ...
  assert.ok(/for update/.test(guard.slice(0, 1400)),
  ...
  assert.ok(/allow_negative/.test(guard),
...
test("a job line consumes stock — the thing that did not exist at all", () => {
  ...
  assert.ok(body.indexOf('from("job_items").insert') < body.indexOf("recordInventoryMovement"),
    "the line must be written before the stock is taken");
  assert.ok(/delete\(\)\.eq\("id", line\.id\)/.test(body),
...
test("purchase orders are multi-line, and receiving writes inventory movements", () => {
  ...
  assert.ok(/form\.getAll\("description"\)/.test(actions),
  ...
  assert.ok(/receive_purchase_order_line/.test(actions), "receiving must go through the atomic DB function");
  assert.ok(!/quantity: qtyMilli \/ 1000, unit_cost_minor: unitCostMinor \}\)/.test(actions),
  ...
  assert.ok(/insert into public\.inventory_movements/.test(fn.slice(0, 3000)),
    "receiving stock that never touches inventory_items is the whole 5.19 defect");
  assert.ok(/guard_purchase_order_status/.test(sql), "the lifecycle is enforced at the database too");
});

test("purchase_order_items.quantity is no longer a raw float", () => {
  const sql = stripSql("db/033_inventory_movements.sql").toLowerCase();
  assert.ok(/add column if not exists qty_milli\s+bigint/.test(sql),
  ...
  assert.ok(/sync_purchase_order_item_qty/.test(sql),
    "the legacy numeric column must be kept in step, not dropped");
  assert.ok(/sync_purchase_order_total/.test(sql),
...
test("the migration is idempotent — it can be run twice", () => {
  ...
  for (const m of sql.match(/add constraint \w+/g) ?? []) {
    ...
    assert.ok(/do \$\$/.test(before), `${m} must be inside a DO block that swallows duplicate_object`);
  }
});
...
test("technicians can record what they fitted, and nothing more", () => {
  ...
  assert.ok(/kind = 'consumption'/.test(head), "a technician must be able to record a part they fitted");
  assert.ok(/created_by = auth\.uid\(\)/.test(head), "and only as themselves");
  assert.ok(/current_user_role\(\) in \('owner','office'\)/.test(head),
    "receiving and free-hand adjustments stay with owner/office");
  assert.ok(/job_id is not null/.test(head),
