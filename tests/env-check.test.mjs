...
test("a missing required variable is FATAL", () => {
  for (const missing of [
    ...
    assert.ok(
      report.fatal.some((line) => line.includes(missing)),
      `the message must name ${missing}`,
    );
  }
});
...
test("an absent optional capability is reported as disabled, not fatal", () => {
  ...
  assert.ok(
    report.disabled.some((line) => /SMS/.test(line)),
    "SMS should be listed as disabled",
  );
...
test("a wrong-length encryption key is caught at boot", () => {
  ...
  assert.ok(
    report.fatal.some((line) => /first use/.test(line)),
    "the message must say WHEN it would have broken",
  );
});
...
test("a correct 32-byte key in hex or base64 is accepted", () => {
  ...
  for (const key of [hex, b64]) {
    ...
    assert.equal(
      report.ok,
      true,
      `${key.slice(0, 8)}… should be accepted: ${report.fatal.join("; ")}`,
    );
