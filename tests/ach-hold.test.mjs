...
const ach = (minor, status) => ({
  base_amount_minor: minor,
  refunded_minor: 0,
  normalized_status: status,
  method: "ACH",
});
const card = (minor, status) => ({
  base_amount_minor: minor,
  refunded_minor: 0,
  normalized_status: status,
  method: "Credit card",
...
test("an in-flight ACH transfer is recognised", () => {
  ...
  assert.equal(
    isAchPayment({ method: "ach" }),
    true,
    "matched case-insensitively for hand-recorded rows",
  );
...
test("the hold holds when it is on, and does not when it is off", () => {
  ...
  assert.deepEqual(achHoldState({ holdEnabled: true, payments }), {
    held: true,
    pendingMinor: 250_00,
    count: 1,
  });
...
test("nothing is held when nothing is in flight", () => {
  ...
  assert.equal(
    achHoldState({ holdEnabled: true, payments: [card(250_00, "processing")] }).held,
    false,
...
test("a settled deposit releases the work", () => {
  const result = depositReleaseDecision({
    holdEnabled: true,
    requiredMinor: 250_00,
    payments: [ach(250_00, "settled")],
  });
...
test("an in-flight ACH deposit does NOT release the work while the hold is on", () => {
  const result = depositReleaseDecision({
    holdEnabled: true,
    requiredMinor: 250_00,
    payments: [ach(250_00, "processing")],
  });
...
test("a part-paid deposit does not release", () => {
  const result = depositReleaseDecision({
    holdEnabled: false,
    requiredMinor: 250_00,
    payments: [ach(100_00, "processing")],
  });
  assert.equal(result.released, false);
  assert.equal(result.reason, "deposit_unpaid");
});
...
test("the setting is actually read, and the permission actually gates something", () => {
  ...
  assert.ok(
    /export async function releaseAchHold/.test(actions),
    "there must be a way to release a hold",
  );
  assert.ok(/mayOverrideAchHold\(/.test(actions), "and it must be permission-checked server-side");
  assert.ok(
    /action: "ach_hold_released"/.test(actions),
    "an early release is a financial decision with an author",
  );
  assert.ok(
    /milestone\.status !== "processing"/.test(actions),
...
test("the hold is released by the job that learns a transfer cleared", () => {
  ...
  assert.ok(
    reconcileAt > -1 && releaseAt > reconcileAt,
    "reconciliation must release deposit-gated work",
  );
  assert.ok(
    /estimate_id/.test(server.slice(reconcileAt, reconcileAt + 900)),
