...
test("a percentage default deposit is applied, exactly", () => {
  ...
  assert.equal(defaultDepositMinor({ default_deposit_type: "percent", default_deposit_bps: 2500 }, 100000), 25000);
  assert.equal(defaultDepositMinor({ default_deposit_type: "percent", default_deposit_bps: 3333 }, 100000), 33330);
});

test("a fixed default deposit is applied, exactly", () => {
  assert.equal(defaultDepositMinor({ default_deposit_type: "fixed", default_deposit_minor: 15000 }, 100000), 15000);
});

test("'none' means none", () => {
  assert.equal(defaultDepositMinor({ default_deposit_type: "none", default_deposit_bps: 5000 }, 100000), 0);
...
test("percentage rounding is half-up integer arithmetic", () => {
  ...
  assert.equal(defaultDepositMinor({ default_deposit_type: "percent", default_deposit_bps: 3333 }, 1001), 334);
  // Exactly half rounds up rather than to-even, deterministically.
  assert.equal(defaultDepositMinor({ default_deposit_type: "percent", default_deposit_bps: 5000 }, 3), 2);
});
...
test("a deposit can never exceed the document it belongs to", () => {
  ...
  assert.equal(defaultDepositMinor({ default_deposit_type: "fixed", default_deposit_minor: 50000 }, 20000), 20000);
  assert.equal(defaultDepositMinor({ default_deposit_type: "percent", default_deposit_bps: 10000 }, 20000), 20000);
  assert.equal(defaultDepositMinor({ default_deposit_type: "percent", default_deposit_bps: 99999 }, 20000), 20000);
});

test("a zero or negative document total asks for nothing", () => {
  assert.equal(defaultDepositMinor({ default_deposit_type: "percent", default_deposit_bps: 2500 }, 0), 0);
  assert.equal(defaultDepositMinor({ default_deposit_type: "percent", default_deposit_bps: 2500 }, -100), 0);
});
...
test("a malformed settings row yields no deposit, never NaN", () => {
  ...
  for (const bad of [null, undefined, {}, { default_deposit_type: "sometimes" }, { default_deposit_type: "percent", default_deposit_bps: "lots" }]) {
...
test("each booking payment mode produces the deposit it promises", () => {
  ...
  assert.equal(bookingDepositMinor({ mode: "percentage", value: 50, servicePriceMinor: price }), 20000);
...
test("a booking deposit never exceeds the price the customer was shown", () => {
  ...
  assert.equal(bookingDepositMinor({ mode: "percentage", value: 150, servicePriceMinor: 40000 }), 40000);
});
...
test("a service with no price collects no deposit at all", () => {
  ...
  for (const mode of BOOKING_PAYMENT_MODES) {
    assert.equal(bookingDepositMinor({ mode, value: 50, servicePriceMinor: 0 }), 0, `${mode} must not charge`);
  }
});

test("a malformed booking setting collects nothing rather than a NaN charge", () => {
  for (const bad of [{ mode: "percentage", value: "half", servicePriceMinor: 40000 }, { mode: "wat", value: 50, servicePriceMinor: 40000 }, {}]) {
...
test("the organisation default deposit is applied where it cannot be forgotten", () => {
  ...
  assert.ok(/coalesce\(new\.deposit_minor, 0\) <> 0/.test(sql),
...
test("the booking deposit is charged through the payment screen that already works", () => {
  ...
  assert.ok(/bookingDepositMinor\(/.test(src), "the deposit must be computed from booking_settings");
  assert.ok(/from\("estimates"\)\.insert/.test(src), "and raised as a real document");
  assert.ok(/deposit_minor:/.test(src));
  assert.ok(/allocate_document_number/.test(src),
  ...
  assert.ok(/deposit:\{amountMinor/.test(route.replace(/\s/g, "")) || /deposit=\{amountMinor/.test(route.replace(/\s/g, "")),
  ...
  assert.ok(!/a secure payment link will be sent after confirmation/.test(form),
...
test("a booking deposit does not create a job until the money is in", () => {
  const route = read("app/api/booking/[org]/submit/route.ts");
  const src = read("lib/payments/booking-deposit.ts");
  assert.ok(/auto_release_on_deposit/.test(route) && /auto_release_on_deposit/.test(src),
    "a business that requires approval must still get to approve");
  assert.ok(/estimateDepositRelease\(/.test(src), "the release must consult the money, not the calendar");
  assert.ok(/lead\.status === "won"/.test(src),
