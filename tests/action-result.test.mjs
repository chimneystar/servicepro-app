...
test("a refused write always produces a message", () => {
  assert.equal(actionFailureMessage({ ok: false, error: "Row level security" }), "Row level security");
  assert.equal(actionFailureMessage({ ok: false }), fallbackMessage(false), "a failure with no reason still has to say something");
  assert.equal(actionFailureMessage({ ok: false, error: "" }), fallbackMessage(false));
  assert.equal(actionFailureMessage({ ok: false, error: "   " }), fallbackMessage(false), "whitespace is not an explanation");
...
test("actions that predate the contract are treated as success, not invented failures", () => {
  for (const result of [undefined, null, void 0]) {
    assert.equal(actionFailureMessage(result), null, `${String(result)} must not be reported as a failure`);
  }
...
test("the discarded-await pattern detector recognises the real bug, and only it", () => {
  ...
  assert.ok(DISCARDED_AWAIT.test("start(async () => { await setJobStage(jobId, s); router.refresh(); });"),
    "this is the exact shape that was shipped 26 times");
  assert.ok(DISCARDED_AWAIT.test("await deleteLead(id);"));
  assert.equal(DISCARDED_AWAIT.test("const r = await deleteLead(id);"), false, "an assigned result is not discarded");
  assert.equal(DISCARDED_AWAIT.test("return await deleteLead(id);"), false, "a returned result is not discarded");
...
test("every one of those components can actually show a failure", () => {
  const missing = CALL_SITES.filter((file) => {
    ...
    return !/useActionStatus|ActionError/.test(src) && !/set(Err|Error|Msg|Message|Toast|Notice)/.test(src);
  });
...
test("the recurring plans banner is no longer hard-coded green", () => {
  ...
  assert.ok(!/background: "#e6f6ec", color: "#15803d", padding: "9px 12px"/.test(src),
...
const VOID_ACTIONS = {
  "app/(app)/operations/actions.ts": ["createCrew", "createServiceArea", "createAutomation", "createVendor", "createPurchaseOrder", "createSubcontractor"],
  "app/(app)/growth/actions.ts": ["createCampaign", "createReferralProgram", "recordAdSpend", "scheduleEstimateFollowup"],
};
...
test("every /operations and /growth action returns the house {ok,error} contract", () => {
  for (const [file, names] of Object.entries(VOID_ACTIONS)) {
    ...
    for (const name of names) {
      const signature = new RegExp(`export async function ${name}\\([^)]*\\):\\s*Promise<ActionResult>`);
      assert.ok(signature.test(src), `${file}#${name} must declare Promise<ActionResult>, not void`);
    }
    assert.ok(/export type ActionResult = \{ ok: boolean; error\?: string \}/.test(src),
...
test("no /operations or /growth action ignores the Supabase error", () => {
  for (const file of Object.keys(VOID_ACTIONS)) {
    ...
    assert.ok(!/(?:^|[;{]|=>\s*)\s*await\s+ctx\.supabase\.from\(/m.test(src),
...
test("/operations and /growth actually render what the action said", () => {
  for (const page of ["app/(app)/operations/page.tsx", "app/(app)/growth/page.tsx"]) {
    const src = read(page);
    assert.ok(!/<form action=\{create/.test(src) && !/<form action=\{record/.test(src) && !/<form action=\{schedule/.test(src),
  ...
  assert.ok(/state\.error/.test(form) && /role="alert"/.test(form), "the error must reach the screen");
...
test("/operations and /growth keep every form they had", () => {
  ...
  for (const heading of ["Crews", "Service areas", "Automations", "Vendors", "Purchase orders", "Subcontractors"]) {
