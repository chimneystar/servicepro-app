...
test("a row with no times is a WHOLE day, expressed as 0..1440", () => {
  ...
  assert.deepEqual(rowWindow({ start_time: "09:00", end_time: "11:00" }), {
    start: 540,
    end: 660,
    allDay: false,
  });
...
test("a closure removes ALL capacity, however large the team", () => {
  const closed = bookingCapacity({
    teamSize: 12,
    rows: [{ profile_id: null, starts_on: "2026-07-01", ends_on: "2026-07-01" }],
    day: "2026-07-01",
  });
  ...
  assert.deepEqual(
    buildBookingSlots({
      ...
      busy: [],
      closedWindows: closed.closedWindows,
      awayWindows: closed.awayWindows,
    }),
    [],
  );
});
...
test("a PARTIAL-day absence closes only its own hours", () => {
  ...
  assert.ok(
    starts.includes("11:00") && starts.includes("14:00"),
    "the afternoon is still available",
  );
});
...
test("time off and existing jobs consume the SAME capacity pool", () => {
  ...
  assert.ok(
    ...
    "2 people, 1 away and 1 busy => nothing free",
  );
  assert.ok(slots.some((slot) => slot.start === "10:00"));
});

test("passing NO time off reproduces the old behaviour byte for byte", () => {
  const before = buildBookingSlots({
    ...BASE,
    capacity: 2,
    busy: [{ start: "13:00", end: "14:00" }],
  });
  const after = buildBookingSlots({
    ...BASE,
    capacity: 2,
    busy: [{ start: "13:00", end: "14:00" }],
    awayWindows: [],
...
test("the refusal names the hours, so a dispatcher can act on it", () => {
  ...
  const message = describeUnavailable(isProfileOff(rows, "a", "2026-07-01", "10:00", "11:00"), {
    locale: "en",
    name: "Dana",
  });
...
test("a REQUESTED absence blocks nothing until it is approved", () => {
  const rows = [
    { profile_id: "a", starts_on: "2026-07-01", ends_on: "2026-07-01", status: "requested" },
  ];
...
// TypeScript sources are read through `codeShape`, not raw: these probes assert
// that a particular CALL with particular ARGUMENTS is present, never where the
// line breaks fall. `codeShape` is the file's token stream in one canonical
// spelling, so `.eq("status", "approved")` matches however Prettier wraps it —
// and a changed column, value or call still fails. Proven both ways in
// tests/source-shape.test.mjs.
const readTs = (relative) =>
  codeShape(readFileSync(new URL(`../${relative}`, import.meta.url), "utf8"), relative);
const slotsRoute = readTs("app/api/booking/[org]/slots/route.ts");
