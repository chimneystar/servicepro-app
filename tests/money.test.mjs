...
test("document total: subtotal + 8.25% US sales tax", () => {
  const r = computeDocument({
    items: [{ qtyMilli: 2000, unitPriceMinor: 14000 }],
    taxRateBps: 825,
  });
...
test("discount never makes taxable negative", () => {
  const r = computeDocument({
    items: [{ qtyMilli: 1000, unitPriceMinor: 28000 }],
    discountMinor: 30000,
    taxRateBps: 825,
  });
...
test("large amounts stay exact", () => {
  const r = computeDocument({
    items: [{ qtyMilli: 1000, unitPriceMinor: 999999999 }],
    taxRateBps: 825,
  });
...
test("negative inputs are rejected", () => {
  ...
  assert.throws(() =>
    computeDocument({ items: [{ qtyMilli: 1000, unitPriceMinor: 5000 }], discountMinor: -1 }),
  );
});
...
test("jurisdictions combine ADDITIVELY into one rate", () => {
  ...
  const { effectiveBps, applied, skipped } = resolveTaxJurisdictions(rules, {
    onDate: "2026-07-31",
  });
...
test("a resolved rate produces EXACTLY the flat-rate figures — no drift", () => {
  ...
  for (let cents = 1; cents <= 20000; cents += 1) {
    const flat = computeDocument({
      items: [{ qtyMilli: 1000, unitPriceMinor: cents }],
      taxRateBps: 825,
    });
    const viaRules = computeDocument({
      items: [{ qtyMilli: 1000, unitPriceMinor: cents }],
      taxRateBps: bps,
    });
...
test("an inactive rule is skipped and says so", () => {
  const out = resolveTaxJurisdictions(
    [{ rate_bps: 900, applies_to: "all", active: false, effective_from: "2020-01-01" }],
    { onDate: "2026-07-31" },
  );
...
test("a jurisdiction with a non-integer rate is rejected, not rounded", () => {
  assert.throws(() =>
    resolveTaxJurisdictions([{ rate_bps: 8.25, applies_to: "all" }], { onDate: "2026-07-31" }),
  );
  assert.throws(() =>
    resolveTaxJurisdictions([{ rate_bps: 100001, applies_to: "all" }], { onDate: "2026-07-31" }),
  );
});

test("customer exemption: a valid certificate means no tax, and the base is still reported", () => {
  const taxed = computeDocument({
    items: [{ qtyMilli: 1000, unitPriceMinor: 50000 }],
    taxRateBps: 825,
  });
  assert.equal(taxed.taxMinor, 4125);
  const exempt = computeDocument({
    items: [{ qtyMilli: 1000, unitPriceMinor: 50000 }],
    taxRateBps: 825,
    taxExempt: true,
  });
...
test("exemption validity is checked against the document date", () => {
  ...
  assert.equal(
    isCustomerTaxExempt([{ active: true, expires_on: null }], { onDate: "2099-01-01" }),
    true,
  );
  assert.equal(
    isCustomerTaxExempt([{ active: false, expires_on: null }], { onDate: "2026-07-31" }),
    false,
  );
