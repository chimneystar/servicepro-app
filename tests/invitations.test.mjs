...
test("the acceptance link carries the token to /join", () => {
  ...
  assert.equal(
    invitationAcceptUrl("https://app.servicepro.test/", token),
    `https://app.servicepro.test/join?token=${token}`,
  );
});
...
test("the invitation email contains the link and escapes everything untrusted", () => {
  ...
  const { subject, html, text } = invitationEmail({
    locale: "en",
    businessName: `Ace <script>alert("x")</script> Plumbing`,
    inviterName: "Dana & Co",
    role: "office",
    acceptUrl,
  });
...
test("an invitation nobody was told about is NOT reported as sent", () => {
  ...
  assert.match(
    describeInviteDelivery({ delivery_status: "unavailable" }, "en").text,
    /NOT emailed/,
  );
  assert.equal(
    describeInviteDelivery(
      { delivery_status: "failed", delivery_error: "550 mailbox unavailable" },
      "en",
    ).tone,
    "error",
  );
  ...
  assert.notEqual(
    describeInviteDelivery({ delivery_status: "sent" }, "he").text,
    describeInviteDelivery({ delivery_status: "sent" }, "en").text,
  );
});
...
test("acceptance goes through the token, not the email address alone", () => {
  ...
  // `\s*` throughout: the guarded property is that acceptance passes the TOKEN
  // as the RPC's `invite_token` argument, not where Prettier wrapped the object
  // literal (ledger 6.4 moved it onto four lines). Every identifier and every
  // string in the call is still named, so an email-only call, a renamed
  // argument or a different RPC all still fail.
  assert.match(
    onboarding,
    /rpc\(\s*"accept_invitation",\s*\{\s*invite_token:\s*inviteToken,?\s*\}\s*\)/,
  );
  assert.doesNotMatch(
    onboarding,
    /rpc\("accept_invitation"\)/,
    "the email-only form must not be what joins a business",
  );
...
test("migration 034 makes the token a requirement and keeps the 023 owner guard", () => {
  ...
  assert.match(
    sql,
    /lower\(inv\.email\) is distinct from lower\(em\)/,
    "the email must still have to match the token's invitation",
  );
  ...
  assert.match(
    sql,
    /grant\s+execute on function public\.accept_invitation\(text\) to authenticated/,
  );
  assert.match(
    sql,
  ...
  assert.doesNotMatch(
    sql.split("pending_invitation_hint")[1] ?? "",
    /i\.token/,
    "the hint must never return the token",
  );
});
...
test("every column migration 034 writes to exists in the schema it is built on", () => {
  ...
  const foundation = stripSqlComments(
    readFileSync(join(root, "db", "018_product_foundation.sql"), "utf8"),
  );
  const platform = stripSqlComments(
    readFileSync(join(root, "db", "022_operations_privacy_team_admin.sql"), "utf8"),
  );

  // Tables this migration alters must already exist somewhere.
  for (const [table, definition] of [
    ["public.invitations", baseline],
    ["public.push_notification_events", foundation],
  ]) {
    assert.match(
      sql,
      new RegExp(`alter table ${table.replace(".", "\\.")}`),
      `${table} is altered`,
    );
    assert.match(
      definition,
