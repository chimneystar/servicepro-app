...
test("cash is computed by the SHARED reporting rule, not re-summed here", () => {
  ...
  assert.doesNotMatch(
    source,
    /normalized_status/,
    "a second copy of the collected-status rule would drift",
  );
});
...
test("a draft or void invoice is NEVER billed on a statement", () => {
  const withDraft = [
    ...invoices,
    { id: "i9", number: 5009, issue_date: "2026-06-20", total_minor: 999900, status: "draft" },
  ];
  assert.equal(
    buildStatement({ invoices: withDraft, payments, asOf: "2026-07-01" }).balanceMinor,
    50000,
  );
  const withVoid = [
    ...invoices,
    { id: "i8", number: 5008, issue_date: "2026-06-20", total_minor: 999900, status: "void" },
  ];
  assert.equal(
    buildStatement({ invoices: withVoid, payments, asOf: "2026-07-01" }).balanceMinor,
    50000,
  );
...
test("a deleted invoice is excluded even when its status says unpaid", () => {
  ...
  assert.equal(
    buildStatement({ invoices: withDeleted, payments, asOf: "2026-07-01" }).balanceMinor,
    50000,
  );
});
...
test("a payment against an invoice not on this statement cannot reduce the balance", () => {
  const foreign = [
    ...payments,
    settled({
      invoice_id: "someone-elses",
      paid_at: "2026-06-01T00:00:00Z",
      base_amount_minor: 50000,
    }),
  ];
  assert.equal(
    buildStatement({ invoices, payments: foreign, asOf: "2026-07-01" }).balanceMinor,
    50000,
  );
});
...
test("nothing dated after asOf appears on the statement", () => {
  ...
  assert.equal(
    s.lines.some((l) => l.date > "2026-05-10"),
    false,
  );
...
test("a windowed statement folds earlier activity into the OPENING balance, losing nothing", () => {
  ...
  assert.equal(
    windowed.balanceMinor,
    full.balanceMinor,
    "a narrower window must not change what is owed",
  );
...
test("a fully paid invoice appears in no aging bucket", () => {
  ...
  assert.equal(
    Object.values(s.aging).reduce((a, b) => a + b, 0),
    0,
  );
...
test("each rung fires in turn as the invoice ages", () => {
  const at = (days, sent) =>
    nextDunningStage({ ageDays: days, outstandingMinor: 10000 }, sent)?.stage ?? null;
...
test("a rung is never sent twice", () => {
  assert.equal(
    nextDunningStage({ ageDays: 20, outstandingMinor: 10000 }, ["reminder", "overdue"]),
    null,
  );
});
...
test("the dunning sender honours the SHARED opt-out rule", () => {
  ...
  assert.doesNotMatch(
    body,
    /sms_opt_in\s*===\s*false/,
    "an inline consent check would be the second copy",
  );
});
