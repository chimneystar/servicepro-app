...
function policiesCreatedIn(sql) {
  return [...sql.matchAll(/create\s+policy\s+([a-z0-9_]+)\s+on\s+([a-z0-9_."]+)/gi)].map((m) => ({
    policy: m[1].toLowerCase(),
    table: clean(m[2]),
  }));
}
  ...
  for (const { file, table } of REPLACEMENTS) {
    ...
    for (const name of priorNames) {
      if (!droppedHere.has(name)) {
        problems.push(
          `${file} tightens ${table} but never drops the pre-existing policy "${name}" — it survives and is OR'd with the new one, so nothing is actually restricted`,
        );
      }
    }
  }
