...
const allDay={"1":["00:00","23:59"],"2":["00:00","23:59"],"3":["00:00","23:59"],"4":["00:00","23:59"],"5":["00:00","23:59"],"6":["00:00","23:59"],"7":["00:00","23:59"]};
...
const base={intervalMin:60,arrivalWindowMin:120,minNoticeHours:0,maxDaysAhead:90,capacity:1,busy:[]};
const at=(iso)=>new Date(iso).getTime();

test("booking slots respect business hours and service duration",()=>{
  const slots=buildBookingSlots({...base,date:"2026-08-03",hours,durationMin:90,timeZone:"America/Chicago",now:at("2026-08-01T13:00:00Z")});
...
test("a full-capacity overlap removes the slot",()=>{
  const slots=buildBookingSlots({...base,date:"2026-08-03",hours,durationMin:60,arrivalWindowMin:60,busy:[{start:"09:00",end:"10:00"}],timeZone:"America/Chicago",now:at("2026-08-01T13:00:00Z")});
  assert.equal(slots.some((slot)=>slot.start==="09:00"),false);
  assert.equal(slots.some((slot)=>slot.start==="10:00"),true);
});

test("team capacity keeps a slot until every technician is busy",()=>{
  const slots=buildBookingSlots({...base,date:"2026-08-03",hours,durationMin:60,arrivalWindowMin:60,capacity:2,busy:[{start:"09:00",end:"10:00"}],timeZone:"America/Chicago",now:at("2026-08-01T13:00:00Z")});
  assert.equal(slots.some((slot)=>slot.start==="09:00"),true);
});
...
test("minimum notice is measured in the business's zone, not the server's",()=>{
  ...
  const input={...base,date:"2026-08-03",hours,durationMin:60,minNoticeHours:4,timeZone:"America/Chicago",now:at("2026-08-03T14:00:00Z")};
  const slots=buildBookingSlots(input);
  // REJECTS what is already too soon...
  assert.equal(slots.some((slot)=>slot.start==="09:00"),false,"09:00 local is now — must not be offered");
  assert.equal(slots.some((slot)=>slot.start==="12:00"),false,"12:00 local is inside the 4h notice window");
...
test("the same instant produces different cutoffs in different zones",()=>{
  const shared={...base,date:"2026-08-03",hours,durationMin:60,minNoticeHours:2,now:at("2026-08-03T15:00:00Z")};
...
test("the day boundary is the business's midnight, not the server's",()=>{
  ...
  const today=buildBookingSlots({...base,date:"2026-08-03",hours:allDay,durationMin:30,maxDaysAhead:0,timeZone:"America/Chicago",now:at("2026-08-04T03:00:00Z")});
  ...
  const tomorrow=buildBookingSlots({...base,date:"2026-08-04",hours:allDay,durationMin:30,maxDaysAhead:0,timeZone:"America/Chicago",now:at("2026-08-04T03:00:00Z")});
...
test("a past day is refused even when the server has not reached midnight",()=>{
  ...
  assert.equal(buildBookingSlots({...base,date:"2026-08-03",hours:allDay,durationMin:30,timeZone:"Asia/Tokyo",now:at("2026-08-03T23:00:00Z")}).length,0);
  assert.ok(buildBookingSlots({...base,date:"2026-08-04",hours:allDay,durationMin:30,timeZone:"Asia/Tokyo",now:at("2026-08-03T23:00:00Z")}).length>0);
});
...
test("DST transitions resolve to the correct real instant",()=>{
  ...
  const spring=buildBookingSlots({...base,date:"2026-03-09",hours,durationMin:60,minNoticeHours:1,timeZone:"America/New_York",now:at("2026-03-09T12:00:00Z")});
  ...
  const phoenix=buildBookingSlots({...base,date:"2026-03-09",hours,durationMin:60,minNoticeHours:1,timeZone:"America/Phoenix",now:at("2026-03-09T12:00:00Z")});
...
test("an unknown or missing timezone falls back instead of throwing",()=>{
  ...
  const slots=buildBookingSlots({...base,date:"2026-08-03",hours,durationMin:60,timeZone:"Not/AZone",now:at("2026-08-01T13:00:00Z")});
...
test("a malformed date yields no slots rather than an invalid instant",()=>{
  for (const date of ["","not-a-date","2026-13-01","2026-02-31","20260803"]) {
    assert.equal(buildBookingSlots({...base,date,hours,durationMin:60,timeZone:"America/Chicago",now:at("2026-08-01T13:00:00Z")}).length,0,date);
  }
});
...
const zipAndCity=[{area_type:"zip",values_json:["78701","78702"],active:true},{area_type:"city",values_json:["Lakeway"],active:true}];
...
test("evaluateServiceArea reports match and outside for checkable areas",()=>{
  ...
  assert.equal(evaluateServiceArea("00000","Dallas",[{area_type:"zip",values_json:["78701"],active:false}]),"match");
});

test("a polygon-only configuration is unevaluable, never a silent accept",()=>{
  const polygonsOnly=[{area_type:"polygon",values_json:["whatever the owner typed"],active:true}];
...
test("enforcement gaps tell the owner exactly what is not being checked",()=>{
  assert.deepEqual(serviceAreaEnforcementGaps(zipAndCity),{total:2,polygons:0,enforceable:2,unenforceable:false});
  assert.deepEqual(serviceAreaEnforcementGaps([{area_type:"polygon",values_json:[],active:true}]),{total:1,polygons:1,enforceable:0,unenforceable:true});
  assert.deepEqual(serviceAreaEnforcementGaps([...zipAndCity,{area_type:"polygon",values_json:[],active:true}]),{total:3,polygons:1,enforceable:2,unenforceable:false});
  // Nothing configured is not a lie — the toggle simply has nothing to enforce.
  assert.deepEqual(serviceAreaEnforcementGaps([]),{total:0,polygons:0,enforceable:0,unenforceable:false});
  assert.equal(serviceAreaEnforcementGaps([{area_type:"polygon",values_json:[],active:false}]).unenforceable,false);
});
