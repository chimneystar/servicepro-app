...
import {
  LOGIN_POLICY, accountLockMs, evaluateLoginAttempt, attemptsRemaining,
  describeRetryAfter, describeThrottle, invalidCredentialsMessage,
...
test("a normal sign-in is ALLOWED, including after a couple of typos", () => {
  assert.deepEqual(
    evaluateLoginAttempt({ accountFailures: 0, ipFailures: 0, now: NOW }),
    { allowed: true, reason: null, retryAfterSeconds: 0 },
  );
  // Four failures is under the threshold: somebody who mistypes must still get in.
  assert.equal(evaluateLoginAttempt({ accountFailures: 4, lastAccountFailureAt: NOW - 1000, now: NOW }).allowed, true);
...
test("the fifth failure LOCKS the account, and the lock expires on its own", () => {
  ...
  const later = evaluateLoginAttempt({
    accountFailures: 5, lastAccountFailureAt: NOW, now: NOW + LOGIN_POLICY.accountBaseLockMs + 1000,
  });
...
test("the lock escalates with persistence and then stops escalating", () => {
  ...
  assert.equal(accountLockMs(1000), LOGIN_POLICY.accountMaxLockMs, "capped, so a lock can never become permanent");
});
...
test("a password spray across many accounts is stopped by the NETWORK gate", () => {
  ...
  const spray = evaluateLoginAttempt({ accountFailures: 1, ipFailures: 20, lastIpFailureAt: NOW, now: NOW });
  ...
  assert.equal(evaluateLoginAttempt({ accountFailures: 1, ipFailures: 19, lastIpFailureAt: NOW, now: NOW }).allowed, true);
});

test("the network gate is checked FIRST, so a spray is named as a spray", () => {
  const both = evaluateLoginAttempt({
    accountFailures: 5, lastAccountFailureAt: NOW, ipFailures: 20, lastIpFailureAt: NOW, now: NOW,
  });
...
test("counts with no timestamp cannot lock anyone out for ever", () => {
  ...
  assert.equal(evaluateLoginAttempt({ accountFailures: 99, lastAccountFailureAt: null, now: NOW }).allowed, true);
  assert.equal(evaluateLoginAttempt({ ipFailures: 99, lastIpFailureAt: null, now: NOW }).allowed, true);
});
...
test("no message ever reveals whether the account exists", () => {
  for (const locale of ["en", "he"]) {
    ...
    assert.ok(!/exist|registered|unknown|לא נמצא|קיים/.test(wrong), `"${wrong}" must not disclose account existence`);
    const throttled = describeThrottle("account_locked", 300, locale);
    assert.ok(!/exist|registered|לא נמצא/.test(throttled));
    assert.ok(describeThrottle("network_locked", 300, locale) !== throttled, "the two reasons read differently");
  }
});
...
test("a genuinely new device DOES alert, and a known one does not", () => {
  ...
  assert.equal(isNewSignInDevice("Chrome on Mac|192.0.2.0/24", known), true, "same browser, different country");
});

test("the alert email escapes what it interpolates and links to the security page", () => {
  const alert = loginAlertEmail({
    locale: "en", device: `Chrome <script>alert("x")</script>`, ip: "203.0.113.9",
    at: "Fri, 31 Jul 2026 12:00:00 GMT", appUrl: "https://app.servicepro.test/", escape: escapeHtml,
  });
  ...
  const he = loginAlertEmail({ locale: "he", device: "Chrome on Mac", ip: null, at: "x", appUrl: "" });
...
test("sign-in happens on the server and is both throttled and recorded", () => {
  ...
  assert.ok(action.includes("login_throttle_counts"), "counts come from Postgres, not from process memory alone");
  ...
  assert.ok(action.includes("invalidCredentialsMessage"), "failures must not disclose account existence");
});
...
test("the login form no longer calls Supabase from the browser", () => {
  ...
  assert.ok(!form.includes("signInWithPassword"), "the browser cannot throttle, record or alert on itself");
...
test("the limiter's per-instance caveat is respected rather than ignored", () => {
  ...
  assert.match(limiter, /NOT a distributed[\s\S]{0,8}limiter/, "the honest note must still be there");
