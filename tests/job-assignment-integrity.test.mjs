...
const readSql = (p) => read(p).replace(/^\s*--.*$/gm, "").toLowerCase();
...
test("23505 is recognised as a unique violation, and nothing else is", () => {
  ...
  for (const other of [{ code: "23P01" }, { code: "23503" }, { code: "42501" }, { code: "2350" }, { code: "235050" }]) {
    assert.equal(isUniqueViolation(other), false, `${JSON.stringify(other)} must not read as a duplicate`);
  }
});

test("23P01 is recognised as a double-book conflict, and nothing else is", () => {
  assert.equal(isDoubleBookConflict({ code: "23P01", message: "conflicting key value violates exclusion constraint" }), true);
  for (const other of [{ code: "23505" }, { code: "23p01" }, { code: "P0001" }, { code: "" }]) {
    assert.equal(isDoubleBookConflict(other), false, `${JSON.stringify(other)} must not read as a booking conflict`);
  }
});
...
test("the unique index is what makes the second clock-in a no-op", () => {
  ...
  const store = [];
  const insert = (row) => {
    if (store.some((e) => e.job_id === row.job_id && e.user_id === row.user_id && e.ended_at === null)) {
  ...
  assert.deepEqual(clockIn("job-1", "tech-1"), { ok: true }, "the double tap must succeed silently, not error at the technician");
  assert.equal(store.filter((e) => e.ended_at === null).length, 1, "exactly one open timer — the job page must not double-count");
...
test("clockIn relies on the constraint instead of a pre-read", () => {
  ...
  const body = code.slice(code.indexOf("export async function clockIn"), code.indexOf("export async function clockOut"));
  assert.ok(body.length > 0, "clockIn must still exist");
  assert.doesNotMatch(body, /from\("job_time_entries"\)\s*\n?\s*\.select/, "the read-then-write race must be gone");
  assert.match(body, /isUniqueViolation\(error\)/, "23505 must be handled as 'already clocked in'");
  assert.match(body, /if \(error && !isUniqueViolation\(error\)\) return \{ ok: false/, "every OTHER error must still surface");
});

test("the open-entry uniqueness actually exists in the schema", () => {
  const sql = readSql("db/023_authorization_hardening.sql");
  assert.match(sql, /create unique index if not exists uq_job_time_entries_one_open\s+on public\.job_time_entries \(job_id, user_id\)\s+where ended_at is null/);
});
...
const extraCrew = (assignments, job) =>
  assignments.filter((row) => row.job_id === job.id && row.profile_id && row.profile_id !== job.assigned_to)
...
test("THE BUG: the previous lead is left behind and renders as extra crew", () => {
  ...
  assert.deepEqual(extraCrew(assignments, job), ["tech-a"], "tech-a still shows on a job they no longer have");
});

/** The fixed move: retire stale lead rows, then claim the new one. */
function moveAssignments(assignments, jobId, profileId) {
  const kept = assignments.filter((row) => !(row.job_id === jobId && row.is_lead && row.profile_id !== profileId));
...
test("reassigning retires the outgoing lead's row", () => {
  const assignments = moveAssignments([{ job_id: "job-1", profile_id: "tech-a", is_lead: true }], "job-1", "tech-b");
...
test("moving a job to Unassigned removes the lead row too", () => {
  ...
  const assignments = moveAssignments([{ job_id: "job-1", profile_id: "tech-a", is_lead: true }], "job-1", null);
...
test("genuine extra crew and other jobs survive a reassignment", () => {
  ...
  assert.deepEqual(extraCrew(after, { id: "job-1", assigned_to: "tech-b" }), ["tech-c"], "crew must be untouched");
  assert.ok(after.some((r) => r.job_id === "job-2" && r.profile_id === "tech-a" && r.is_lead), "another job's lead must be untouched");
});
...
test("moveDispatchJob clears the stale lead in both the reassign and unassign paths", () => {
  ...
  const body = code.slice(code.indexOf("export async function moveDispatchJob"), code.indexOf("export async function addJobTechnician"));
  assert.match(body, /from\("job_assignments"\)\.delete\(\)/, "the outgoing lead's row must be deleted");
  assert.match(body, /\.eq\("is_lead", true\)/, "only lead rows may be retired — crew must survive");
  assert.match(body, /profileId \? stale\.neq\("profile_id", profileId\) : stale/, "the unassign path must delete too, not skip");
});

test("a double-book is reported as a double-book, not as 'something went wrong'", () => {
  const code = readCode("app/(app)/dispatch/actions.ts");
  assert.match(code, /isDoubleBookConflict\(error\)\) return \{ ok: false, error: t\(locale, "sched\.conflict"\)/);
  assert.match(code, /export type DispatchResult = \{ ok: boolean; error\?: string \}/, "the action must be able to carry a reason");
  const board = readCode("components/DispatchBoard.tsx");
  assert.match(board, /setNotice\(result\.error \|\| failed\)/, "the board must show the reason it was given");
  assert.equal((board.match(/setNotice\(result\.error \|\| failed\)/g) ?? []).length, 3, "move, add and remove must all report");
});
...
test("THE BUG: only jobs.assigned_to is covered by an exclusion constraint", () => {
  ...
  const table = growth.slice(growth.indexOf("create table if not exists public.job_assignments"), growth.indexOf("create table if not exists public.service_areas"));
  for (const timeColumn of ["start_time", "end_time", "scheduled_date", "slot"]) {
    assert.doesNotMatch(table, new RegExp(timeColumn), `job_assignments has no ${timeColumn} — the overlap lives on the job`);
  }
});
...
test("the crew predicate catches overlaps the lead-only constraint misses", () => {
  ...
  const jobs = [
    { id: "j1", date: "2026-08-03", start: "09:00", end: "11:00", assigned_to: "lead-1", status: "scheduled", deleted_at: null },
    { id: "j2", date: "2026-08-03", start: "10:00", end: "12:00", assigned_to: "lead-2", status: "scheduled", deleted_at: null },
    { id: "j3", date: "2026-08-03", start: "13:00", end: "14:00", assigned_to: "lead-2", status: "scheduled", deleted_at: null },
    { id: "j4", date: "2026-08-04", start: "10:00", end: "12:00", assigned_to: "lead-2", status: "scheduled", deleted_at: null },
    { id: "j5", date: "2026-08-03", start: "10:00", end: "12:00", assigned_to: "lead-3", status: "cancelled", deleted_at: null },
    { id: "j6", date: "2026-08-03", start: "10:30", end: "11:30", assigned_to: null, status: "scheduled", deleted_at: "2026-07-01" },
  ];
  ...
  const doubleBooked = (jobId, profileId) => {
    ...
    return jobs.some((other) => other.id !== jobId && !other.deleted_at && other.status !== "cancelled" && overlaps(other, job)
      && (other.assigned_to === profileId || crew.some((a) => a.job_id === other.id && a.profile_id === profileId && a.status !== "declined")));
  };
  // FIRES: tech-x is crew on j2 (10-12); putting them on j1 (9-11) overlaps.
  assert.equal(doubleBooked("j1", "tech-x"), true, "the guard must refuse an overlapping crew assignment");
  // FIRES: the reverse direction — the lead of an overlapping job as crew.
  assert.equal(doubleBooked("j1", "lead-2"), true, "crew must also collide with that person's own lead jobs");
  ...
  assert.equal(doubleBooked("j5", "tech-x"), false, "assigning onto a cancelled job is not checked");
});

test("migration 028 enforces the same rule at the database", () => {
  const sql = readSql("db/028_crew_double_book.sql");
  assert.match(sql, /create or replace function public\.crew_double_booked/, "the predicate must exist");
  assert.match(sql, /public\.job_assignments/, "it must look at crew rows, not only jobs.assigned_to");
  assert.match(sql, /other\.slot && v_slot/, "overlap must be range overlap, matching jobs_no_double_book");
  ...
  assert.match(sql, /create trigger trg_job_assignments_no_double_book\s+before insert or update[\s\S]{0,120}on public\.job_assignments/);
  assert.match(sql, /create trigger trg_jobs_crew_no_double_book\s+after update[\s\S]{0,120}on public\.jobs/);
...
test("migration 028 is re-runnable and destroys nothing", () => {
  ...
  assert.equal((sql.match(/create or replace function/g) ?? []).length, 3, "functions must be replaceable");
  for (const destructive of [/drop table/, /drop column/, /drop constraint/, /drop function/, /delete from/, /truncate/]) {
    assert.doesNotMatch(sql, destructive, `${destructive} must not appear in a hardening migration`);
  }
