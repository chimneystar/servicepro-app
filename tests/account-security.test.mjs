...
test("every new table has RLS enabled, and none is reachable by anon", () => {
  ...
  for (const table of NEW_TABLES) {
    ...
    assert.match(sql038, new RegExp(`revoke all on public\\.%I from anon|'${table}'`), `${table} must be in the anon revoke list`);
  }
...
test("the audit tables are READ-ONLY to members — an audit trail a member can write is one they can forge", () => {
  ...
  for (const table of ["auth_login_attempts", "account_security_events", "permission_change_log", "document_signature_events"]) {
  ...
  for (const table of ["auth_login_attempts", "permission_change_log", "document_signature_events"]) {
    assert.ok(!new RegExp(`create policy [a-z_]+ on public\\.${table} for (insert|all)`).test(sql038), `${table} must have no write policy`);
  }
});
...
test("login attempts and permission history are OWNER-only, and scoped to one organisation", () => {
  for (const policy of ["auth_login_attempts_select", "permission_change_log_select"]) {
    const clause = sql038.slice(sql038.indexOf(`create policy ${policy}`), sql038.indexOf(`create policy ${policy}`) + 300);
    assert.match(clause, /organization_id = public\.current_org_id\(\)/, `${policy} must be tenant-scoped`);
...
test("038 does not touch a single object 023 hardened", () => {
  for (const object of ["profiles_self_update", "accept_invitation", "invitations_write", "invitations_select", "jobs_update", "subscriptions_select"]) {
    assert.ok(!sql038.includes(object), `038 must not reference ${object}`);
  }
  assert.ok(!/drop policy/i.test(sql038.replace(/drop policy if exists (auth_login_attempts|account_security_events|permission_change_log|document_signature_events|profile_security)[a-z_]* on[^;]*;/g, "")),
...
test("023's guards are still present in 023, unmodified", () => {
  ...
  assert.match(sql023, /create policy profiles_self_update on public\.profiles for update to authenticated/);
  assert.match(sql023, /role\s+= \(select p\.role\s+from public\.profiles p where p\.id = auth\.uid\(\)\)/);
...
test("023 §6's sign-once guard survives being moved behind the evidence function", () => {
  ...
  assert.equal((sql038.match(/and signed_at is null/g) ?? []).length, 2, "the guard applies to BOTH estimates and invoices");
  assert.match(sql038, /create or replace function public\.approve_document\(p_token uuid, p_name text, p_sig text\)\s*\nreturns boolean/,
    "the signature and return type must be unchanged, or every existing caller breaks");
  assert.match(sql038, /grant\s+execute on function public\.approve_document\(uuid, text, text\) to anon, authenticated;/,
...
test("the evidence-taking entry point is service-role only — forged evidence is worse than none", () => {
  assert.match(sql038, /revoke all on function public\.approve_document_with_evidence\([^)]*\) from public, anon, authenticated;/);
  assert.match(sql038, /grant execute on function public\.approve_document_with_evidence\([^)]*\) to service_role;/);
  assert.ok(!/grant execute on function public\.approve_document_with_evidence\([^)]*\) to anon/.test(sql038),
...
test("the documents themselves gain the evidence columns", () => {
  for (const table of ["estimates", "invoices"]) {
    assert.match(sql038, new RegExp(`alter table public\\.${table}\\s+add column if not exists signature_ip inet`));
    assert.match(sql038, new RegExp(`alter table public\\.${table}\\s+add column if not exists signature_user_agent text`));
  }
  assert.match(sql038, /capture\s+text not null default 'none' check \(capture in \('none', 'server'\)\)/,
...
test("signing goes through the server, which is the only place a request context exists", () => {
  ...
  assert.ok(!component.includes('supabase.rpc("approve_document"'), "the browser must no longer call the RPC directly");
  ...
  assert.ok(action.includes("approve_document_with_evidence"), "the evidence-taking entry point is used");
  assert.ok(action.includes("getRequestContext"), "the IP and user agent come from the SERVER's view of the request");
  assert.ok(action.includes("createHash"), "a hash of exactly what was stored makes the record checkable later");
  assert.ok(action.includes("consume("), "signing is anonymous and reachable by anyone holding the link");
});
...
test("permission changes are recorded by TRIGGER, so PostgREST cannot skip the log", () => {
  ...
  for (const [table, timing] of [
    ...
    assert.ok(flat.includes(`${timing} on public.${table} for each row execute function public.record_permission_change()`),
...
test("the watched columns are exactly the ones that confer authority", () => {
  ...
  for (const column of (capabilities.match(/can_[a-z_]+/g) ?? [])) {
    if (!capabilities.includes(`  ${column}      `) && !capabilities.includes(`  ${column} `)) continue;
    assert.ok(sql038.includes(`'${column}'`), `profile_capabilities.${column} must be watched`);
  }
  for (const column of ["can_confirm_manual_payments", "can_refund_payments", "can_override_ach_holds"]) {
    assert.ok(sql038.includes(`'${column}'`), `${column} must be watched — this is the refund authority`);
  }
  assert.match(sql038, /if changes = '\{\}'::jsonb then/, "an ordinary edit must not fill the history with noise");
});
...
test("the team actions attach the request context to the change they just caused", () => {
  ...
  assert.ok(actions.includes("stamp_permission_change_context"), "the trigger cannot see an HTTP header");
  // Every authority-changing action must stamp.
  for (const name of ["changeRole", "updateCapabilities", "updatePaymentPermissions", "removeMember", "inviteMember", "cancelInvite"]) {
    const body = actions.slice(actions.indexOf(`export async function ${name}`));
    const end = body.indexOf("export async function", 10);
    assert.ok((end === -1 ? body : body.slice(0, end)).includes("stampPermissionContext"), `${name} must stamp the context`);
  }
});

test("stamping can only ever touch the caller's OWN changes", () => {
  const body = sql038.slice(sql038.indexOf("create or replace function public.stamp_permission_change_context"));
  assert.match(body, /where actor_profile_id = auth\.uid\(\)/, "otherwise it is a way to forge provenance");
...
test("the login functions are service-role only — the browser must not be able to write its own record", () => {
  for (const fn of ["record_login_attempt", "login_throttle_counts"]) {
    assert.ok(new RegExp(`revoke all on function public\\.${fn}\\([^)]*\\) from public, anon, authenticated;`).test(sql038), `${fn} must be revoked from anon and authenticated`);
    assert.ok(new RegExp(`grant execute on function public\\.${fn}\\([^)]*\\) to service_role;`).test(sql038), `${fn} must be granted to service_role`);
  }
});
...
test("the audit log has a real reader with filters and pagination", () => {
  ...
  assert.ok(page.includes('from("audit_log")'), "this is the first reader beyond a single record's 30-row timeline");
  assert.ok(page.includes('count: "exact"') && page.includes(".range("), "a log you cannot page through is a log you cannot read");
...
test("the previous single-record reader is untouched", () => {
  ...
  assert.ok(activity.includes(".limit(30)"), "the per-record timeline must keep working exactly as before");
});
...
test("the security centre is reachable from the screens that lead to it", () => {
  for (const file of ["app/(app)/team/page.tsx", "app/(app)/settings/privacy/page.tsx"]) {
    assert.ok(read(file).includes("/settings/security"), `${file} must link to it — an unreachable screen is not a feature`);
  }
});
...
test("two-factor is enrolled through Supabase itself, and its state is mirrored for audit only", () => {
  ...
  assert.ok(panel.includes("auth.mfa.challengeAndVerify"), "enrolment is not complete until a code is verified");
  assert.ok(panel.includes("auth.mfa.unenroll"));
  assert.ok(panel.includes("auth.mfa.listFactors"), "the factor list on screen must be the real one, not our mirror");
  assert.ok(panel.includes("recordTwoFactorChange"), "turning MFA OFF must be as auditable as turning it on");

  const login = strip(read("app/login/actions.ts"));
  assert.ok(login.includes("getAuthenticatorAssuranceLevel"), "a verified factor must actually gate the sign-in");
...
test("a lost phone can be signed out, and the revocation is recorded", () => {
  ...
  assert.ok(actions.includes('signOut({ scope: "global" })'), "this is the real revocation, not a cosmetic flag");
...
test("a new-device sign-in alerts, and an undeliverable alert is RECORDED rather than dropped", () => {
  ...
  assert.ok(login.includes("login_alert_undelivered"), "silence is the failure mode this branch exists to remove");
  assert.ok(login.includes("login_alert_failed"));
  assert.ok(login.includes("login_alerts_enabled"), "the alert must be something a person can turn off");
});
...
test("the request context is captured where it is evidence and nowhere else", () => {
  const users = [
    "app/login/actions.ts", "app/signup/actions.ts", "app/p/[token]/actions.ts",
    "app/(app)/team/actions.ts", "app/(app)/settings/security/actions.ts",
  ];
  ...
  for (const file of ["proxy.ts", "app/(app)/layout.tsx", "lib/supabase/middleware.ts"]) {
    assert.ok(!read(file).includes("getRequestContext"), `${file} must NOT capture a context for every request`);
  }
  assert.match(read("lib/core/request-context.mjs"), /never as a\s*\n\/\/ by-product of ordinary browsing/);
});
