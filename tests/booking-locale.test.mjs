...
const readOrEmpty = (relative) => { try { return read(relative); } catch { return ""; } };
...
function effectiveTrigger(name) {
  ...
  for (const { sql } of MIGRATIONS) {
    const clean = stripSqlComments(sql);
    const re = new RegExp(`create\\s+(?:or\\s+replace\\s+)?trigger\\s+${name}\\b([\\s\\S]*?);`, "gi");
    let match;
...
function columnsOf(table) {
  ...
  if (found) {
    for (const line of found[1].split("\n")) {
      ...
      if (column && !/^(primary|unique|check|constraint|foreign|exclude)$/i.test(column[1])) columns.add(column[1].toLowerCase());
    }
  }
  const added = new RegExp(`alter\\s+table\\s+public\\.${table}\\s+add\\s+column\\s+(?:if\\s+not\\s+exists\\s+)?([a-z_][a-z0-9_]*)`, "gi");
  let match;
...
test("A5: translating a job type re-syncs its booking service", () => {
  ...
  assert.match(trigger, /\bname\b/, "renaming a job type must still re-sync (020 behaviour preserved)");
  assert.match(trigger, /duration_min[\s\S]*default_price_minor[\s\S]*sort/, "020's columns must all still fire it");
});
...
test("A5: the repair is re-runnable by the owner, not a one-shot backfill", () => {
  ...
  assert.match(SQL_041, /grant\s+execute\s+on\s+function\s+public\.repair_booking_service_names\(uuid\)\s+to\s+authenticated/i);
  assert.match(SQL_041, /revoke\s+all\s+on\s+function\s+public\.repair_booking_service_names\(uuid\)\s+from\s+public,\s*anon/i);
  ...
  assert.match(read("app/(app)/settings/booking/BookingSettingsForm.tsx"), /repairServiceNames\(\)/);
});
...
test("A5: repair replaces a mis-seeded Hebrew name and never a human's", () => {
  ...
  assert.ok(!/name_he\s*is\s+not\s+null\s*\)?\s*$/.test(where), "it must not skip null translations");

  const sync = effectiveFunction("sync_booking_service_from_job_type");
  assert.match(sync.replace(/\s+/g, " "), /bs\.name_he is null or bs\.name_he = bs\.name_en/,
...
test("A5: an unresolvable Hebrew name is left null, not filled with English", () => {
  ...
  assert.doesNotMatch(heExpr, /resolved_en/, "the Hebrew name must never fall back to the English one");
  // The public page is what makes null safe: it already falls back at render.
  assert.match(read("app/book/[org]/BookingForm.tsx"), /name_he\s*\|\|\s*item\.name_en/,
...
test("A5: the booking settings screen stops writing English into the Hebrew field", () => {
  ...
  assert.doesNotMatch(form, /name_he:\s*jobType\.name\b/, "a job type's name must not be reused as its Hebrew name");
  assert.doesNotMatch(form, /nameHe_\$\{key\}`\}\s*defaultValue=\{row\.name_he\s*\?\?\s*row\.name_en\}/,
    "the Hebrew input must not be pre-filled with the English name");
  assert.match(form, /placeholder=\{row\.name_en\}/, "it should show the English name as a hint instead");
  assert.match(read("app/(app)/settings/booking/page.tsx"), /job_types"\)\.select\("id,name,name_en,name_he/,
...
function packServices() {
  const source = read("lib/industry-packs.ts");
  const body = source.slice(source.indexOf("export const INDUSTRY_PACKS"));
  const packRe = /\{\s*key:\s*"([^"]+)",\s*en:\s*"([^"]+)",\s*he:\s*"([^"]+)",\s*items:\s*\[([\s\S]*?)\]\}/g;
  const out = [];
  let pack;
...
function catalogRows() {
  ...
  const rowRe = /\(\s*'((?:[^']|'')*)'\s*,\s*'((?:[^']|'')*)'\s*,\s*'((?:[^']|'')*)'\s*,\s*'((?:[^']|'')*)'\s*,\s*(\d+)\s*\)/g;
  const out = [];
  let row;
  while ((row = rowRe.exec(values))) {
    out.push({ pack: row[1], key: row[2], en: row[3].replaceAll("''", "'"), he: row[4].replaceAll("''", "'"), sort: Number(row[5]) });
  }
  return out;
}
...
test("A6: the database's trade catalogue is the packs, row for row", () => {
  ...
  assert.equal(sql.length, ts.length, "migration 041 must seed exactly the services lib/industry-packs.ts declares");
...
test("A6: onboarding publishes the chosen trades, bilingually", () => {
  ...
  assert.match(page, /bookableServicesFor\(trades, packLocale\)/, "the menu must come from the chosen trades");
  ...
  assert.match(insert, /pack_item_key: service\.pack_item_key/, "the job type must remember which pack item it is");
...
test("A6: a business that picks no trade still gets its own bookable services", () => {
  ...
  assert.ok(!tradesOnly.includes("job_types"), "seeding job types must not be conditional on picking a trade");
  assert.ok(!tradesOnly.includes("booking_services"), "seeding booking services must not be conditional on picking a trade");
  ...
  const generic = packs.slice(packs.indexOf("export const GENERIC_SERVICES"), packs.indexOf("export type BookableService"));
  assert.doesNotMatch(generic, /\bAC\b|HVAC|Chimney|Plumb|Duct/i, "the fallback must not name a trade");
  assert.match(generic, /Service call/);
  for (const line of generic.split("\n").filter((l) => l.includes('s("'))) assert.match(line, HEBREW, "the fallback must be translated too");
});

test("A6: the price book is still seeded from the packs, exactly as before", () => {
  const page = read("app/onboarding/page.tsx");
  const tradesOnly = blockAfter(page, "if (trades.length > 0) {");
  assert.match(tradesOnly, /catalogItemsFor\(trades, includeParts, packLocale\)/);
  ...
  for (const field of ["name:", "category:", "unit:", "price_minor:", "cost_minor:", "taxable:", "industry_key:", "pack_item_key:", "item_kind:"]) {
    assert.ok(catalog.includes(field), `price-book row lost ${field}`);
  }
  assert.match(packs, /return `\$\{packKey\}:\$\{kind\}:\$\{itemKey\}`/, "the pack item key format must not change");
});

test("A6: the hardcoded HVAC default cannot seed anybody new", () => {
  assert.match(SQL_041, /alter\s+table\s+public\.organizations\s+alter\s+column\s+job_types\s+set\s+default\s+'\{\}'::text\[\]/i);
...
test("A6: an organisation that already has job types is left alone", () => {
  ...
  assert.match(statement.replace(/\s+/g, " "), /not exists \( ?select 1 from public\.job_types jt where jt\.organization_id = oi\.organization_id ?\)/,
  ...
  assert.doesNotMatch(SQL_041, /delete\s+from\s+public\.(job_types|booking_services)/i, "no service list may be removed");
  assert.doesNotMatch(SQL_041, /update\s+public\.job_types[\s\S]{0,400}set[\s\S]{0,200}\bname\s*=/i, "no job type may be renamed");
});
...
test("041 drops nothing", () => {
  ...
  assert.doesNotMatch(SQL_041, /\bdrop\s+(table|column|policy|trigger|function|index|constraint|type|view)\b/i);
  assert.doesNotMatch(SQL_041, /\btruncate\b/i);
});

test("041 is idempotent", () => {
  for (const match of SQL_041.matchAll(/create\s+(table|index|unique index|policy|trigger|function)\b/gi)) {
    if (/policy/i.test(match[1])) {
      ...
      assert.match(block, /not\s+exists\s*\(\s*select\s+1\s+from\s+pg_policies/i, "unguarded create policy");
      continue;
    }
    const line = SQL_041.slice(Math.max(0, match.index - 40), match.index + 80);
    assert.match(line, /if\s+not\s+exists|or\s+replace/i,
      `unguarded ${match[1]} at: ${line.trim().slice(0, 90)}`);
  }
  assert.match(SQL_041, /on\s+conflict\s*\(pack_key,item_key\)\s*do\s+update/i, "re-seeding the catalogue must refresh, not fail");
...
test("041 only touches columns and triggers that exist", () => {
  ...
  for (const column of ["id", "organization_id", "name", "color", "duration_min", "default_price_minor", "sort", "name_en", "name_he", "pack_key", "pack_item_key"]) {
  ...
  for (const column of ["organization_id", "job_type_id", "name_en", "name_he", "duration_min", "price_minor", "sort", "active", "book_as"]) {
    assert.ok(bookingServices.has(column), `booking_services.${column} is referenced but never created`);
  }
  const industries = columnsOf("organization_industries");
  for (const column of ["organization_id", "industry_key", "services_imported"]) {
    assert.ok(industries.has(column), `organization_industries.${column} is referenced but never created`);
  }
  // The upsert target: 020 declares unique (organization_id, job_type_id).
  assert.match(stripSqlComments(read("db/020_booking_experience.sql")), /unique\s*\(organization_id,\s*job_type_id\)/i);
...
test("041 keeps the reference catalogue away from anon", () => {
  assert.match(SQL_041, /alter\s+table\s+public\.industry_pack_services\s+enable\s+row\s+level\s+security/i);
  assert.match(SQL_041, /revoke\s+all\s+on\s+public\.industry_pack_services\s+from\s+anon/i);
  assert.match(SQL_041, /grant\s+select\s+on\s+public\.industry_pack_services\s+to\s+authenticated/i);
