...
import {
  RECOVERABLE_KINDS, KIND_TABLE, RESTORE_ROLES,
  isRecoverableKind, tableForKind,
  restoreBlockers, canRestore, restoreFailureMessage,
...
test("a child cannot be restored while its customer is still deleted", () => {
  for (const kind of ["job", "estimate", "invoice"]) {
    const blockers = restoreBlockers(kind, { deleted: true, customer: { deleted: true, name: "Ada Lovelace" } });
    assert.equal(blockers.length, 1, `${kind} must be blocked`);
    assert.equal(blockers[0].code, "parent_customer_deleted");
    assert.match(blockers[0].message, /Ada Lovelace/, "the refusal must name the customer to restore first");
  }
});
...
test("the same child restores the moment its customer is back", () => {
  ...
  for (const kind of ["job", "estimate", "invoice"]) {
    assert.deepEqual(restoreBlockers(kind, { deleted: true, customer: { deleted: false, name: "Ada Lovelace" } }), []);
...
test("an invoice also waits for its job and its originating estimate", () => {
  ...
  assert.equal(
    restoreBlockers("invoice", { deleted: true, customer: live, job: { deleted: true, name: "Boiler service" } })[0].code,
    "parent_job_deleted",
  );
  assert.equal(
    restoreBlockers("invoice", { deleted: true, customer: live, estimate: { deleted: true, name: "104" } })[0].code,
    "parent_estimate_deleted",
  );
  // Both parents gone: BOTH are reported, so the user is not told one at a time.
  const both = restoreBlockers("invoice", { deleted: true, customer: live, job: { deleted: true }, estimate: { deleted: true } });
  assert.deepEqual(both.map((b) => b.code), ["parent_job_deleted", "parent_estimate_deleted"]);
});
...
test("a dangling job reference blocks, because the KEY is the signal, not the value", () => {
  ...
  const blockers = restoreBlockers("invoice", { deleted: true, customer: { deleted: false }, job: null });
...
test("an exclusion-constraint violation becomes an instruction a dispatcher can follow", () => {
  const result = restoreFailureMessage({ code: "23P01", message: 'conflicting key value violates exclusion constraint "jobs_no_double_book"' });
  assert.equal(result.code, "double_booked");
  assert.match(result.message, /already booked/i);
  assert.doesNotMatch(result.message, /23P01|exclusion constraint/, "raw Postgres is not an explanation");
});
...
test("each trigger refusal maps to its own sentence, and anything unknown fails safe", () => {
  ...
  assert.equal(restoreFailureMessage({ code: "42501", message: "permission denied for table invoices" }).code, "forbidden");
  ...
  assert.doesNotMatch(unknown.message, /connection failure/, "internal errors are not shown verbatim");
});
...
test("the trash screen exists, is owner/office, and pages", () => {
  ...
  assert.match(page, /role === "tech"[\s\S]{0,40}redirect/, "restoring is owner/office, exactly as deleting is");
  assert.match(page, /\.not\("deleted_at", "is", null\)/, "the trash lists deleted rows and only deleted rows");
  assert.match(page, /\.range\(from, to\)/, "an unpaginated list would stop at PostgREST's 1000-row cap");
...
test("trash and archive are kept apart", () => {
  ...
  assert.doesNotMatch(page, /\.eq\("archived", true\)/, "archived=true is the ARCHIVE, not the trash");
...
test("the restore action re-checks the rules and cannot be called by a technician", () => {
  ...
  assert.match(actions, /assertRole\(profile, RESTORE_ROLES/, "role is checked server-side, not in the component");
  assert.match(actions, /restoreBlockers/, "the action must apply the same rules the screen displayed");
  assert.match(actions, /isRecoverableKind\(kind\)/, "the table name comes from a fixed map, never from the caller");
  assert.match(actions, /deleted_at: null/, "restore clears deleted_at");
  assert.match(actions, /\.not\("deleted_at", "is", null\)/, "the write itself is conditional on the row still being deleted");
  assert.match(actions, /\.eq\("organization_id"/, "the write is scoped to the caller's organisation");
  assert.match(actions, /restoreFailureMessage/, "a database refusal must be translated, not leaked");
...
test("migration 037 enforces the same rules in the database, and drops nothing", () => {
  ...
  assert.match(sql, /restore_privacy_erased/, "the privacy-erasure rule must be enforced by the database, not only the UI");
  assert.match(sql, /restore_parent_deleted/, "the parent-first rule must be enforced by the database");
  assert.match(sql, /restore_denied/, "restore must be owner/office at the database too");
  assert.match(sql, /audit_log/, "historical deleted_by must be backfilled, not left blank forever");
  assert.match(sql, /create index if not exists idx_customers_trash/, "every existing index excludes deleted rows");
