...
import {
  CONFIRMATION_STATES, APPOINTMENT_RESPONSES, tokenExpiryFor, tokenState,
  normalizeResponse, canRespond, arrivalState, minutesUntilArrival,
  describeArrival, normalizeEtaMinutes, confirmationSms,
...
test("REVOCATION is checked before expiry", () => {
  ...
  const token = { expires_at: new Date(NOW + 86400_000).toISOString(), revoked_at: new Date(NOW - 60_000).toISOString() };
  assert.deepEqual(tokenState(token, NOW), { valid: false, reason: "revoked" });
});

test("an expired link is refused", () => {
  assert.deepEqual(tokenState({ expires_at: new Date(NOW - 1000).toISOString() }, NOW), { valid: false, reason: "expired" });
});
...
test("a live link is valid — proven the other way", () => {
  assert.deepEqual(tokenState({ expires_at: new Date(NOW + 3600_000).toISOString(), revoked_at: null }, NOW), { valid: true });
});
...
test("a leaked link cannot hammer the row", () => {
  assert.deepEqual(canRespond({ status: "scheduled", customer_response_count: 10 }), { ok: false, error: "too_many_responses" });
...
test("arrival state is derived, so it can never be left stale", () => {
  ...
  assert.equal(arrivalState({ status: "scheduled", on_my_way_at: new Date(NOW - 300_000).toISOString(), eta_minutes: 20 }, NOW), "on_the_way");
  assert.equal(arrivalState({ status: "scheduled", on_my_way_at: new Date(NOW - 300_000).toISOString(), arrived_at: new Date(NOW).toISOString() }, NOW), "arrived");
  assert.equal(arrivalState({ status: "done", completed_at: new Date(NOW).toISOString() }, NOW), "completed");
...
test("an ETA that ran out becomes DUE, never a false 'arrived'", () => {
  ...
  const late = { status: "scheduled", on_my_way_at: new Date(NOW - 3600_000).toISOString(), eta_minutes: 15 };
...
test("minutes until arrival, or null when nothing is known", () => {
  assert.equal(minutesUntilArrival({ on_my_way_at: new Date(NOW - 300_000).toISOString(), eta_minutes: 20 }, NOW), 15);
...
test("the customer sentence names the technician and adapts to the state", () => {
  const onWay = { status: "scheduled", technician: "Ada", on_my_way_at: new Date(NOW - 300_000).toISOString(), eta_minutes: 20 };
...
test("the confirmation SMS carries the link and stays short", () => {
  const body = confirmationSms({ businessName: "Acme", service: "Boiler service", date: "2026-07-02", time: "09:00:00", url: "https://x.test/p/t/visit" });
...
const visitPage = stripSqlComments(readFileSync(new URL("../app/p/[token]/visit/page.tsx", import.meta.url), "utf8"));
const visitClient = stripSqlComments(readFileSync(new URL("../app/p/[token]/visit/VisitClient.tsx", import.meta.url), "utf8"));
...
test("the token table enforces expiry and revocation in its own shape", () => {
  ...
  assert.match(migration, /create unique index if not exists uq_appointment_tokens_live[\s\S]*?where revoked_at is null/);
...
test("public_appointment exposes only this appointment", () => {
  ...
  for (const forbidden of ["price_minor", "total_minor", "public_token", "invoice", "job_address", "customers"]) {
    assert.doesNotMatch(fn[0], new RegExp(forbidden), `public_appointment must not expose ${forbidden}`);
  }
  // First name only.
...
test("responding is bounded, refuses a closed job, and does not cancel it", () => {
  const fn = /create or replace function public\.respond_to_appointment[\s\S]*?\$\$;/.exec(migration);
  assert.ok(fn);
