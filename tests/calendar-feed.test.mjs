...
test("price, notes, phone, email and every public token are STRIPPED", () => {
  ...
  for (const secret of ["45000", "gate code", "secret-token", "+15125550100", "dana@example.com"]) {
    assert.equal(
      ...
      false,
      `${secret} must not reach a passwordless feed`,
    );
  }
});
...
test("a service name containing a comma cannot break the line into a second property", () => {
  const ics = buildCalendar({
    events: [redactEvent({ ...job, service: "Repair, replace; test" })],
    name: "F",
    stampISO: NOW,
  });
...
test("a job with no time becomes an all-day event with an EXCLUSIVE end date", () => {
  const ics = buildCalendar({
    events: [redactEvent({ ...job, start_time: null, end_time: null })],
    name: "F",
    stampISO: NOW,
  });
...
test("a job with a start but no end gets an hour, not a zero-length event", () => {
  const ics = buildCalendar({
    events: [redactEvent({ ...job, end_time: null })],
    name: "F",
    stampISO: NOW,
  });
...
test("a multi-day job spans to its end date", () => {
  const ics = buildCalendar({
    events: [redactEvent({ ...job, end_date: "2026-08-06" })],
    name: "F",
    stampISO: NOW,
  });
...
test("a cancelled job is EXPORTED as cancelled, so it disappears from the subscriber", () => {
  const ics = buildCalendar({
    events: [redactEvent({ ...job, status: "cancelled" })],
    name: "F",
    stampISO: NOW,
  });
...
test("the document is well-formed, CRLF-terminated and balanced", () => {
  const ics = buildCalendar({
    events: [redactEvent(job), redactEvent({ ...job, id: "job-2" })],
    name: "My feed",
    stampISO: NOW,
  });
...
test("a row with no date is skipped rather than emitting a broken VEVENT", () => {
  const ics = buildCalendar({
    events: [{ id: "x", date: "" }, redactEvent(job)],
    name: "F",
    stampISO: NOW,
  });
...
test("the feed is capped, so it can never become a full export of the business", () => {
  const many = Array.from({ length: CALENDAR_MAX_EVENTS + 50 }, (_, i) =>
    redactEvent({ ...job, id: `job-${i}` }),
  );
...
test("migration 040 bounds the feed token: expiry, revocation, scope", () => {
  ...
  assert.match(
    sql,
    /calendar_feed_tokens_scope_check[\s\S]{0,200}check\s*\(\s*scope\s+in\s*\(\s*'mine'\s*,\s*'organization'\s*\)/i,
  );
  ...
  assert.match(
    sql,
    /alter\s+table\s+public\.calendar_feed_tokens\s+enable\s+row\s+level\s+security/i,
  );
