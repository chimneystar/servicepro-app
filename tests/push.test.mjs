...
import { createECDH, createDecipheriv, createHmac, randomBytes, createPublicKey, verify as verifySignature } from "node:crypto";
import {
  vapidStatus, pushUnavailableMessage, checkVapidKeys, publicKeyFromPrivate,
  buildVapidAuthorization, vapidAudience, encryptPushPayload,
  classifyPushResponse, isDelivered, shouldDropSubscription, jobAssignedNotification,
  toB64Url, fromB64Url,
...
function fakeSubscriber() {
  ...
  return {
    ...
    decrypt(body) {
      ...
      const expand = (info, length) => hmac(hmac(salt, ikm), Buffer.concat([Buffer.from(info, "utf8"), Buffer.from([1])])).subarray(0, length);
...
test("an encrypted push payload is decryptable by the subscriber it was sealed for", () => {
  ...
  const message = JSON.stringify({ title: "New job assigned to you", body: "Boiler service · Dana Levi · 2026-08-03 09:00", url: "/jobs/abc" });
  const body = encryptPushPayload({ payload: message, p256dh: subscriber.p256dh, auth: subscriber.auth });
...
test("the push body carries the aes128gcm header the push service requires", () => {
  ...
  const body = encryptPushPayload({ payload: "{}", p256dh: subscriber.p256dh, auth: subscriber.auth, salt });
...
test("a payload sealed for one subscriber CANNOT be opened by another", () => {
  ...
  const body = encryptPushPayload({ payload: "secret job details", p256dh: intended.p256dh, auth: intended.auth });
  assert.throws(() => other.decrypt(body));
});

test("a wrong auth secret fails authentication rather than yielding plaintext", () => {
  const subscriber = fakeSubscriber();
  const body = encryptPushPayload({ payload: "secret", p256dh: subscriber.p256dh, auth: toB64Url(randomBytes(16)) });
  assert.throws(() => subscriber.decrypt(body));
});

test("malformed subscription keys are refused, not silently sent to", () => {
  const subscriber = fakeSubscriber();
  assert.throws(() => encryptPushPayload({ payload: "x", p256dh: "not-a-key", auth: subscriber.auth }), /invalid_p256dh/);
  assert.throws(() => encryptPushPayload({ payload: "x", p256dh: subscriber.p256dh, auth: toB64Url(randomBytes(8)) }), /invalid_auth_secret/);
  assert.throws(() => encryptPushPayload({ payload: "x".repeat(5000), p256dh: subscriber.p256dh, auth: subscriber.auth }), /payload_too_large/);
});
...
test("the VAPID assertion verifies against the advertised public key", () => {
  ...
  const { authorization, jwt, aud, exp } = buildVapidAuthorization({
    ...
    subject: "mailto:ops@example.com", publicKey, privateKey, now: 1_800_000_000_000,
  });
  ...
  const key = createPublicKey({ format: "jwk", key: { kty: "EC", crv: "P-256", x: toB64Url(raw.subarray(1, 33)), y: toB64Url(raw.subarray(33, 65)) } });
  assert.equal(
    verifySignature("sha256", Buffer.from(`${header}.${payload}`, "utf8"), { key, dsaEncoding: "ieee-p1363" }, fromB64Url(signature)),
    true,
  );
});
...
test("a VAPID assertion signed by the WRONG key does not verify", () => {
  ...
  const { jwt } = buildVapidAuthorization({ endpoint: "https://push.example.com/x", subject: "mailto:a@b.c", publicKey: mine.publicKey, privateKey: mine.privateKey });
  const [header, payload, signature] = jwt.split(".");
...
test("the audience is the push service origin, and plaintext endpoints are refused", () => {
  assert.equal(vapidAudience("https://updates.push.services.mozilla.com/wpush/v2/abc"), "https://updates.push.services.mozilla.com");
...
test("push reports itself UNAVAILABLE with a reason when keys are absent or broken", () => {
  ...
  assert.equal(vapidStatus({ NEXT_PUBLIC_VAPID_PUBLIC_KEY: publicKey }).reason, "missing_private_key");
  assert.equal(vapidStatus({ NEXT_PUBLIC_VAPID_PUBLIC_KEY: publicKey, VAPID_PRIVATE_KEY: privateKey }).reason, "missing_subject");
  assert.equal(vapidStatus({ NEXT_PUBLIC_VAPID_PUBLIC_KEY: publicKey, VAPID_PRIVATE_KEY: privateKey, VAPID_SUBJECT: "ops@example.com" }).reason, "invalid_subject");
  assert.equal(vapidStatus({ NEXT_PUBLIC_VAPID_PUBLIC_KEY: vapidPair().publicKey, VAPID_PRIVATE_KEY: privateKey, VAPID_SUBJECT: "mailto:a@b.c" }).reason, "key_pair_mismatch");
  ...
  for (const reason of ["not_configured", "missing_public_key", "missing_private_key", "missing_subject", "invalid_subject", "key_pair_mismatch", "no_devices"]) {
...
test("push reports itself AVAILABLE on a correct configuration", () => {
  const { publicKey, privateKey } = vapidPair();
  const status = vapidStatus({ NEXT_PUBLIC_VAPID_PUBLIC_KEY: publicKey, VAPID_PRIVATE_KEY: privateKey, VAPID_SUBJECT: "mailto:ops@example.com" });
...
test("the assignment notification says what, for whom and when, in the device's language", () => {
  const en = jobAssignedNotification({ locale: "en", service: "Boiler service", customerName: "Dana Levi", scheduledDate: "2026-08-03", startTime: "09:00:00", jobId: "job-1" });
  ...
  const he = jobAssignedNotification({ locale: "he", service: "טיפול בדוד", customerName: "דנה לוי", scheduledDate: "2026-08-03", startTime: "09:00:00", jobId: "job-1" });
...
test("the sender removes subscriptions the push service says are gone", () => {
  ...
  assert.match(sender, /device_subscriptions"\)\.delete\(\)/);
...
test("VAPID secrets are placeholders in .env.example and never real values", () => {
  ...
  for (const name of ["NEXT_PUBLIC_VAPID_PUBLIC_KEY", "VAPID_PRIVATE_KEY", "VAPID_SUBJECT"]) {
    assert.match(example, new RegExp(`^${name}=\\s*$`, "m"), `${name} must be present and EMPTY in .env.example`);
  }
});
