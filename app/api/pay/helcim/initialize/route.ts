...
export async function POST(request: NextRequest) {
  try {
    const body = await request.json() as { token?: unknown; tipPercent?: unknown; tipAmount?: unknown };
    ...
    const tipPercent = typeof body.tipPercent === "number" ? body.tipPercent
      : typeof body.tipPercent === "string" && body.tipPercent.trim() !== "" ? Number.parseInt(body.tipPercent, 10)
      : null;
    let tipCustomMinor: number | null = null;
    if (typeof body.tipAmount === "string" && body.tipAmount.trim() !== "") {
      try { tipCustomMinor = parseAmountToMinor(body.tipAmount); }
      catch { throw new PaymentError("Enter the tip as an amount, for example 10.00", "invalid_tip", 400); }
    }
    ...
    return NextResponse.json({ ok: true, ...(await startHelcimCheckout(token, { percent: tipPercent, customMinor: tipCustomMinor })) });
  } catch (error) {
