...
export async function POST(request: Request, { params }: { params: Promise<{ org: string }> }) {
  ...
  const name = clean(body.name, 120),
    ...
    serviceId = clean(body.serviceId, 60),
    date = clean(body.date, 10),
    start = clean(body.start, 5);
  const address = clean(body.address, 200),
    city = clean(body.city, 80),
    postalCode = clean(body.postalCode, 20),
    notes = clean(body.notes, 2000),
    source = clean(body.source, 80) || "Online booking",
    campaign = clean(body.campaign, 120),
    contactPreference = clean(body.contactPreference, 20) || "phone",
    urgency = clean(body.urgency, 20) || "standard";
  if (
    !name ||
    !phone ||
    !serviceId ||
  ...
  try {
    ...
    if ((recent ?? 0) >= 25)
      return NextResponse.json({ ok: false, error: "try_later" }, { status: 429 });
    ...
    if (areaUnverified)
      console.warn(
        JSON.stringify({
          event: "booking.service_area_unevaluable",
          organizationId: org,
          reason: "polygon_only_areas_cannot_be_evaluated_without_geocoding",
        }),
      );
    ...
    const available = buildBookingSlots({
      date,
      hours: settings.hours_json as BookingHours,
      intervalMin: settings.slot_interval_min,
      durationMin: service.duration_min,
      arrivalWindowMin: settings.arrival_window_min,
    ...
    const bookingAnswers = {
      ...(areaUnverified ? { ...answers, service_area_unverified: true } : answers),
      ...(depositDue
    ...
    const leadPayload = {
      ...
      preferred_window_min: settings.arrival_window_min,
      booking_service_id: service.id,
      booking_answers: bookingAnswers,
      booking_reference: reference,
      booking_status: needsReview || depositDue ? "requested" : "confirmed",
    ...
    if (depositDue) {
      ...
      const raised = await raiseBookingDeposit(admin, {
        ...
        depositValue: settings.deposit_value,
        taxRateBps: Number(organization?.tax_rate_bps ?? 0),
        notes: notes || null,
      });
      if (raised) {
        deposit = { amountMinor: raised.amountMinor, url: `/p/${raised.publicToken}` };
        status = "deposit_due";
      }
    } else if (autoRelease) {
      const customer = await resolveCustomer();
      ...
      const { error: jobError } = await admin.from("jobs").insert({
        ...
        end_date: date,
        start_time: start,
        end_time: addMinutes(start, service.duration_min),
        source,
        notes: notes || null,
      });
      ...
      await admin
        .from("leads")
        .update({ status: "won", converted_customer_id: customer.id, booking_status: "confirmed" })
        .eq("id", lead.id);
      status = "confirmed";
    }
    return NextResponse.json(
