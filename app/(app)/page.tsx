...
export default async function DashboardPage() {
  ...
  const [
    ...
    supabase
      .from("organizations")
      .select("currency, logo_url, tax_rate_bps, review_url, onboarding_dismissed")
      .single(),
    supabase
      .from("invoices")
    ...
    supabase
      .from("expenses")
      .select("amount_minor, expense_date")
    ...
    supabase
      .from("jobs")
      .select(
        "id, assigned_to, service, source, status, price_minor, scheduled_date, start_time, customer_id, customers!jobs_customer_org_fk(name)",
      )
      .is("deleted_at", null)
  ...
  const steps: Step[] = [
    {
      label: he ? "הוספת לוגו לעסק" : "Add your business logo",
      done: !!org?.logo_url,
      href: "/settings",
    },
    {
      label: he ? "הגדרת שיעור המס" : "Set your sales-tax rate",
      done: (org?.tax_rate_bps ?? 0) > 0,
      href: "/settings",
    },
    {
      label: he ? "הוספת הלקוח הראשון" : "Add your first customer",
      done: (custCount ?? 0) > 0,
      href: "/customers",
    },
    {
      label: he ? "יצירת הצעת המחיר הראשונה" : "Create your first estimate",
      done: (estimateCount ?? 0) > 0,
      href: "/estimates",
    },
    {
      label: he ? "שיבוץ העבודה הראשונה" : "Schedule your first job",
      done: (jobCount ?? 0) > 0,
      href: "/schedule",
    },
    {
      label: he ? "הוספת קישור לביקורת" : "Add your review link",
      done: !!org?.review_url,
      href: "/settings",
    },
  ];
  const showChecklist =
    profile.role === "owner" && !org?.onboarding_dismissed && steps.some((s) => !s.done);
  ...
  const truncated: boolean =
    isTruncated(inv.length, ROW_CEILING) ||
    isTruncated(est.length, ROW_CEILING) ||
    isTruncated(jb.length, JOB_CEILING);

  const estBy = (st: string) => est.filter((e) => e.status === st);
  const wonN = estBy("approved").length,
    lostN = estBy("rejected").length;
  ...
  const series = Array.from({ length: 6 }, (_, k) => {
    ...
    return {
      ...
      value: Math.round(collectedMinor(inMonth) / 100),
    };
  });
  const collectRate =
    collected12 + dueSum > 0 ? Math.round((collected12 / (collected12 + dueSum)) * 100) : 0;
  const todayJobs = jb
    .filter((j) => j.scheduled_date === today)
    .sort((a, b) => (a.start_time ?? "").localeCompare(b.start_time ?? ""));
  const upcoming = jb
    .filter((j) => j.scheduled_date >= today && j.status === "scheduled")
    .sort((a, b) =>
  ...
  jb.forEach((j) => {
    byType[j.service] = (byType[j.service] || 0) + 1;
  });
  ...
  jb.forEach((j) => {
    if (j.source) bySrc[j.source] = (bySrc[j.source] || 0) + 1;
  });
  ...
  const focus =
    profile.role === "owner"
      ? {
          eyebrow: he ? "ממתין לגבייה" : "Waiting to be collected",
          title: money(dueSum, cur),
          copy: he
            ? `${unpaid.length} חשבוניות פתוחות, מהן ${pastDue.length} באיחור.`
            : `${unpaid.length} open invoices, including ${pastDue.length} past due.`,
          href: "/invoices?filter=unpaid",
          action: he ? "לחשבוניות הפתוחות" : "Open invoices",
        }
      : profile.role === "office"
        ? {
            eyebrow: he ? "צריך לשבץ" : "Needs dispatch",
            title: he
              ? `${unassigned.length} עבודות ללא שיבוץ`
              : `${unassigned.length} unassigned jobs`,
            copy: he
              ? "פותחים את לוח השיבוץ ומחברים את העבודה לאדם הנכון."
              : "Open dispatch and put the right person on each job.",
            href: "/dispatch",
            action: he ? "ללוח השיבוץ" : "Open dispatch",
          }
        : {
            eyebrow: he ? "העבודה הבאה" : "Your next stop",
            title: nextAssigned
              ? `${(nextAssigned.start_time ?? "").slice(0, 5) || "—"} · ${nextAssigned.customers?.name ?? nextAssigned.service}`
              : he
                ? "אין עבודה נוספת"
                : "No next job",
            copy: nextAssigned
              ...
              : he
                ? "אפשר לבדוק שהסנכרון מעודכן או לפנות למשרד."
                : "Check sync status or contact the office if you expect an assignment.",
            href: nextAssigned ? `/jobs/${nextAssigned.id}` : "/tech",
            action: nextAssigned
              ? he
                ? "לפרטי העבודה"
                : "Open job"
  ...
  const attention = [
    ...(pastDue.length
      ? [
          {
            tone: "danger",
            title: he ? `${pastDue.length} חשבוניות באיחור` : `${pastDue.length} past-due invoices`,
            copy: money(pastDueSum, cur),
            href: "/invoices?filter=unpaid",
          },
        ]
      : []),
      ...
      ? [
          {
            ...
            title: he
              ? `${unassigned.length} עבודות ללא שיבוץ`
              : `${unassigned.length} jobs need dispatch`,
            copy: he
              ? "העבודה הקרובה מחכה לאיש צוות"
              : "Upcoming work is waiting for a team member",
            href: "/dispatch",
          },
        ]
      : []),
    ...(newLeadsTotal
      ? [
          {
            tone: "blue",
            title: he ? `${newLeadsTotal} לידים חדשים` : `${newLeadsTotal} new leads`,
            copy: he ? "כדאי לחזור אליהם לפני שימשיכו הלאה" : "Respond before they move on",
            href: "/leads",
          },
        ]
      : []),
      ...
      ? [
          {
            tone: "blue",
            title: he
              ? `${waitingEstimates.length} הצעות מחכות לתשובה`
              : `${waitingEstimates.length} estimates await a decision`,
            copy: he ? "אפשר לקבוע מעקב מהיר" : "Schedule a clear follow-up",
            href: "/growth",
          },
        ]
      : []),
    ...
    <div className="dashboard-experience">
      <section className="dashboard-hero">
        <div>
          ...
          <h1>{t(locale, "dash.greeting", { name: profile.full_name || "👋" })}</h1>
          <p>
            {profile.role === "owner"
              ? he
                ? "העסק מול העיניים. מתחילים במה שהכי חשוב."
                : "Start with what needs your attention, then review the numbers."
              : profile.role === "office"
                ? he
                  ? "השיבוץ והלקוחות מסודרים לפי מה שצריך לקרות עכשיו."
                  : "Dispatch and customer work are ordered by what happens next."
                : he
                  ? "כל מה שצריך לעבודה הבאה נמצא כאן."
                  : "Everything for your next stop is ready here."}
          </p>
        </div>
      ...
      >
        {[
          ["/schedule", he ? "עבודה חדשה" : "New job"],
          ["/estimates", he ? "הצעות מחיר" : "Estimates"],
          ["/invoices", he ? "חשבוניות" : "Invoices"],
          ["/leads", he ? "לידים" : "Leads"],
          ["/route", he ? "המסלול של היום" : "Today’s route"],
          ["/messages", he ? "הודעות" : "Messages"],
          ["/reports", he ? "דוחות" : "Reports"],
        ].map(([href, label]) => (
          <Link
            key={href}
            href={href}
...
function JobPulse({ jobs, he }: { jobs: PulseJob[]; he: boolean }) {
  return (
    <article className="job-pulse">
      <header>
        <div>
          <span>{he ? "מה קורה היום" : "Today’s job pulse"}</span>
          <h2>{he ? "מה הסתיים, מה הבא ומה מחכה" : "Done, next, and coming up"}</h2>
        </div>
        <Link href="/schedule">{he ? "לכל היומן" : "Full schedule"}</Link>
      </header>
      <div className="job-pulse-track">
        {jobs.length ? (
          jobs.slice(0, 7).map((job, index) => {
            const done = job.status === "done",
              active =
                !done &&
                index ===
                  jobs.findIndex((row) => row.status !== "done" && row.status !== "cancelled");
            return (
              <Link
                href={`/jobs/${job.id}`}
                key={job.id}
              ...
              >
                <i>{done ? "✓" : index + 1}</i>
                <b>{(job.start_time ?? "").slice(0, 5) || "—"}</b>
                <strong>{job.customers?.name ?? job.service}</strong>
          ...
          <div className="job-pulse-empty">
            {he
              ? "אין עבודות היום. אפשר לפתוח את היומן ולהוסיף עבודה."
              : "No jobs today. Open the schedule to add one."}
          </div>
        )}
      </div>
    </article>
...
function AttentionQueue({
  ...
  rows: { tone: string; title: string; copy: string; href: string }[];
  he: boolean;
}) {
  return (
    <aside className="dashboard-attention">
      <header>
        <div>
          <span>{he ? "צריך טיפול" : "Needs attention"}</span>
          <h2>{he ? "מה לא כדאי לפספס" : "Don’t let these wait"}</h2>
        </div>
        <b>{rows.length}</b>
      </header>
      {rows.length ? (
        rows.map((row, index) => (
          <Link href={row.href} key={`${row.href}-${index}`}>
        ...
        <div className="dashboard-all-clear">
          <span>✓</span>
          {he ? "אין כרגע דברים דחופים." : "Nothing urgent right now."}
        </div>
      )}
    </aside>
  );
...
function Big({ children, small }: { children: React.ReactNode; small?: boolean }) {
  return (
    <div
      style={{ fontSize: small ? "1.375rem" : "1.75rem", fontWeight: 800, letterSpacing: "-.3px" }}
    >
      {children}
    </div>
  );
...
function Row({ label, value, strong }: { label: string; value: string; strong?: boolean }) {
  return (
    ...
    >
      <span style={{ color: "#5c6675" }}>{label}</span>
      <b style={{ color: strong ? "#15803d" : "#0b1524" }}>{value}</b>
    </div>
  );
}
...
function Td({ children }: { children: React.ReactNode }) {
  return <td style={{ padding: "9px 6px", textAlign: "start" }}>{children}</td>;
}
function statusChip(s: string): React.CSSProperties {
  const map: Record<string, string> = {
    scheduled: "#e0ebff|#2563eb",
    in_progress: "#fdf1dc|#b45309",
    done: "#e6f6ec|#15803d",
