...
import { CALENDAR_TOKEN_TTL_DAYS, calendarTokenExpiry, canCreateFeed } from "@/lib/core/calendar.mjs";
 ...
 */
export async function createCalendarFeed(_prev: ActionResult, formData: FormData): Promise<ActionResult> {
  ...
  if (!permitted.ok) {
    return {
      ok: false,
      error: permitted.reason === "scope_not_permitted"
  ...
  const { count } = await supabase.from("calendar_feed_tokens")
    .select("id", { count: "exact", head: true })
    .eq("profile_id", profile.id).is("revoked_at", null);
  if ((count ?? 0) >= 5) {
    return { ok: false, error: "You already have 5 live calendar feeds. Revoke one before creating another." };
  }
...
export async function revokeCalendarFeed(id: string): Promise<ActionResult> {
  const profile = await requireProfile();
  const supabase = await createClient();
  const { error } = await supabase.from("calendar_feed_tokens")
    .update({ revoked_at: new Date().toISOString(), revoked_by: profile.id })
    .eq("id", id).eq("profile_id", profile.id).is("revoked_at", null);
  if (error) return { ok: false, error: error.message };
  revalidatePath("/reports/calendar");
  return { ok: true };
}
...
export async function rotateCalendarFeed(id: string): Promise<ActionResult> {
  const profile = await requireProfile();
  const supabase = await createClient();
  const { data: existing } = await supabase.from("calendar_feed_tokens")
    .select("id, scope, label").eq("id", id).eq("profile_id", profile.id).is("revoked_at", null).maybeSingle();
  ...
  if (error) return { ok: false, error: `The old link was revoked but the new one could not be created: ${error.message}` };
