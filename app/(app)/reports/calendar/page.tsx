...
export default async function CalendarFeedPage() {
  ...
  return (
    <div style={{ maxWidth: 760 }}>
      ...
      >
        <b>Treat this URL as a password.</b> Anyone who has it can read the schedule it covers, with
        no login, until it expires or you revoke it. It expires automatically after{" "}
        <b>{CALENDAR_TOKEN_TTL_DAYS} days</b> — rotate it to get a new one. The feed carries the
        service, the time, the customer&rsquo;s name and the address only:{" "}
        <b>no prices, no notes, and no payment links</b>, and it covers {CALENDAR_WINDOW_PAST_DAYS}{" "}
        days back to {CALENDAR_WINDOW_FUTURE_DAYS} days ahead.
      </div>
      ...
      {!origin && (
        ...
        >
          NEXT_PUBLIC_APP_URL is not configured, so the full feed URL cannot be shown. Set it and
          reload.
        </div>
      )}

      <ActionForm
        action={createCalendarFeed}
        successLabel="Calendar feed created"
        className="ops-form"
      >
        <div
          style={{
        ...
        >
          <input
            name="label"
            placeholder="Name this feed (optional)"
            aria-label="Name this feed (optional)"
            maxLength={80}
            style={input}
          />
      ...
      {live.map((feed: Feed) => (
        <FeedRow key={feed.id} feed={feed} origin={origin} daysLeft={daysLeft(feed)} />
      ))}
