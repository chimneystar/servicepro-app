...
export default async function CustomReportPage({
  ...
  searchParams: Promise<{ from?: string; to?: string; sec?: string | string[] }>;
}) {
  ...
  const selected = new Set(
    secParam == null ? ALL.map((s) => s[0]) : Array.isArray(secParam) ? secParam : [secParam],
  );
  ...
  const revByInv: Record<string, number> = {},
    costByInv: Record<string, number> = {};
  items.forEach((it) => {
    revByInv[it.invoice_id] = (revByInv[it.invoice_id] || 0) + itemRev(it);
    costByInv[it.invoice_id] = (costByInv[it.invoice_id] || 0) + itemCost(it);
  });
  const byTech: Record<string, { collected: number; profit: number; count: number }> = {};
  invs.forEach((i: any) => {
    const n = i.jobs?.profiles?.full_name || "Unassigned";
    const b = byTech[n] || { collected: 0, profit: 0, count: 0 };
  ...
  return (
    <div style={{ maxWidth: 780 }}>
      ...
      {/* Report */}
      <div
        style={{ background: "#fff", border: "1px solid #e2e8f0", borderRadius: 14, padding: 22 }}
      ...
      >
        ...
        {chk("sales") && (
          <Section title="Sales summary">
            ...
            <KV
              ...
              value={money(invs.length ? Math.round(revenue / invs.length) : 0, cur)}
            />
          </Section>
        )}
            ...
            {Object.entries(byTech)
              ...
              .map(([n, b]) => (
                <KV
                  key={n}
                  label={`${n} (${b.count})`}
                  value={`${money(b.collected, cur)} · ${money(b.profit, cur)} profit`}
                />
              ))}
        ...
        {chk("aging") && (
          <Section title="Aging — unpaid invoices">
            {aging.map((b) => (
              <KV key={b.label} label={`${b.label} (${b.count})`} value={money(b.total, cur)} />
            ))}
          </Section>
        )}
        ...
        {chk("expenses") && (
          <Section title="Expenses by category">
            {Object.entries(byCat)
              .sort((a, b) => b[1] - a[1])
              .map(([c, v]) => (
                <KV key={c} label={c} value={money(v, cur)} />
              ))}
        ...
        {selected.size === 0 && (
          <div style={{ textAlign: "center", color: "#5c6675", padding: 20 }}>
