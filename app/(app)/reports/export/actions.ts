...
export async function exportCsv(
  ...
  try {
    if (kind === "invoices") {
      const data = await fetchAllPages<any>((a, b) =>
        supabase
          .from("invoices")
          .select(
            "number, issue_date, status, total_minor, tax_rate_bps, discount_minor, customers!invoices_customer_org_fk(name)",
          )
          .is("deleted_at", null)
          .gte("issue_date", from)
          .lte("issue_date", to)
      ...
      return {
        ...
        csv: toCsv(["Invoice #", "Date", "Customer", "Status", "Total"], rows),
      };
    }
    ...
    if (kind === "payments") {
      ...
      const data = await fetchAllPages<any>((a, b) =>
        supabase
          .from("payments")
          .select(
            "amount_minor, base_amount_minor, refunded_minor, normalized_status, method, reference, paid_at, invoices(number, customers(name))",
          )
      ...
      return {
        ...
        csv: toCsv(
          ["Date", "Invoice #", "Customer", "Method", "Reference", "Amount", "Refunded", "Status"],
          rows,
        ),
      };
    }
    ...
    return {
      ok: true,
      filename: `expenses_${from}_${to}.csv`,
      csv: toCsv(["Date", "Category", "Vendor", "Amount", "Notes"], rows),
    };
  } catch (error: unknown) {
    ...
    return {
      ok: false,
      error: message === "export_too_large" ? "export_too_large" : "export_failed",
    };
  }
}
...
export async function exportForAccounting(
  ...
  try {
    ...
    if (kind === "invoices") {
      const invoices = await fetchAllPages<any>((a, b) =>
        supabase
          .from("invoices")
          .select(
            "id, number, issue_date, total_minor, discount_minor, tax_rate_bps, customers!invoices_customer_org_fk(name)",
          )
          .is("deleted_at", null)
          .gte("issue_date", from)
          .lte("issue_date", to)
      ...
      for (const invoice of invoices) {
        ...
        mapped.push(
          mapRow(target, "invoices", {
            ...
            qty_milli: 1000,
            unit_price_minor: exTax,
            line_total_minor: exTax,
            tax_minor: taxMinor,
        ...
        ledger.push({
          sourceType: "invoice",
          sourceId: invoice.id,
          ref: externalRef("invoice", invoice.id) as string,
          amountMinor: Number(invoice.total_minor ?? 0),
        });
      }
    } else if (kind === "payments") {
      const payments = await fetchAllPages<any>((a, b) =>
        supabase
          .from("payments")
          .select(
            "id, amount_minor, base_amount_minor, refunded_minor, method, paid_at, invoices(number, customers(name))",
          )
          .gte("paid_at", `${from}T00:00:00`)
          .lte("paid_at", `${to}T23:59:59`)
          .order("paid_at")
      ...
      for (const payment of payments) {
        const net =
          Number(payment.base_amount_minor ?? payment.amount_minor ?? 0) -
          Number(payment.refunded_minor ?? 0);
        mapped.push(
          mapRow(target, "payments", {
            id: payment.id,
            customer_name: payment.invoices?.customers?.name ?? "",
            invoice_number: payment.invoices?.number ?? "",
            paid_at: payment.paid_at,
            method: payment.method ?? "",
            base_amount_minor: payment.base_amount_minor ?? payment.amount_minor,
        ...
        ledger.push({
          sourceType: "payment",
          sourceId: payment.id,
          ref: externalRef("payment", payment.id) as string,
          amountMinor: net,
        });
      }
    } else {
      const expenses = await fetchAllPages<any>((a, b) =>
        supabase
          .from("expenses")
      ...
      for (const expense of expenses) {
        ...
        ledger.push({
          sourceType: "expense",
          sourceId: expense.id,
          ref: externalRef("expense", expense.id) as string,
    ...
    const { data: batch, error: batchError } = await supabase
      ...
      .maybeSingle();
    if (batchError)
      console.error(`[export] could not record the ${target} batch: ${batchError.message}`);

    if (ledger.length) {
      const { error: rowsError } = await supabase.from("accounting_export_rows").upsert(
        ledger.map((row) => ({
          organization_id: profile.organization_id,
          export_id: batch?.id ?? null,
          target,
      ...
      if (rowsError) {
        ...
        console.error(
          `[export] could not record ${target} export references: ${rowsError.message}`,
        );
        return {
          ok: false,
          error: `The file was built but its references could not be recorded, so it cannot be reconciled later: ${rowsError.message}`,
        };
      }
    }
...
export async function reconcileAgainstLedger(
  ...
  if (text.length > 4_000_000)
    return { ok: false, error: "That file is too large to reconcile in one pass." };
  ...
  const refIndex = header.findIndex((cell) =>
    /(reference|privatenote|memo|description|ref)/.test(cell),
  );
  const amountIndex = header.findIndex((cell) => /(amount|total|debit|credit)/.test(cell));
  if (refIndex < 0 || amountIndex < 0) {
    return {
      ok: false,
      error: "Could not find a reference column and an amount column in that file.",
    };
  }
  ...
  for (const line of lines.slice(1)) {
    ...
    const amount = Math.round(
      Number(String(cells[amountIndex] ?? "0").replace(/[^0-9.\-]/g, "")) * 100,
    );
  ...
  const local = await fetchAllPages<{ external_ref: string; amount_minor: number }>((a, b) =>
    supabase
      .from("accounting_export_rows")
      .select("external_ref, amount_minor")
      .eq("target", target)
      .range(a, b),
  );
