...
export default function AccountingSync({
  ...
  status: { status: string; built: readonly string[]; remaining: readonly string[]; reason: string };
}) {
  const now = new Date();
  ...
  const [to, setTo] = useState(new Date(now.getFullYear(), now.getMonth(), 0).toISOString().slice(0, 10));
  ...
  const download = (kind: "invoices" | "payments" | "expenses") => {
    ...
    start(async () => {
      ...
      if (!r.ok || !r.csv) { setMessage({ ok: false, text: r.error ?? "The export failed." }); return; }
      const blob = new Blob([r.csv], { type: "text/csv;charset=utf-8;" });
      const url = URL.createObjectURL(blob);
      const a = document.createElement("a"); a.href = url; a.download = r.filename ?? "export.csv"; a.click();
      URL.revokeObjectURL(url);
      setMessage({ ok: true, text: `✓ ${r.rows} row${r.rows === 1 ? "" : "s"} (${money(r.totalMinor)}) — every row carries its SP- reference, so a re-import updates rather than duplicates.` });
    });
  };
  ...
  const check = () => {
    ...
    start(async () => {
      ...
      if (!r.ok) { setMessage({ ok: false, text: r.error ?? "The reconciliation failed." }); return; }
      setMessage(null);
      setResult(r);
    });
  };
    ...
    <div style={{ maxWidth: 620, marginTop: 22 }}>
      ...
      <div role="note" style={{ background: "#fff7ed", border: "1px solid #fcd9a8", color: "#9a3412", borderRadius: 12, padding: "12px 14px", fontSize: "0.8125rem", marginBottom: 14 }}>
        ...
        <ul style={{ margin: "4px 0 0", paddingInlineStart: 18 }}>
          {status.built.map((line) => <li key={line}>{line}</li>)}
        </ul>
        <div style={{ marginTop: 8, fontWeight: 700 }}>What is still missing:</div>
        <ul style={{ margin: "4px 0 0", paddingInlineStart: 18 }}>
          {status.remaining.map((line) => <li key={line}>{line}</li>)}
        </ul>
      </div>
      ...
      <div style={{ background: "#fff", border: "1px solid #e2e8f0", borderRadius: 14, padding: 18 }}>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(150px,1fr))", gap: 10, marginBottom: 14 }}>
          <div>
            <label style={lbl} htmlFor="acct-target">Ledger</label>
            <select id="acct-target" value={target} onChange={(e) => setTarget(e.target.value as "quickbooks" | "xero")} style={inp}>
              <option value="quickbooks">QuickBooks Online</option>
              <option value="xero">Xero</option>
            </select>
          </div>
          <div><label style={lbl} htmlFor="acct-from">From</label><input id="acct-from" type="date" value={from} onChange={(e) => setFrom(e.target.value)} style={inp} /></div>
          <div><label style={lbl} htmlFor="acct-to">To</label><input id="acct-to" type="date" value={to} onChange={(e) => setTo(e.target.value)} style={inp} /></div>
        </div>
        <div style={{ display: "grid", gap: 8 }}>
          <button type="button" onClick={() => download("invoices")} disabled={pending} style={btn}>⬇ Invoices for {target === "xero" ? "Xero" : "QuickBooks"}</button>
          <button type="button" onClick={() => download("payments")} disabled={pending} style={btn}>⬇ Payments for {target === "xero" ? "Xero" : "QuickBooks"}</button>
      ...
      {result && (
        ...
        >
          ...
          {(result.amountMismatch?.length ?? 0) > 0 && (
            <>
              <div style={{ marginTop: 8, fontWeight: 700 }}>Same row, different money ({result.amountMismatch!.length}):</div>
              <ul style={list}>
                {result.amountMismatch!.map((row) => (
                  <li key={row.ref}>{row.ref}: here {money(row.localMinor)}, ledger {money(row.remoteMinor)} (off by {money(row.differenceMinor)})</li>
                ))}
              </ul>
            </>
          )}
          {(result.missingRemote?.length ?? 0) > 0 && (
            <>
              <div style={{ marginTop: 8, fontWeight: 700 }}>Sent, but not in the ledger ({result.missingRemote!.length}):</div>
              <ul style={list}>{result.missingRemote!.map((row) => <li key={row.ref}>{row.ref}: {money(row.amountMinor)}</li>)}</ul>
            </>
          )}
          {(result.missingLocal?.length ?? 0) > 0 && (
            <>
              <div style={{ marginTop: 8, fontWeight: 700 }}>In the ledger, but not here ({result.missingLocal!.length}):</div>
              <ul style={list}>{result.missingLocal!.map((row) => <li key={row.ref}>{row.ref}: {money(row.amountMinor)}</li>)}</ul>
            </>
          )}
        </div>
      )}
