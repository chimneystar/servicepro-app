...
export default function ExportClient() {
  ...
  function download(kind: "invoices" | "payments" | "expenses") {
    ...
    start(async () => {
      ...
      if (!r.ok || !r.csv) { setMsg(r.error ?? "Export failed"); return; }
  ...
  return (
    <div style={{ maxWidth: 560 }}>
      <div style={{ background: "#fff", border: "1px solid #e2e8f0", borderRadius: 14, padding: 18, boxShadow: "0 6px 18px rgba(15,42,94,.06)" }}>
        ...
        {msg && <div style={{ marginTop: 12, color: msg.startsWith("✓") ? "#15803d" : "#dc2626", fontWeight: 700, fontSize: 14 }}>{msg}</div>}
      </div>
      <p style={{ color: "#5c6675", fontSize: 14, marginTop: 12 }}>These CSV files import cleanly into QuickBooks, Xero, Wave, or a spreadsheet for your bookkeeper.</p>
    </div>
  );
}
