...
export default async function TimesheetsPage({
  ...
  const rows: TSRow[] = entries.map((e: any) => {
    ...
    return {
      tech: e.profiles?.full_name || "Unassigned",
      date: (e.started_at ?? "").slice(0, 10),
      service: e.jobs?.service || "—",
      hours: hrs.toFixed(2),
    };
  });
  const byTech: Record<string, number> = {};
  rows.forEach((r) => {
    byTech[r.tech] = (byTech[r.tech] || 0) + parseFloat(r.hours);
  });
  ...
  return (
    <div style={{ maxWidth: 720 }}>
      ...
      <div style={{ fontWeight: 800, fontSize: "0.9375rem", margin: "4px 0 8px" }}>
        Hours by technician
      </div>
      <div className="rlist">
        {totals.map(([tech, hrs]) => (
          <div className="ritem" key={tech}>
            <div className="rmain">
              <div className="rtitle">{tech}</div>
            </div>
            <div className="rend">
              <b>{hrs.toFixed(2)} h</b>
            </div>
        ...
        {totals.length === 0 && (
          <div className="rempty">No clock-in time recorded in this period.</div>
        )}
      </div>
    </div>
  );
