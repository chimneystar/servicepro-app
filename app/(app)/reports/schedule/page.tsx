...
export default async function ReportSchedulePage() {
  ...
  const [{ data: schedules }, { data: members }, { data: recent }] = await Promise.all([
    supabase.from("report_schedules")
      .select("id, name, frequency, enabled, recipient_profile_ids, last_period_key, last_run_at, last_error")
      .order("created_at", { ascending: false }),
    supabase.from("profiles")
      .select("id, full_name, role, active, notify_email_opt_in")
      .eq("active", true).order("full_name"),
    supabase.from("report_deliveries")
  ...
  return (
    <div style={{ maxWidth: 760 }}>
      ...
      <h1 style={{ fontSize: 24, fontWeight: 800, margin: "8px 0 4px" }}>Emailed reports</h1>
      ...
      {!providers.email() && (
        <div role="alert" style={{ background: "#fdeaea", border: "1px solid #f5b5b5", color: "#b91c1c", borderRadius: 12, padding: "11px 14px", fontSize: 13, marginBottom: 16 }}>
          <b>No email provider is connected</b>, so nothing will be delivered. Schedules stay due rather than
          being marked sent, and they will go out the day Resend is configured.
        </div>
      )}
      ...
      <ActionForm action={createReportSchedule} successLabel="Report scheduled" className="ops-form">
        <div style={{ display: "grid", gap: 10, background: "#fff", border: "1px solid #e2e8f0", borderRadius: 12, padding: 14, marginBottom: 18 }}>
          ...
          <fieldset style={{ border: "1px solid #eef1f6", borderRadius: 10, padding: 10 }}>
            <legend style={{ fontSize: 12.5, fontWeight: 700, color: "#5c6675", padding: "0 6px" }}>Send to</legend>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(190px,1fr))", gap: 4 }}>
              {(members ?? []).map((member) => (
                <label key={member.id} style={{ display: "flex", alignItems: "center", gap: 7, fontSize: 13, cursor: "pointer" }}>
                  <input type="checkbox" name="recipients" value={member.id} defaultChecked={member.id === profile.id} />
                  <span>
                    {member.full_name || "(no name)"} <span style={{ color: "#5c6675" }}>· {member.role}</span>
                    {member.notify_email_opt_in === false && <b style={{ color: "#b45309" }}> · alerts off</b>}
                  </span>
                </label>
              ))}
            </div>
            <p style={{ fontSize: 12, color: "#5c6675", margin: "8px 0 0" }}>
              Recipients are teammates, not free-text addresses. Anyone who has turned notification email off is
              skipped at send time, with the reason recorded — the same opt-out rule that protects customers.
            </p>
          </fieldset>
          <div><button type="submit" style={primary}>+ Schedule report</button></div>
        </div>
      </ActionForm>
            ...
            {(recent ?? []).map((run) => (
              <div className="ritem" key={run.id}>
                <div className="rmain">
                  ...
                  <div className="rsub">
                    {run.created_at?.slice(0, 10)} · {run.recipients} recipient{run.recipients === 1 ? "" : "s"}
                    {run.reason ? ` · ${run.reason}` : ""}
                  </div>
                </div>
                <div className="rend">
                  <b style={{ color: run.status === "sent" ? "#15803d" : run.status === "failed" ? "#dc2626" : "#5c6675" }}>
                    {run.status}
                  </b>
                </div>
              </div>
