...
export async function createReportSchedule(
  ...
  try {
    ...
    return { ok: false, error: "Only the owner can schedule reports." };
  }

  const name =
    String(formData.get("name") ?? "")
      .trim()
      .slice(0, 80) || "Business summary";
  ...
  if (!recipients.length) {
    ...
    return {
      ok: false,
      error: "Choose at least one recipient — a schedule with nobody on it would send nothing.",
    };
  }
...
export async function setReportScheduleEnabled(
  ...
  try {
    ...
    return { ok: false, error: "Only the owner can change a schedule." };
  }
  const supabase = await createClient();
...
export async function deleteReportSchedule(id: string): Promise<ActionResult> {
  ...
  try {
    ...
    return { ok: false, error: "Only the owner can delete a schedule." };
  }
  const supabase = await createClient();
  const { error } = await supabase
    .from("report_schedules")
