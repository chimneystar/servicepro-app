...
export default async function MessagesPage({ searchParams }: { searchParams: Promise<{ show?: string }> }) {
  ...
  const list = [...threads.entries()].map(([key, thread]) => ({ ...thread, name: nameByPhone.get(key) ?? thread.phone }));
...
async function namesForThreads(supabase: Awaited<ReturnType<typeof createClient>>, keys: string[]) {
  ...
  const suffixes = [...new Set(keys.map((key) => phoneSearchSuffix(key)).filter(Boolean))] as string[];
  ...
  const results = await Promise.all(chunks.map((chunk) => supabase
    .from("customers").select("name, phone").is("deleted_at", null)
    .or(chunk.map((suffix) => `phone.ilike.${quoteFilterValue(`%${escapeLikePattern(suffix)}`)}`).join(","))
    .limit(400)));

  for (const { data } of results) {
    (data ?? []).forEach((c) => { if (c.phone) byPhone.set(digits(c.phone), c.name); });
  }
  return byPhone;
}
