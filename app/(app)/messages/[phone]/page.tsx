...
export default async function ThreadPage({
  ...
  const [{ data: msgs }, { data: cust }] = await Promise.all([
    suffix
      ? supabase
          .from("sms_messages")
          .select("body, direction, created_at, to_phone, from_phone")
  ...
  const name =
    (cust ?? []).find((c) => (c.phone ?? "").replace(/[^0-9]/g, "").slice(-10) === last10)?.name ??
    phone;
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", gap: 8, paddingBottom: 12 }}>
        {truncated && (
          ...
          >
            ...
            {pageSize < MAX_THREAD_PAGE && (
              <Link
                href={`/messages/${encodeURIComponent(phone)}?show=${Math.min(pageSize * 2, MAX_THREAD_PAGE)}`}
                style={{ color: "#9a3412", fontWeight: 800 }}
              >
        ...
        {thread.map((m: any, i: number) => (
          ...
          >
            <div
              style={{
                background: m.direction === "inbound" ? "#fff" : "#2563eb",
                color: m.direction === "inbound" ? "#0b1524" : "#fff",
