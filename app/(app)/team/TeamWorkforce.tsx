...
export default function TeamWorkforce({
  ...
  const nameOf = (id: string | null) =>
    members.find((m) => m.id === id)?.full_name || (he ? "כל העסק" : "Whole business");
  const currentRate = (id: string) => {
    const applicable = rates
      .filter((r) => r.profile_id === id && r.effective_from <= today)
    ...
    <div style={{ maxWidth: 720 }}>
      ...
      <div style={card}>
        ...
        >
          <select
            value={skillFor}
            onChange={(e) => setSkillFor(e.target.value)}
            style={{ ...inp, flex: "1 1 150px" }}
            aria-label={he ? "עובד" : "Team member"}
          >
            {members.map((m) => (
          ...
          <input
            ...
            placeholder={he ? "קוד (gas, hvac…)" : "code (gas, hvac…)"}
            style={{ ...inp, flex: "1 1 130px" }}
            aria-label={he ? "קוד (gas, hvac…)" : "code (gas, hvac…)"}
          />
          <datalist id="sp-skill-codes">
            {(COMMON_SKILLS as { code: string; en: string; he: string }[]).map((s) => (
          ...
          <input
            ...
            placeholder={he ? "מספר רישיון" : "licence no."}
            style={{ ...inp, flex: "1 1 130px" }}
            aria-label={he ? "מספר רישיון" : "licence no."}
          />
          <input
            type="date"
            value={skillExpires}
            onChange={(e) => setSkillExpires(e.target.value)}
            style={{ ...inp, flex: "0 0 150px" }}
            aria-label={he ? "תוקף עד" : "Expires"}
          />
          <button
            type="button"
            ...
            onClick={() =>
              run(async () => {
                const result = await addSkill(skillFor, {
                  skill: skillCode,
                  certificationNumber: skillNumber,
                  expiresOn: skillExpires,
                });
                if (result.ok) {
                  setSkillCode("");
                  setSkillNumber("");
        ...
        {skills.length === 0 && (
          <div style={hint}>{he ? "עוד לא נרשמו הסמכות." : "No certifications recorded yet."}</div>
        )}
        {skills.map((s) => {
      ...
      <div style={card}>
        ...
        >
          <select
            value={offFor}
            onChange={(e) => setOffFor(e.target.value)}
            style={{ ...inp, flex: "1 1 150px" }}
            aria-label={he ? "עובד" : "Team member"}
          >
          ...
          <input
            ...
            style={{ ...inp, flex: "0 0 150px" }}
            aria-label={he ? "עד תאריך" : "To"}
          />
          <select
            value={offKind}
            onChange={(e) => setOffKind(e.target.value)}
            style={{ ...inp, flex: "0 0 140px" }}
            aria-label={he ? "סוג" : "Type"}
          >
            {["time_off", "vacation", "sick", "personal", "training", "holiday", "other"].map(
              (k) => (
        ...
        {timeOff.length === 0 && (
          <div style={hint}>{he ? "לא נרשמו ימי אי-עבודה." : "Nothing recorded."}</div>
        )}
        {timeOff.map((row2) => (
          <div key={row2.id} style={row}>
            <div style={{ flex: 1, minWidth: 0 }}>
              ...
              <div style={{ fontSize: "0.75rem", color: "#5c6675" }}>
                ...
                {row2.start_time
                  ? ` · ${row2.start_time.slice(0, 5)}–${(row2.end_time ?? "").slice(0, 5)}`
                  : ` · ${he ? "כל היום" : "all day"}`}
                {` · ${row2.kind}`}
                {row2.note ? ` · ${row2.note}` : ""}
              </div>
            ...
            <button
              ...
              disabled={busy}
              style={rm}
              onClick={() => run(() => removeTimeOff(row2.id))}
            >
              {he ? "הסרה" : "Remove"}
            </button>
          </div>
      ...
      <div style={{ flex: 1, minWidth: 140 }}>
        ...
        <div style={{ fontSize: "0.75rem", color: rate ? "#5c6675" : "#a15c07" }}>
          {rate
            ...
            : he
              ? "אין תעריף — שעות העובד הזה מתומחרות באפס והמסך אומר זאת."
              : "No rate — this person's hours cost 0 and job costing says so rather than pretending they are free."}
        </div>
      </div>
      <input
        value={amount}
        ...
        aria-label={he ? "עלות לשעה" : "Cost per hour"}
      />
      <input
        type="date"
        value={from}
        ...
        style={{ ...inp, flex: "0 0 150px" }}
        aria-label={he ? "בתוקף מ" : "Effective from"}
      />
      <button
        type="button"
        disabled={busy || !amount.trim()}
        style={btn}
        onClick={() => onSave(amount, from)}
      >
        {he ? "שמירה" : "Save"}
      </button>
    </div>
