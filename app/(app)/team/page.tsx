...
import TeamWorkforce, {
  type WorkforceSkill,
  type WorkforceTimeOff,
  type WorkforceRate,
} from "./TeamWorkforce";
...
export default async function TeamPage() {
  ...
  const [
    ...
    supabase
      .from("invitations")
      .select("id, email, role, delivery_status, delivery_error, sent_at")
      .is("accepted_at", null)
      .order("created_at", { ascending: false }),
    supabase
      .from("profile_payment_permissions")
      .select(
        "profile_id, can_confirm_manual_payments, can_refund_payments, can_override_ach_holds",
      ),
    supabase
      .from("profile_capabilities")
      .select(
        "profile_id, can_view_customers, can_edit_customers, can_manage_schedule, can_edit_jobs, can_manage_estimates, can_manage_invoices, can_manage_payments, can_view_reports, can_manage_purchasing, can_manage_automations, can_manage_settings, can_manage_team",
      ),
    ...
    supabase
      ...
      .select("id, profile_id, skill_code, label, certification_number, issued_on, expires_on")
      .order("skill_code"),
    supabase
      .from("technician_time_off")
      .select("id, profile_id, starts_on, ends_on, start_time, end_time, kind, note")
      .gte("ends_on", timeOffFrom)
      .order("starts_on"),
    supabase
      .from("technician_pay_rates")
      .select("profile_id, cost_rate_minor, effective_from")
  ...
  return (
    <div>
      ...
      <p style={{ marginBottom: 16, fontSize: "0.8125rem" }}>
        <Link href="/settings/security">
          {locale === "he"
            ...
            : "Permission history & account security →"}
        </Link>
      </p>
      <TeamClient
        locale={locale}
      ...
      <TeamWorkforce
        ...
        members={members ?? []}
        skills={(skills ?? []) as WorkforceSkill[]}
        timeOff={(timeOff ?? []) as WorkforceTimeOff[]}
        rates={(rates ?? []) as WorkforceRate[]}
        canSeePay={profile.role === "owner"}
      />
    </div>
  );
}
