      ...
      <div style={card}>
        ...
        {members.map((m) => {
          ...
          return <div key={m.id} style={{ ...row, alignItems: "flex-start" }}>
              ...
              <select value={m.role} disabled={m.id === myId || pending} onChange={(e) => run(() => changeRole(m.id, e.target.value))} style={{ ...inp, width: "auto", padding: "7px 10px", fontSize: 13 }} aria-label={he ? "תפקיד" : "Role"}>
                <option value="tech">{roleLabel("tech")}</option>
                <option value="office">{roleLabel("office")}</option>
                <option value="owner">{roleLabel("owner")}</option>
              </select>
