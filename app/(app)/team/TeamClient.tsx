...
export default function TeamClient({
  ...
  const roleLabel = (r: string) =>
    t(locale, r === "owner" ? "role.owner" : r === "office" ? "role.office" : "role.tech");
  ...
  return (
    <div style={{ maxWidth: 720 }}>
      ...
      <div style={card}>
        <h3 style={h3}>
          {t(locale, "team.members")} ({members.length})
        </h3>
          ...
          return (
            <div key={m.id} style={{ ...row, alignItems: "flex-start" }}>
              ...
              <div style={{ flex: 1, minWidth: 180 }}>
                <b>
                  {m.full_name || "—"}
                  {m.id === myId ? ` (${t(locale, "team.you")})` : ""}
                </b>
                {m.role !== "owner" && m.id !== myId && (
                  <CapabilityEditor
                    locale={locale}
                    memberId={m.id}
                    role={m.role}
                    initial={capabilityAccess}
                    onSaved={() => router.refresh()}
                  />
                )}
                {m.role === "office" && m.id !== myId && (
                  <PaymentPermissionEditor
                    locale={locale}
                    memberId={m.id}
                    initial={paymentAccess}
                    onSaved={() => router.refresh()}
                  />
                )}
              </div>
              ...
              {m.id !== myId && (
                <button
                  type="button"
                  onClick={() => {
                    if (confirm(he ? "להסיר את העובד מהצוות?" : "Remove this team member?"))
                      run(() => removeMember(m.id));
                  }}
                  disabled={pending}
                  style={rm}
                >
                  {t(locale, "team.remove")}
                </button>
              )}
            </div>
          );
      ...
      {invites.length > 0 && (
        <div style={card}>
          <h3 style={h3}>
            {t(locale, "team.pending")} ({invites.length})
          </h3>
          {invites.map((iv) => (
            <div key={iv.id} style={row}>
              <div className="sp-flex-fill">
                ...
                >
                  {describeInviteDelivery(iv, locale).text}
                </div>
              </div>
              <button
                type="button"
                onClick={() => run(() => resendInvite(iv.id))}
                disabled={pending}
                style={rm}
              >
              ...
              <button
                type="button"
                onClick={() => run(() => cancelInvite(iv.id))}
                disabled={pending}
                style={rm}
              >
                {t(locale, "team.cancelInvite")}
              </button>
            </div>
          ))}
        </div>
      ...
      >
        {he
          ? "אנחנו שולחים לעובד מייל עם קישור הצטרפות אישי. ההצטרפות מחייבת גם את הקישור וגם את כתובת האימייל שאליה נשלח — כתובת אימייל לבדה אינה מספיקה. טכנאים רואים רק עבודות ששובצו להם."
          : "We email the person a personal join link. Joining needs both that link and the email address it was sent to — the email address alone is not enough. Technicians see only jobs assigned to them."}
      </div>
    </div>
  );
}
...
function CapabilityEditor({
  ...
  const options: { key: keyof CapabilityValues; en: string; he: string }[] = [
    { key: "viewCustomers", en: "View customers", he: "צפייה בלקוחות" },
    { key: "editCustomers", en: "Edit customers", he: "עריכת לקוחות" },
    { key: "manageSchedule", en: "Manage schedule", he: "ניהול לוח הזמנים" },
    { key: "editJobs", en: "Update jobs", he: "עדכון עבודות" },
    { key: "manageEstimates", en: "Manage estimates", he: "ניהול הצעות מחיר" },
    { key: "manageInvoices", en: "Manage invoices", he: "ניהול חשבוניות" },
    { key: "managePayments", en: "Manage payments", he: "ניהול תשלומים" },
    { key: "viewReports", en: "View reports", he: "צפייה בדוחות" },
    { key: "managePurchasing", en: "Purchasing and vendors", he: "רכש וספקים" },
    { key: "manageAutomations", en: "Manage automations", he: "ניהול אוטומציות" },
    { key: "manageSettings", en: "Business settings", he: "הגדרות העסק" },
    { key: "manageTeam", en: "Team and permissions", he: "צוות והרשאות" },
  ];
  return (
    <div className="team-capability-editor">
      <button
        type="button"
        className="team-capability-toggle"
        onClick={() => setOpen((current) => !current)}
        aria-expanded={open}
      >
        {he ? "הרשאות באפליקציה" : "App access"}
        <span aria-hidden="true">{open ? "−" : "+"}</span>
      </button>
      {open && (
        <div className="team-capability-panel">
          <p>
            {he
              ? "בחרו בדיוק מה העובד יכול לראות ולעשות."
              : "Choose exactly what this team member can see and do."}
          </p>
          <div className="team-capability-grid">
            {options.map((option) => (
              <label key={option.key}>
                ...
                />
                <span>{he ? option.he : option.en}</span>
              </label>
            ))}
          </div>
          <button
            type="button"
            className="team-capability-save"
            disabled={saving}
            onClick={() =>
              startSaving(async () => {
                const result = await updateCapabilities(memberId, values);
                setMessage(
                  result.ok
                    ? he
                      ? "ההרשאות נשמרו"
          ...
          >
            {saving ? (he ? "שומרים…" : "Saving…") : he ? "שמירת הרשאות" : "Save access"}
          </button>
...
function PaymentPermissionEditor({
  locale,
  memberId,
  initial,
  onSaved,
  ...
  const options = [
    ...
    {
      key: "overrideAchHold" as const,
      label: he ? "שחרור עבודת ACH בהמתנה" : "Override ACH holds",
    },
  ];
  return (
    <div className="team-payment-permissions">
      ...
      <button
        ...
        onClick={() =>
          startSaving(async () => {
            const result = await updatePaymentPermissions(memberId, values);
            setMessage(
              result.ok
                ? he
                  ? "נשמר"
                  : "Saved"
                : (result.error ?? (he ? "לא נשמר" : "Not saved")),
            );
            if (result.ok) onSaved();
          })
        }
...
function SendBtn({ locale }: { locale: Locale }) {
  ...
  return (
    <button type="submit" disabled={pending} style={{ ...btn, flex: "0 0 auto" }}>
      {pending ? (
        t(locale, "common.saving")
      ) : (
        <>
          <span aria-hidden="true">➕</span> {t(locale, "team.send")}
        </>
      )}
    </button>
  );
...
const h3: React.CSSProperties = { fontSize: "0.9375rem", fontWeight: 800, marginBottom: 12 };
...
const deliveryColour = (tone: string) =>
  tone === "ok" ? "#15803d" : tone === "error" ? "#dc2626" : "#a15c07";
