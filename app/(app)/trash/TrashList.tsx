...
export default function TrashList({ rows }: { rows: TrashRow[] }) {
  ...
  if (!rows.length) {
    return (
      <div style={{ background: "#fff", border: "1px solid #e2e8f0", borderRadius: 14, padding: 28, textAlign: "center", color: "#5c6675", fontSize: "0.875rem" }}>
        ...
        return (
          <div key={`${row.kind}:${row.id}`} style={{ background: "#fff", border: "1px solid #e2e8f0", borderRadius: 14, padding: 14, opacity: isRestored ? 0.55 : 1 }}>
            <div style={{ display: "flex", gap: 12, alignItems: "flex-start", flexWrap: "wrap" }}>
              <div style={{ fontSize: "1.25rem", lineHeight: 1.2 }} aria-hidden>{ICON[row.kind]}</div>
              <div style={{ flex: "1 1 240px", minWidth: 0 }}>
                <div style={{ fontSize: "0.8125rem", fontWeight: 800, letterSpacing: 0.4, textTransform: "uppercase", color: "#94a3b8" }}>{LABEL[row.kind]}</div>
                <div style={{ fontWeight: 700, fontSize: "0.9375rem", color: "#0f172a", overflowWrap: "anywhere" }}>{row.title}</div>
                {row.detail && <div style={{ color: "#5c6675", fontSize: "0.8125rem", marginTop: 2, overflowWrap: "anywhere" }}>{row.detail}</div>}
              ...
              <div style={{ flex: "0 0 auto" }}>
                {isRestored ? (
                  <span style={{ color: "#15803d", fontWeight: 700, fontSize: "0.8125rem" }}>✓ Restored</span>
                ) : (
                  <button
                    type="button"
                    onClick={() => restore(row)}
            ...
            {blocked && !isRestored && (
              <div style={{ background: "#fff7ed", border: "1px solid #fed7aa", color: "#7c2d12", borderRadius: 10, padding: "9px 12px", fontSize: "0.8125rem", marginTop: 10, lineHeight: 1.5 }}>
                {row.blockers.map((blocker) => (<div key={blocker}>{blocker}</div>))}
              </div>
            )}

            {message?.id === row.id && (
              <div style={{ marginTop: 10, fontSize: "0.8125rem", fontWeight: 700, color: message.ok ? "#15803d" : "#dc2626" }}>{message.text}</div>
            )}
          </div>
        );
      })}
