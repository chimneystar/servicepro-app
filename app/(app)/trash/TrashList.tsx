...
const ICON: Record<TrashRow["kind"], string> = {
  customer: "👤",
  job: "💼",
  estimate: "📝",
  invoice: "🧾",
};
const LABEL: Record<TrashRow["kind"], string> = {
  customer: "Customer",
  job: "Job",
  estimate: "Estimate",
  invoice: "Invoice",
...
export default function TrashList({ rows }: { rows: TrashRow[] }) {
  ...
  function restore(row: TrashRow) {
    ...
    start(async () => {
      ...
      if (result.ok) {
        ...
        setMessage({
          id: row.id,
          text: `✓ Restored — ${row.title} is back in your ${row.kind === "customer" ? "customers" : `${row.kind}s`}.`,
          ok: true,
        });
      } else {
  ...
  if (!rows.length) {
    return (
      ...
      >
        Nothing has been deleted. When someone deletes a customer, job, estimate or invoice it will
        appear here.
      </div>
    );
  }
        ...
        return (
          <div
            ...
            style={{
              ...
              borderRadius: 14,
              padding: 14,
              opacity: isRestored ? 0.55 : 1,
            }}
          >
            <div style={{ display: "flex", gap: 12, alignItems: "flex-start", flexWrap: "wrap" }}>
              <div style={{ fontSize: "1.25rem", lineHeight: 1.2 }} aria-hidden>
                {ICON[row.kind]}
              </div>
