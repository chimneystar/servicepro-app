...
function parseCsv(text: string): Row[] {
  ...
  let start = 0;
  if (/name|phone|email|city/i.test(lines[0])) {
  ...
  return (
    <div style={{ maxWidth: 720 }}>
      ...
      <p style={{ color: "#5c6675", fontSize: "0.875rem", marginBottom: 14 }}>
        Paste rows from your spreadsheet. First line can be a header. Columns:{" "}
        <b>name, phone, email, city</b> (only name is required).
      </p>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 12, marginTop: 12 }}>
        <button
          ...
          style={{
            background: rows.length ? "#2563eb" : "#94a3b8",
            color: "#fff",
            border: "none",
            borderRadius: 12,
            padding: "12px 20px",
          ...
          {rows.slice(0, 8).map((r, i) => (
            <div className="ritem" key={i}>
              <div className="rmain">
                ...
                <div className="rsub">{[r.phone, r.city].filter(Boolean).join(" · ")}</div>
              </div>
            </div>
          ))}
