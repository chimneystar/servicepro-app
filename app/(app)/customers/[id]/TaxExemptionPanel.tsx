  ...
  return (
    <div style={card}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 10 }}>
        <h3 style={{ fontSize: "0.9375rem", fontWeight: 800 }}>{he ? "פטור ממס מכירה" : "Sales tax exemption"}</h3>
      ...
      {exemptions.map((row) => (
        <div key={row.id} style={{ display: "flex", alignItems: "center", gap: 10, padding: "9px 0", borderTop: "1px solid #f1f4f9", fontSize: "0.8125rem" }}>
          ...
          {row.document_url && <a href={row.document_url} target="_blank" rel="noreferrer" style={{ fontSize: "0.75rem", color: "#2563eb", fontWeight: 700 }}>{he ? "מסמך" : "Document"}</a>}
      ...
      {exemptions.length === 0 && <div style={{ fontSize: "0.8125rem", color: "#5c6675" }}>{he ? "לא נרשם אישור פטור." : "No exemption certificate on file."}</div>}
      {message && <div style={{ fontSize: "0.8125rem", color: "#2563eb", marginTop: 8 }} role="status">{message}</div>}

      <details style={{ marginTop: 10 }}>
        <summary style={{ fontSize: "0.8125rem", fontWeight: 700, color: "#2563eb", cursor: "pointer" }}>{he ? "רישום אישור פטור" : "Record an exemption certificate"}</summary>
