...
export default function TaxExemptionPanel({
  ...
  const day = (value: string) =>
    new Date(`${value}T12:00:00Z`).toLocaleDateString(he ? "he-IL" : "en-US");
    ...
    <div style={card}>
      ...
      >
        ...
        <span
          ...
          style={
            exempt
              ? { background: "#e6f6ec", color: "#15803d" }
              : { background: "#eef1f6", color: "#57606f" }
          }
        >
      ...
      <p style={{ fontSize: "0.8125rem", color: "#5c6675", margin: "4px 0 10px" }}>
        {exempt
          ? he
            ...
            : "New documents for this customer are raised with no tax."
          : he
            ? "מסמכים חדשים ללקוח הזה ייווצרו עם מס לפי ההגדרות."
            : "New documents for this customer are taxed at the configured rate."}
      </p>
      ...
      {exemptions.map((row) => (
        ...
        >
          ...
          >
            {row.active ? (he ? "ביטול" : "Revoke") : he ? "החזרה" : "Restore"}
          </button>
        </div>
      ))}
      {exemptions.length === 0 && (
        ...
        <form action={action} style={{ display: "grid", gap: 8, marginTop: 10 }}>
          ...
          <label style={lbl}>
            ...
            <input
              ...
              required
              style={inp}
              placeholder={he ? "מוסד ללא כוונת רווח" : "Non-profit organisation"}
            />
          </label>
          <label style={lbl}>
            {he ? "מספר אישור" : "Certificate number"}
            <input name="certificate" style={inp} />
          </label>
          <label style={lbl}>
            {he ? "בתוקף עד" : "Expires on"}
            <input name="expiresOn" type="date" style={inp} />
          </label>
          <label style={lbl}>
            {he ? "קישור למסמך" : "Document link"}
            <input name="documentUrl" type="url" style={inp} />
          </label>
          <Save he={he} />
          {state.error && (
