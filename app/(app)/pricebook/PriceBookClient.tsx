...
type Item = { id: string; name: string; category: string | null; unit: string | null; price_minor: number; cost_minor: number };
const initial: ActionResult = { ok: false };
const sym: Record<string, string> = { USD: "$", ILS: "₪", EUR: "€" };

export default function PriceBookClient({ locale, items, currency }: { locale: Locale; items: Item[]; currency: string }) {
  ...
  async function del(id: string) { if (!confirm("Delete this item?")) return; await deletePriceItem(id); router.refresh(); }

  return (
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
        <h1 style={{ fontSize: "1.5rem", fontWeight: 800 }}>{t(locale, "pb.title")}</h1>
        <button type="button" onClick={() => setEditing(null)} style={btn}><span aria-hidden="true">➕</span> {t(locale, "pb.new")}</button>
      </div>
          ...
          <div className="ritem" key={it.id}>
            ...
            <div className="rend">
              ...
              <div style={{ display: "flex", gap: 6 }}>
                <button type="button" onClick={() => setEditing(it)} style={mini} aria-label={he ? "עריכה" : "Edit"}>✏️</button>
                <button type="button" onClick={() => del(it.id)} style={{ ...mini, background: "#fdeaea" }} aria-label={t(locale, "common.delete")}>🗑️</button>
              </div>
            </div>
          </div>
        ))}
      ...
      {editing !== undefined && (
        <Modal onClose={() => setEditing(undefined)} labelledBy={titleId} width={460}>
          <form action={formAction}>
            <h3 id={titleId} style={{ fontSize: "1.125rem", fontWeight: 800, marginBottom: 14 }}>{editing ? editing.name : t(locale, "pb.new")}</h3>
            {editing && <input type="hidden" name="id" value={editing.id} />}
            <label style={{ display: "block" }}><L>{t(locale, "pb.name")}</L><input name="name" defaultValue={editing?.name ?? ""} style={inp} required /></label>
            <div style={two}>
              <div><label style={{ display: "block" }}><L>{t(locale, "pb.category")}</L><input name="category" defaultValue={editing?.category ?? ""} style={inp} /></label></div>
              <div><label style={{ display: "block" }}><L>{t(locale, "pb.unit")}</L><input name="unit" defaultValue={editing?.unit ?? "unit"} style={inp} /></label></div>
            </div>
            <div style={two}>
              <div><label style={{ display: "block" }}><L>{t(locale, "pb.price")}</L><input name="price" type="number" step="0.01" defaultValue={editing ? (editing.price_minor / 100).toFixed(2) : ""} style={inp} placeholder="0.00" /></label></div>
              <div><label style={{ display: "block" }}><L>{t(locale, "pb.cost")}</L><input name="cost" type="number" step="0.01" defaultValue={editing ? (editing.cost_minor / 100).toFixed(2) : ""} style={inp} placeholder="0.00" /></label></div>
            </div>
            {state.error && <div style={err}>{state.error}</div>}
            <div style={{ display: "flex", gap: 10, marginTop: 14 }}>
              <Save locale={locale} />
              <button type="button" onClick={() => setEditing(undefined)} style={{ ...btn, background: "#e2e9f4", color: "#2563eb" }}>{t(locale, "common.cancel")}</button>
            </div>
          </form>
        </Modal>
      )}
