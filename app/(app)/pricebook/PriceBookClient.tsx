      ...
      {editing !== undefined && (
        <div style={overlay} onClick={(e) => e.target === e.currentTarget && setEditing(undefined)}>
          <form action={formAction} style={modal}>
            ...
            <div style={two}>
              <div><L>{t(locale, "pb.category")}</L><input name="category" defaultValue={editing?.category ?? ""} style={inp} /></div>
              <div><L>{t(locale, "pb.unit")}</L><input name="unit" defaultValue={editing?.unit ?? "unit"} style={inp} /></div>
            </div>
            <div style={two}>
              <div><L>{t(locale, "pb.price")}</L><input name="price" type="number" step="0.01" defaultValue={editing ? (editing.price_minor / 100).toFixed(2) : ""} style={inp} placeholder="0.00" /></div>
              <div><L>{t(locale, "pb.cost")}</L><input name="cost" type="number" step="0.01" defaultValue={editing ? (editing.cost_minor / 100).toFixed(2) : ""} style={inp} placeholder="0.00" /></div>
            </div>
