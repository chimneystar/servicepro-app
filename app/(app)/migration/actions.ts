...
export async function importMigrationCustomers(
  source: "workiz" | "housecall_pro" | "spreadsheet",
  filename: string,
  input: MigrationCustomer[],
) {
  ...
  const { data: batch, error: batchError } = await supabase
    ...
    .insert({
      ...
      status: "importing",
      counts_json: { received: input.length, valid: rows.length },
  ...
  const payload = rows.map((row) => ({
    ...
    phone: (row.phone ?? "").slice(0, 60),
    email: (row.email ?? "").slice(0, 180) || null,
    address: (row.address ?? "").slice(0, 240) || null,
    city: (row.city ?? "").slice(0, 120) || null,
    notes: (row.notes ?? "").slice(0, 4000) || null,
    source:
      source === "housecall_pro" ? "Housecall Pro" : source === "workiz" ? "Workiz" : "Import",
    created_by: profile.id,
  ...
  if (error) {
    await supabase
      .from("migration_batches")
      .update({ status: "failed", errors_json: [{ message: "Customer import failed" }] })
      .eq("id", batch.id);
  ...
  await supabase
    ...
    .update({
      status: "completed",
      counts_json: { received: input.length, valid: rows.length, imported: inserted?.length ?? 0 },
