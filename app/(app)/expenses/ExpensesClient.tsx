      ...
      <div className="rlist">
        {expenses.map((e) => (
          <div className="ritem" key={e.id}>
            <div className="avatar-sm" style={{ background: "#fdeaea", color: "#dc2626", fontSize: "1rem" }}>💸</div>
            <div className="rmain">
            ...
            <div className="rend">
              ...
              <button type="button" onClick={() => del(e.id)} style={{ background: "#fdeaea", border: "none", borderRadius: 8, padding: "4px 8px", cursor: "pointer", fontSize: "0.75rem" }} aria-label={t(locale, "common.delete")}>🗑️</button>
            </div>
          </div>
        ))}
      ...
      {open && (
        <Modal onClose={() => setOpen(false)} labelledBy={titleId} width={440}>
          <form action={formAction}>
            <h3 id={titleId} style={{ fontSize: "1.125rem", fontWeight: 800, marginBottom: 14 }}>{t(locale, "exp.new")}</h3>
            <div style={two}>
...
function Kpi({ label, value, tone }: { label: string; value: string; tone: string }) {
  return <div style={{ background: "#fff", border: "1px solid #e2e8f0", borderRadius: 14, padding: 15, boxShadow: "0 6px 18px rgba(15,42,94,.06)" }}><div style={{ fontSize: "1.375rem", fontWeight: 800, color: tone }}>{value}</div><div style={{ fontSize: "0.8125rem", color: "#5c6675", fontWeight: 600 }}>{label}</div></div>;
}
