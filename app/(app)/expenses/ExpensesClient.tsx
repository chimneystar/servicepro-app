        ...
        {expenses.map((e) => (
          <div className="ritem" key={e.id}>
            ...
            <div className="rend">
              ...
              <button onClick={() => del(e.id)} style={{ background: "#fdeaea", border: "none", borderRadius: 8, padding: "4px 8px", cursor: "pointer", fontSize: "0.75rem" }}>🗑️</button>
            </div>
          </div>
        ))}
      ...
      {open && (
        <div style={overlay} onClick={(e) => e.target === e.currentTarget && setOpen(false)}>
          <form action={formAction} style={modal}>
            <h3 style={{ fontSize: "1.125rem", fontWeight: 800, marginBottom: 14 }}>{t(locale, "exp.new")}</h3>
            <div style={two}>
