...
export async function saveAppearance(_previous: AppearanceResult, formData: FormData): Promise<AppearanceResult> {
  ...
  if (!["light", "dark", "system"].includes(theme) || !["normal", "high"].includes(contrast) || !["normal", "large"].includes(textScale)) {
    return { ok: false, error: locale === "he" ? "אחת מהאפשרויות אינה תקינה." : "One of the appearance options is invalid." };
  }
  ...
  if (error) return { ok: false, error: locale === "he" ? "לא הצלחנו לשמור את המראה. נסו שוב." : "We couldn't save your appearance. Please try again." };
