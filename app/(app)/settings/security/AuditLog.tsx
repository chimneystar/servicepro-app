...
export default function AuditLog({
  ...
  permissionChanges: PermissionRow[];
  ...
  const who = (id: string | null) =>
    id ? (names.get(id) ?? (he ? "משתמש שהוסר" : "removed user")) : he ? "המערכת" : "system";
  ...
  return (
    <div style={{ display: "grid", gap: 14 }}>
      <section className="ops-card">
        <header>
          <div>
            <h2>{he ? "מה השתנה בעסק" : "What changed in the business"}</h2>
            <p>
              {he
                ? "כל שינוי בעבודות, לקוחות, הצעות, חשבוניות ותשלומים."
                : "Every change to jobs, customers, estimates, invoices and payments."}
            </p>
          </div>
        ...
        <form method="get" className="ops-form">
          ...
          <div className="ops-actions">
            ...
            <Link href="/settings/security" className="ops-secondary">
              {he ? "ניקוי" : "Clear"}
            </Link>
          </div>
        </form>
        ...
        <ul className="ops-list">
          {rows.length === 0 && (
            <li className="ops-empty">
              {he ? "אין רישומים בטווח הזה." : "Nothing recorded in that range."}
            </li>
          )}
          {rows.map((row) => (
            <li key={row.id}>
              <div>
                <strong>
                  {verb(row.action, he)} {row.table_name}
                </strong>
                <small>
                  {who(row.actor)} · {new Date(row.at).toLocaleString()}
        ...
        {lastPage > 1 && (
          <div className="ops-actions" style={{ padding: "12px 17px" }}>
            {filters.page > 1 && (
              <Link className="ops-secondary" href={pageHref(filters.page - 1)}>
                {he ? "הקודם" : "Previous"}
              </Link>
            )}
            <small>
              {he ? `עמוד ${filters.page} מתוך ${lastPage}` : `Page ${filters.page} of ${lastPage}`}
            </small>
      ...
      <section className="ops-card">
        <header>
          <div>
            <h2>{he ? "שינויי הרשאות" : "Permission changes"}</h2>
            <p>
              {he
                ? "תפקידים, יכולות והרשאות תשלום — נרשמים במסד הנתונים, לא באפליקציה."
                : "Roles, capabilities and payment permissions — recorded at the database, not by the app."}
            </p>
          </div>
        </header>
        <ul className="ops-list">
          {permissionChanges.length === 0 && (
            <li className="ops-empty">
              {he ? "לא נרשמו שינויי הרשאות." : "No permission changes recorded."}
            </li>
          )}
          {permissionChanges.map((change) => (
            <li key={change.id}>
              <div>
                ...
                <small>
                  {he ? "על" : "for"} {who(change.subject_profile_id)} · {he ? "על ידי" : "by"}{" "}
                  {who(change.actor_profile_id)}
                  {" · "}
                  ...
                  {change.ip
                    ? ` · ${change.ip}`
                    : ` · ${he ? "ללא כתובת (שינוי ישיר במסד)" : "no address (changed outside the app)"}`}
                </small>
              </div>
            </li>
          ))}
      ...
      <section className="ops-card">
        <header>
          <div>
            <h2>{he ? "ניסיונות התחברות" : "Sign-in attempts"}</h2>
            <p>
              {he
                ? "כולל ניסיונות כושלים ולניסיונות נגד כתובות שאינן קיימות."
                : "Including failures, and attempts against addresses that do not exist."}
            </p>
          </div>
        </header>
        <ul className="ops-list">
          {loginAttempts.length === 0 && (
            <li className="ops-empty">{he ? "אין ניסיונות רשומים." : "No attempts recorded."}</li>
          )}
          {loginAttempts.map((attempt) => (
            <li key={attempt.id}>
              <div>
                <strong>
                  {attempt.success ? (he ? "הצלחה" : "Success") : he ? "כישלון" : "Failed"} ·{" "}
                  {attempt.email_key}
                </strong>
                <small>
                  {attempt.device_label || (he ? "מכשיר לא ידוע" : "Unknown device")}
                  {attempt.ip
                    ? ` · ${attempt.ip}${attempt.ip_trusted ? "" : ` (${he ? "לא מאומת" : "unverified"})`}`
                    : ""}
      ...
      <section className="ops-card">
        <header>
          <div>
            <h2>{he ? "ראיות חתימה" : "Signature evidence"}</h2>
            <p>
              {he
                ? "מי חתם, מאיפה, ומה בדיוק נשמר."
                : "Who signed, from where, and a hash of exactly what was stored."}
            </p>
          </div>
        </header>
        <ul className="ops-list">
          {signatures.length === 0 && (
            <li className="ops-empty">
              {he ? "לא נחתמו מסמכים עדיין." : "No documents signed yet."}
            </li>
          )}
          {signatures.map((signature) => (
            <li key={signature.id}>
              <div>
                <strong>
                  {signature.signer_name} · {signature.document_type}
                </strong>
                <small>
                  {signature.capture === "server"
                    ? `${signature.device_label ?? ""}${signature.ip ? ` · ${signature.ip}${signature.ip_trusted ? "" : ` (${he ? "לא מאומת" : "unverified"})`}` : ""}`
                    : he
                      ? "ללא הקשר שרת — נחתם ישירות מול מסד הנתונים"
                      : "no server context — signed directly against the database"}
                  {" · "}
                  {new Date(signature.signed_at).toLocaleString()}
                  {signature.signature_sha256
                    ? ` · sha256 ${signature.signature_sha256.slice(0, 12)}…`
                    : ""}
                </small>
              </div>
              {signature.capture === "server" ? (
                <span className="ops-pill">{he ? "מתועד" : "witnessed"}</span>
              ) : (
                <span className="ops-pill warn">{he ? "ללא תיעוד" : "unwitnessed"}</span>
              )}
            </li>
          ))}
        </ul>
