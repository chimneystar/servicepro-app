    ...
    <div className="ops-grid" style={{ marginBottom: 16 }}>
      <div style={{ display: "grid", gap: 14 }}>
        ...
        <section className="ops-card">
          <header>
            <div>
              <h2>{he ? "שינוי סיסמה" : "Change password"}</h2>
              <p>
                {he
                  ? `לפחות ${MIN_LENGTH} תווים. הכלל נאכף בשרת, לא בדפדפן.`
                  : `At least ${MIN_LENGTH} characters. The rule is enforced on the server, not in the browser.`}
              </p>
            </div>
          </header>
          <form action={passwordAction} className="ops-form">
            ...
            <div className="ops-form-grid">
              ...
              <label>
                {he ? "אישור סיסמה" : "Confirm password"}
                <input
                  name="confirm"
                  type="password"
                  required
            ...
            {passwordState.error && (
              <div className="form-error" role="alert">
                {passwordState.error}
                {passwordState.problems?.length ? (
                  <ul style={listStyle}>
                    {passwordState.problems.map((problem) => (
            ...
            {passwordState.ok && passwordState.notice && (
              <span className="ops-success" role="status">
                ✓ {passwordState.notice}
              </span>
            )}
            <div className="ops-actions">
              <button type="submit" className="ops-primary" disabled={passwordPending}>
                {he ? "עדכון סיסמה" : "Change password"}
              </button>
            </div>
          </form>
        </section>
...
function TwoFactor({
  ...
  async function begin() {
    ...
    try {
      const { data, error } = await createClient().auth.mfa.enroll({
        factorType: "totp",
        friendlyName: `ServicePro ${new Date().toISOString().slice(0, 10)}`,
      });
  ...
  async function remove(factorId: string) {
    ...
    try {
      ...
      setMessage(
        he
          ? "אימות דו-שלבי הוסר. החשבון מוגן בסיסמה בלבד."
          : "Two-factor removed. This account is protected by a password alone.",
      );
    } catch (error: any) {
      setMessage(String(error?.message ?? error));
    } finally {
  ...
  return (
    <section className="ops-card">
      <header>
        <div>
          <h2>{he ? "אימות דו-שלבי" : "Two-factor authentication"}</h2>
          <p>
            {he
              ? "קוד חד-פעמי מאפליקציית אימות, בנוסף לסיסמה."
              : "A one-time code from an authenticator app, on top of your password."}
          </p>
        </div>
        {verified.length > 0 ? (
          <span className="ops-pill">{he ? "פעיל" : "On"}</span>
        ) : (
          <span className="ops-pill danger">{he ? "כבוי" : "Off"}</span>
        )}
      </header>
      <div className="ops-card-body" style={{ display: "grid", gap: 10 }}>
        ...
        {unavailable && (
          <p className="form-error" role="alert">
            {he
              ...
              : `Two-factor is not available on this project: ${unavailable}. Enable it in the Supabase Authentication settings.`}
          </p>
        )}
        ...
        {verified.map((factor) => (
          <div key={factor.id} className="ops-row" style={{ padding: 0 }}>
            <div>
              <strong>
                {factor.friendly_name || (he ? "אפליקציית אימות" : "Authenticator app")}
              </strong>
              <small>
                {he ? "נוסף" : "Added"}{" "}
                {enrolledAt ? new Date(enrolledAt).toLocaleDateString(he ? "he-IL" : "en-US") : "—"}
              </small>
            </div>
            <button
              type="button"
        ...
        {!enrolling && !unavailable && verified.length === 0 && (
          <div className="ops-actions">
            <button type="button" className="ops-primary" disabled={busy} onClick={begin}>
              {he ? "הפעלת אימות דו-שלבי" : "Turn on two-factor"}
            </button>
          </div>
        )}
        ...
        {enrolling && (
          <div style={{ display: "grid", gap: 8 }}>
            {enrolling.qr && (
              <img
                src={enrolling.qr}
                alt={he ? "קוד QR לאפליקציית האימות" : "Authenticator QR code"}
            ...
            <div className="ops-actions">
              <button type="button" className="ops-primary" disabled={busy} onClick={confirm}>
                {he ? "אישור" : "Confirm"}
              </button>
              <button
                type="button"
...
function Devices({
  ...
  return (
    <section className="ops-card">
      <header>
        <div>
          <h2>{he ? "מכשירים והתחברויות" : "Devices & sign-ins"}</h2>
          <p>
            {he
              ? "מה שהשרת ראה. עד עכשיו הוא לא ראה דבר."
              : "What the server observed. Until now it observed nothing."}
          </p>
        </div>
      </header>
      <div className="ops-card-body" style={{ display: "grid", gap: 10 }}>
        <label style={{ display: "flex", alignItems: "center", gap: 8, fontSize: "0.75rem" }}>
          <input
            ...
            onChange={(event) => {
              ...
              startTransition(async () => {
                const result = await setLoginAlerts(next);
          ...
          <span>
            {he
              ? "שלחו לי מייל על התחברות ממכשיר או רשת חדשים"
              : "Email me when a new device or network signs in"}
          </span>
        </label>
        ...
        <div className="ops-actions">
          ...
          <small style={{ color: "var(--muted)" }}>
            {he ? "כולל המכשיר הזה." : "This device included."}
          </small>
        </div>
        {notice && (
          <p className="ops-message" role="status">
        ...
        {security?.sessions_revoked_at && (
          <small style={{ color: "var(--muted)" }}>
            {he ? "ניתוק אחרון" : "Last revoked"}:{" "}
            {new Date(security.sessions_revoked_at).toLocaleString(he ? "he-IL" : "en-US")}
          </small>
        )}

        <ul className="ops-list">
          {events.length === 0 && (
            <li className="ops-empty">{he ? "אין רישומים עדיין." : "Nothing recorded yet."}</li>
          )}
          {events.map((event) => (
            <li key={event.id}>
              <div>
                <strong>
                  {label(event)}
                  {event.details?.new_device ? ` · ${he ? "מכשיר חדש" : "new device"}` : ""}
                </strong>
                <small>
                  {event.device_label || deviceLabel(null)}
                  {event.ip
                    ? ` · ${event.ip}${event.ip_trusted ? "" : ` (${he ? "לא מאומת" : "unverified"})`}`
                    : ` · ${he ? "ללא כתובת" : "no address"}`}
                  {" · "}
