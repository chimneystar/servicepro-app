...
export default async function EditInvoicePage({ params }: { params: { id: string } }) {
  ...
  const { data: inv } = await supabase.from("invoices").select("id, number, customer_id, discount_minor, notes, issue_date").eq("id", params.id).is("deleted_at", null).maybeSingle();
  if (!inv) return <div>Invoice not found.</div>;
  const [{ data: items }, { data: customers }, { data: catalog }] = await Promise.all([
    supabase.from("invoice_items").select("title, description, qty_milli, unit_price_minor, cost_minor, taxable, image_path").eq("invoice_id", params.id).order("sort"),
  ...
  return (
    <div>
      <Link href={`/invoices/${params.id}`} style={{ color: "#2563eb", fontWeight: 700, fontSize: 14, textDecoration: "none" }}>‹ Back</Link>
