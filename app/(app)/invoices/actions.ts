...
async function notifyOwnerOfPayment(
  ...
  try {
    ...
    await notifyPaymentReceived({
      organizationId,
      paymentId,
      amountLabel: formatMoney(Number(invoice?.total_minor ?? 0), {
    ...
    console.error(
      "[invoices] the payment notification could not be sent:",
...
export async function bulkSendInvoices(rawIds: string[]): Promise<BulkActionResult> {
  ...
  try {
    ...
    return refuse(t(await getLocale(), "err.forbidden"));
  }
  ...
  for (const id of selection.ids!) {
    ...
    if (!invoice) {
      results.push({ id, label: id.slice(0, 8), ok: false, reason: "invoice not found" });
      continue;
    }
    ...
    if (channel === "sms" ? !providers.sms() : !providers.email()) {
      results.push({
        ...
        ok: false,
        skipped: true,
        reason: "no email or SMS provider is connected",
      });
      continue;
    }
    ...
    const amount = formatMoney(Number(invoice.total_minor ?? 0), {
      currency: org?.currency ?? "USD",
    }) as string;
    const link = origin && invoice.public_token ? `${origin}/p/${invoice.public_token}` : "";
    const first = String(customer?.name ?? "").split(" ")[0] ?? "";
    const text =
      `Hi ${first}, invoice ${label} from ${org?.name ?? ""} for ${amount} is ready.` +
    ...
    try {
      if (channel === "email") {
        const html =
          `<p>${escapeHtml(text)}</p>` +
          (link
            ? `<p><a href="${escapeHtml(link)}">View invoice ${escapeHtml(label)}</a></p>`
            : "");
        const messageId = await sendEmail(
          eligibility.to!,
          `Invoice ${label} — ${org?.name ?? ""}`.trim(),
          html,
        );
...
export async function bulkSetInvoicePaid(
  rawIds: string[],
  paid: boolean,
  ...
  for (const id of selection.ids!) {
    ...
    const label = `#${invoice?.number ?? id.slice(0, 8)}`;
    if (!invoice) {
      results.push({ id, label, ok: false, reason: "invoice not found" });
      continue;
    }
    if ((invoice.status === "paid") === paid) {
      results.push({
        ...
        ok: false,
        skipped: true,
        reason: `already ${paid ? "paid" : "unpaid"}`,
      });
      continue;
    }
