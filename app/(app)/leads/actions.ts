...
export async function convertLead(id: string): Promise<ActionResult> {
  ...
  const { data: cust, error } = await supabase.from("customers").insert({
    ...
    address: lead.address || null, city: lead.city || null, source: lead.source || "Online booking",
    notes: lead.notes || null,
  }).select("id").single();
  if (error) return { ok: false, error: error.message };

  await supabase.from("leads").update({ status: "won", converted_customer_id: cust.id, updated_at: new Date().toISOString() }).eq("id", id);
