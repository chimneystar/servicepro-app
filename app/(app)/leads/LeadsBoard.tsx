...
const STATUSES: [string, string, string][] = [
  ["new", "New", "#e0ebff|#2563eb"],
  ["contacted", "Contacted", "#ede9fe|#7c3aed"],
  ["quoted", "Quoted", "#fdf1dc|#b45309"],
  ["won", "Won", "#e6f6ec|#15803d"],
  ["lost", "Lost", "#fdeaea|#dc2626"],
];
...
export default function LeadsBoard({ leads, orgId }: { leads: Lead[]; orgId: string }) {
  ...
  function bookingLink() {
    return `${window.location.origin}/book/${orgId}`;
  }
  function copyLink() {
    ...
    <div>
      ...
      >
        ...
        <div style={{ display: "flex", gap: 8 }}>
          <input
            readOnly
            aria-label="Your online booking link"
            value={typeof window !== "undefined" ? bookingLink() : ""}
            style={{
              flex: 1,
              border: "none",
              borderRadius: 9,
      ...
      <div style={{ display: "grid", gap: 10 }}>
          ...
          return (
            ...
            >
              ...
              <div style={{ display: "flex", gap: 14, margin: "10px 0" }}>
                {l.phone && (
                  <a href={"tel:" + l.phone.replace(/[^0-9+]/g, "")} style={clink}>
                    📞 Call
                  </a>
                )}
                {l.phone && (
                  <a href={"sms:" + l.phone} style={clink}>
                    💬 Text
                  </a>
                )}
                {l.email && (
                  <a href={"mailto:" + l.email} style={clink}>
                    ✉️ Email
                  </a>
                )}
              </div>
        ...
        {shown.length === 0 && (
          <div className="rempty">
            No leads here yet. Share your booking link to start getting requests.
          </div>
        )}
      </div>
    </div>
