    ...
    <div>
      ...
      <div style={{ background: "#0f2a5e", color: "#fff", borderRadius: 14, padding: 16, marginBottom: 16 }}>
        ...
        <div style={{ display: "flex", gap: 8 }}>
          <input readOnly aria-label="Your online booking link" value={typeof window !== "undefined" ? bookingLink() : ""} style={{ flex: 1, border: "none", borderRadius: 9, padding: "9px 11px", fontSize: 12.5, color: "#0b1524" }} />
...
function Tab({ k, label, filter, setFilter }: { k: string; label: string; filter: string; setFilter: (s: string) => void }) {
  const on = filter === k;
  return <button type="button" onClick={() => setFilter(k)} style={{ border: "none", borderRadius: 9, padding: "7px 12px", fontWeight: 700, fontSize: 12.5, whiteSpace: "nowrap", cursor: "pointer", background: on ? "#2563eb" : "#eef2f8", color: on ? "#fff" : "#5c6675" }}>{label}</button>;
}
