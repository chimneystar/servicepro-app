 ...
 */
export async function moveDispatchJob(jobId: string, profileId: string | null): Promise<DispatchResult> {
  ...
  if (profileId) {
    ...
    const assignable = await assertAssignableToJob(supabase, {
      organizationId: profile.organization_id!, jobId, profileId, locale,
    });
  ...
  const stale = supabase.from("job_assignments").delete()
    .eq("organization_id", profile.organization_id!).eq("job_id", jobId).eq("is_lead", true);
  ...
  if (profileId) {
    const { error: leadError } = await supabase.from("job_assignments")
      .upsert({ organization_id: profile.organization_id, job_id: jobId, profile_id: profileId, is_lead: true }, { onConflict: "job_id,profile_id" });
    if (leadError) {
...
export async function removeJobTechnician(jobId: string, profileId: string): Promise<DispatchResult> {
  const profile = await dispatcher();
  const locale = await getLocale();
  const supabase = await createClient();
  ...
  const { error } = await supabase.from("job_assignments").delete()
    .eq("organization_id", profile.organization_id!).eq("job_id", jobId).eq("profile_id", profileId);
