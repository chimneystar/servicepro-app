    ...
    <div style={{ maxWidth: 760 }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 4, gap: 10, flexWrap: "wrap" }}>
        ...
        <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
          ...
          <span style={{ fontSize: 14, fontWeight: 700, color: "#5c6675" }}>{fmtDate(date)}</span>
          <Link href={`/route?date=${next}`} style={navBtn}>›</Link>
        </div>
      </div>
      <p style={{ color: "#5c6675", fontSize: 14, marginBottom: 14 }}>{stops.length} stop{stops.length === 1 ? "" : "s"}</p>
        ...
        {stops.map((s, i) => (
          <div key={s.id} style={{ background: "#fff", border: "1px solid #e2e8f0", borderRadius: 14, padding: 14, display: "flex", gap: 12 }}>
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              ...
              <div style={{ fontSize: 14, color: "#5c6675" }}>{s.service}{s.tech ? ` · ${s.tech}` : ""}</div>
              {s.addr && <div style={{ fontSize: 14, color: "#5c6675" }}>📍 {s.addr}</div>}
