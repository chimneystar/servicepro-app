  ...
  return (
    ...
    >
      <div style={{ fontWeight: 800, fontSize: "0.875rem", marginBottom: 8 }}>
        {he ? "רווחיות העבודה" : "Job profitability"}
      </div>
      {!labour.available && (
        <div style={warn}>
          {he
            ? "תמחור עבודה אינו זמין — יש להריץ את מיגרציה 039."
            : "Job costing is unavailable — migration 039 has not been run."}
        </div>
      )}
      ...
      <Row
        label={`${he ? "עבודה" : "Labour"} (${hours} ${he ? "שעות" : "h"})`}
        value={"−" + money(labour.costMinor, currency)}
      />
      <Row
        label={he ? "הוצאות שהוזנו" : "Entered expenses"}
        value={"−" + money(expensesMinor, currency)}
      />
      <div
        style={{
          display: "flex",
      ...
      >
        <button
          type="button"
          disabled={busy || !labour.available}
          style={btn}
          onClick={() =>
            start(async () => {
