 ...
 * the start.
 */
export default function JobAppointment({ locale, jobId, confirmation, confirmedAt, declinedAt, note, link, arrivedAt, onMyWayAt }: {
  locale: Locale; jobId: string;
  confirmation: string; confirmedAt: string | null; declinedAt: string | null; note: string | null;
  link: { token: string; expiresAt: string } | null;
  arrivedAt: string | null; onMyWayAt: string | null;
}) {
  const he = locale === "he";
  ...
  const tone = confirmation === "confirmed" ? { bg: "#e6f6ec", fg: "#15803d" }
    : confirmation === "declined" ? { bg: "#fdeaea", fg: "#dc2626" }
      : { bg: "#eef1f6", fg: "#57606f" };
  const label = confirmation === "confirmed" ? (he ? "הלקוח אישר" : "Customer confirmed")
    : confirmation === "declined" ? (he ? "הלקוח ביקש לשנות" : "Customer declined")
      : (he ? "ממתין לאישור הלקוח" : "Awaiting customer confirmation");
  const stamp = confirmation === "confirmed" ? confirmedAt : confirmation === "declined" ? declinedAt : null;
      ...
      <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginTop: 10 }}>
        <button type="button" disabled={busy} style={btn}
          onClick={() => start(async () => {
            ...
            setNotice(result.sent ? (he ? "נשלחה בקשת אישור ללקוח." : "Confirmation request sent to the customer.") : (result.notice ?? null));
            router.refresh();
          })}>
          {he ? "שליחת בקשת אישור" : "Send confirmation request"}
        </button>
        {url && (
          <button type="button" disabled={busy} style={ghost}
            onClick={() => start(async () => {
              setError(null); setNotice(null);
              const result = await revokeAppointmentLink(jobId);
              if (!result.ok) { setError(result.error ?? null); return; }
              setUrl(null);
              setNotice(he ? "הקישור בוטל ואינו פעיל יותר." : "The link is revoked and no longer works.");
              router.refresh();
            })}>{he ? "ביטול הקישור" : "Revoke link"}</button>
        )}
        {onMyWayAt && !arrivedAt && (
          <button type="button" disabled={busy} style={ghost}
            onClick={() => start(async () => { await markArrived(jobId); router.refresh(); })}>
      ...
      {url && (
        <div style={{ marginTop: 10, fontSize: "0.8125rem" }}>
          ...
          <div style={{ color: "#5c6675", marginTop: 3 }}>
            ...
            {link ? ` ${he ? "בתוקף עד" : "Valid until"} ${new Date(link.expiresAt).toLocaleDateString(he ? "he-IL" : "en-US")}.` : ""}
          </div>
        </div>
      )}
