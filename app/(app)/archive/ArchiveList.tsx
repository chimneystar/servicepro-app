          ...
          <div key={r.id} style={{ background: "#fff", border: "1px solid #e2e8f0", borderRadius: 12, padding: 14 }}>
            <div style={{ display: "flex", justifyContent: "space-between", gap: 10, alignItems: "flex-start" }}>
              <div style={{ minWidth: 0 }}>
                <div style={{ fontWeight: 800 }}>{r.name} <span style={{ fontSize: 14, fontWeight: 700, color: "#9a3412", background: "#fff7ed", borderRadius: 6, padding: "2px 6px", marginInlineStart: 4 }}>ARCHIVED</span></div>
                <div style={{ fontSize: 14, color: "#5c6675" }}>{[r.phone, r.email, [r.address, r.city].filter(Boolean).join(", ")].filter(Boolean).join(" · ")}</div>
                {r.legacy_note && <div style={{ fontSize: 14, color: "#475569", marginTop: 4 }}>🗒️ {r.legacy_note}</div>}
              </div>
              <button onClick={() => restore(r.id)} disabled={pending} style={{ background: "#e0ebff", color: "#2563eb", border: "none", borderRadius: 9, padding: "7px 11px", fontWeight: 700, fontSize: 14, cursor: "pointer", flexShrink: 0 }}>↩ Restore</button>
            </div>
          </div>
        ))}
