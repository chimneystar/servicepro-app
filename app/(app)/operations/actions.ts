...
export async function createCrew(_prev: ActionResult, form: FormData): Promise<ActionResult> {
  ...
  const { error: dbError } = await ctx.supabase.from("crews").insert({
    organization_id: ctx.profile.organization_id,
    name,
    color: text(form, "color") || "#2463eb",
  });
...
export async function createServiceArea(
  ...
  const { error: dbError } = await ctx.supabase.from("service_areas").insert({
    organization_id: ctx.profile.organization_id,
    name,
    area_type: areaType,
    values_json: values,
...
export async function createVendor(_prev: ActionResult, form: FormData): Promise<ActionResult> {
  ...
  const name = text(form, "name");
  if (!name) return { ok: false, error: invalid(he) };
  const { error: dbError } = await ctx.supabase.from("vendors").insert({
    organization_id: ctx.profile.organization_id,
    name,
...
function purchaseOrderLines(form: FormData): {
  description: string;
  qtyMilli: number;
...
export async function createPurchaseOrder(
  ...
  const totalMinor = lines.reduce(
    (sum, l) => sum + lineSubtotalMinor(l.qtyMilli, l.unitCostMinor),
    0,
  );
  ...
  const { data: order, error: orderError } = await ctx.supabase
    ...
    .insert({
      organization_id: ctx.profile.organization_id,
      po_number: poNumber,
      vendor_id: text(form, "vendorId") || null,
      job_id: text(form, "jobId") || null,
      total_minor: totalMinor,
      expected_date: text(form, "expectedDate") || null,
      created_by: ctx.profile.id,
    })
    .select("id")
    .single();
  if (orderError || !order) return { ok: false, error: saveFailed(he) };
  const { error: itemError } = await ctx.supabase.from("purchase_order_items").insert(
    lines.map((l, index) => ({
      organization_id: ctx.profile.organization_id,
      purchase_order_id: order.id,
      description: l.description,
      qty_milli: l.qtyMilli,
      quantity: l.qtyMilli / 1000,
...
export async function addPurchaseOrderLine(
  ...
  if (order.status === "received" || order.status === "cancelled") {
    return {
      ok: false,
      error: he ? "אי אפשר להוסיף שורות להזמנה סגורה." : "This purchase order is closed.",
    };
  }
...
export async function advancePurchaseOrderStatus(
  ...
  if (!canTransitionPurchaseOrder(order.status, status)) {
    return {
      ok: false,
      error: he
        ? `אי אפשר להעביר הזמנה מ-${order.status} ל-${status}.`
        : `A purchase order cannot go from ${order.status} to ${status}.`,
    };
  }
...
export async function receivePurchaseOrderLine(lineId: string, qty: string): Promise<ActionResult> {
  ...
  let qtyMilli = 0;
  try {
    qtyMilli = parseQtyToMilli(qty);
  } catch {
    return { ok: false, error: invalid(he) };
  }

  const poStatus = String(
    (line as { purchase_orders?: { status?: string } | { status?: string }[] }).purchase_orders
      ? Array.isArray((line as any).purchase_orders)
        ? (line as any).purchase_orders[0]?.status
        : (line as any).purchase_orders?.status
      : "ordered",
  );
  const check = validateReceipt(line, qtyMilli, poStatus);
  if (!check.ok) return { ok: false, error: check.error };

  const { error: rpcError } = await ctx.supabase.rpc("receive_purchase_order_line", {
    p_line: lineId,
    p_qty_milli: qtyMilli,
  });
...
export async function createSubcontractor(
  ...
  const { error: dbError } = await ctx.supabase.from("subcontractors").insert({
    organization_id: ctx.profile.organization_id,
    company_name: companyName,
    contact_name: text(form, "contactName") || null,
    email: text(form, "email") || null,
    phone: text(form, "phone") || null,
    trades: text(form, "trades")
      .split(",")
      .map((value) => value.trim())
      .filter(Boolean),
    insurance_expires_on: text(form, "insuranceExpires") || null,
  });
  if (dbError) return { ok: false, error: saveFailed(he) };
  revalidatePath("/operations");
  return { ok: true };
