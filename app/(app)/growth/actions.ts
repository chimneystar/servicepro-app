...
export async function createCampaign(_prev: ActionResult, data: FormData): Promise<ActionResult> {
  ...
  if (!campaignChannels(channel).length || !isKnownSegment(segment)) return { ok: false, error: invalid(he) };
  const { error: dbError } = await ctx.supabase.from("campaigns").insert({ organization_id: ctx.profile.organization_id, name, channel, subject: value(data, "subject") || null, body, audience_json: { segment }, created_by: ctx.profile.id });
...
export async function scheduleCampaign(_prev: ActionResult, data: FormData): Promise<ActionResult> {
  ...
  const { data: updated, error: dbError } = await ctx.supabase.from("campaigns")
    .update({ status: "scheduled", scheduled_at: scheduledAt })
    .eq("id", id).eq("organization_id", ctx.profile.organization_id)
    .in("status", ["draft", "paused"]).select("id").maybeSingle();
  if (dbError) return { ok: false, error: saveFailed(he) };
  if (!updated) return { ok: false, error: he ? "אפשר לתזמן רק קמפיין בטיוטה או מושהה." : "Only a draft or paused campaign can be scheduled." };
  revalidatePath("/growth");
  return { ok: true };
}
...
export async function pauseCampaign(_prev: ActionResult, data: FormData): Promise<ActionResult> {
  ...
  if (!id) return { ok: false, error: invalid(he) };
  const { data: updated, error: dbError } = await ctx.supabase.from("campaigns")
    .update({ status: "paused" }).eq("id", id).eq("organization_id", ctx.profile.organization_id)
    .eq("status", "scheduled").select("id").maybeSingle();
  if (dbError) return { ok: false, error: saveFailed(he) };
  if (!updated) return { ok: false, error: he ? "אפשר להשהות רק קמפיין מתוזמן." : "Only a scheduled campaign can be paused." };
  revalidatePath("/growth");
  return { ok: true };
}
...
export async function issueReferral(_prev: ActionResult, data: FormData): Promise<ActionResult> {
  ...
  const [{ data: program }, { data: customer }] = await Promise.all([
    ctx.supabase.from("referral_programs").select("id, name, reward_text, active").eq("id", programId).maybeSingle(),
    ctx.supabase.from("customers").select("id, name, phone, email, sms_opt_in, email_opt_in, deleted_at").eq("id", customerId).maybeSingle(),
  ]);
  ...
  if (!eligibility.ok) {
    ...
    return {
      ok: false,
      error: optedOut
        ? (he ? "הלקוח ביקש לא לקבל הודעות בערוץ הזה." : "This customer has opted out of that channel.")
        : (he ? "אין ללקוח פרטי קשר לערוץ הזה." : "This customer has no contact details for that channel."),
    };
  }
  if (channel === "sms" ? !providers.sms() : !providers.email()) {
    return { ok: false, error: he ? "הערוץ הזה לא מחובר עדיין." : "That channel isn't connected yet." };
  }

  const code = `REF-${Math.random().toString(36).slice(2, 8).toUpperCase()}`;
  const { data: referral, error: insertError } = await ctx.supabase.from("referrals").insert({
    organization_id: ctx.profile.organization_id, program_id: programId,
    referrer_customer_id: customerId, code, status: "shared", channel,
  }).select("id").maybeSingle();
  if (insertError || !referral) return { ok: false, error: saveFailed(he) };

  const { data: org } = await ctx.supabase.from("organizations").select("name").eq("id", ctx.profile.organization_id).maybeSingle();
  ...
  try {
    if (channel === "sms") {
      ...
      const { error: logError } = await ctx.supabase.from("sms_messages").insert({ organization_id: ctx.profile.organization_id, customer_id: customerId, to_phone: eligibility.to, body, provider: "twilio", provider_message_id: sid, status: "sent", sent_at: new Date().toISOString() });
      ...
      const { error: logError } = await ctx.supabase.from("email_messages").insert({ organization_id: ctx.profile.organization_id, related_type: "referral", related_id: referral.id, to_email: eligibility.to, subject, provider: "resend", provider_message_id: id, status: "sent", sent_at: new Date().toISOString() });
    ...
    const { error: markError } = await ctx.supabase.from("referrals").update({ error_message: detail }).eq("id", referral.id);
    if (markError) console.error("[growth] referral send failed and could not be marked:", markError.message);
    revalidatePath("/growth");
    return { ok: false, error: he ? "לא הצלחנו לשלוח את הקוד. הקוד נשמר וניתן לנסות שוב." : "We couldn't send the code. It was saved, so you can try again." };
  }
  const { error: sentError } = await ctx.supabase.from("referrals").update({ sent_at: new Date().toISOString() }).eq("id", referral.id);
