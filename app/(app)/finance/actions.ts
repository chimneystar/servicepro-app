...
const initialError = (he: boolean) =>
  he
    ? "לא הצלחנו לשמור. בדקו את הפרטים ונסו שוב."
    : "We couldn't save this. Check the details and try again.";
const amountError = (he: boolean) =>
  he
    ? "סכום לא תקין. הזינו מספר, למשל 1234.56"
    : "That amount isn't valid. Enter a number, for example 1234.56";
const forbiddenError = (he: boolean) =>
  he ? "אין לך הרשאה לנהל כספים." : "You don't have access to manage finance.";
const rateError = (he: boolean) =>
  he
    ? "שיעור מס לא תקין. הזינו אחוז, למשל 8.25"
    : "That tax rate isn't valid. Enter a percentage, for example 8.25";
const dateOrderError = (he: boolean) =>
  he ? "תאריך הסיום מוקדם מתאריך ההתחלה." : "The end date is before the start date.";
const migrationError = (he: boolean) =>
  he
 ...
 */
export async function setTaxMode(
  _previous: FinanceResult,
  formData: FormData,
): Promise<FinanceResult> {
  const locale = await getLocale(),
...
export async function setTaxJurisdictionActive(
  id: string,
  active: boolean,
): Promise<FinanceResult> {
  const locale = await getLocale(),
...
export async function createTaxFiling(
  ...
  try {
    ...
    const { error } = await supabase.from("tax_filings").insert({
      ...
      taxable_sales_minor: minor(formData.get("taxableSales")),
...
export async function createSettlement(
  ...
  try {
    const profile = await guardFinance();
    const gross = minor(formData.get("gross")),
      fees = minor(formData.get("fees")),
      refunds = minor(formData.get("refunds"));
    const chargebacks = minor(formData.get("chargebacks")),
      adjustments = minor(formData.get("adjustments"));
    ...
    if (gross < 0 || fees < 0 || refunds < 0 || chargebacks < 0) {
      return {
        ok: false,
        error: he
          ? "סכומים לא יכולים להיות שליליים."
          : "Gross, fees, refunds and chargebacks cannot be negative.",
      };
    }
    if (net !== derivedNet) {
    ...
    const { error } = await supabase.from("settlement_batches").insert({
      ...
      provider_settlement_id: String(formData.get("providerId") ?? "").trim() || null,
      settlement_date: String(
        formData.get("settlementDate") ?? new Date().toISOString().slice(0, 10),
      ),
      expected_arrival: String(formData.get("arrival") ?? "") || null,
      gross_minor: gross,
      fees_minor: fees,
...
export async function createDispute(
  ...
  try {
    ...
    const reason = String(formData.get("reason") ?? "").trim(),
      amount = minor(formData.get("amount"));
    if (!reason || amount <= 0) return { ok: false, error: initialError(he) };
    const supabase = await createClient();
    const { error } = await supabase.from("payment_disputes").insert({
      organization_id: profile.organization_id,
      payment_id: String(formData.get("paymentId") ?? "") || null,
      provider: String(formData.get("provider") ?? "manual").trim() || "manual",
      provider_dispute_id: String(formData.get("providerId") ?? "").trim() || null,
      reason_code: String(formData.get("reasonCode") ?? "").trim() || null,
      reason,
      disputed_minor: amount,
      response_due_at: String(formData.get("responseDue") ?? "") || null,
      evidence_notes: String(formData.get("evidence") ?? "").trim() || null,
      assigned_to: String(formData.get("assignedTo") ?? "") || null,
      created_by: profile.id,
    });
    if (error) return { ok: false, error: initialError(he) };
    revalidatePath("/finance");
  ...
  try {
    ...
    const { error } = await supabase
      ...
      .update({
        ...
        reconciled_by: status === "reconciled" ? profile.id : null,
      })
      .eq("id", id)
      .eq("organization_id", profile.organization_id!);
    if (error) return { ok: false, error: initialError(he) };
    ...
    return { ok: false, error: he ? "אין הרשאה." : "Not allowed." };
  }
}
  ...
  try {
    ...
    const { error } = await supabase
      ...
      .update({
        ...
        closed_at: ["won", "lost", "accepted", "closed"].includes(status)
