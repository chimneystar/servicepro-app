...
import {
  ...
  softDeleteDocument,
  voidDocument,
  reopenEstimate as reopenEstimateDocument,
  markDocumentSent,
  type ActionResult,
} from "@/lib/documents";
...
export async function setEstimateStatus(id: string, status: string): Promise<ActionResult> {
  ...
  const { data: current } = await supabase
    .from("estimates")
    .select("id, status, version, signed_at, sent_at, voided_at, deleted_at")
    .eq("id", id)
    .maybeSingle();
...
export async function convertEstimateToInvoice(estimateId: string): Promise<ConvertResult> {
  ...
  const readiness = conversionReadiness({
    ...
    selectedOptionId: est.selected_option_id,
  ...
  const { data: inv, error } = await supabase
    ...
    .insert({
      ...
      status: "unpaid",
      discount_minor: est.discount_minor,
      tax_rate_bps: est.tax_rate_bps,
      total_minor: est.total_minor,
      notes: est.notes,
...
export async function addEstimateOption(
  ...
  if (est.signed_at)
    return {
      ...
      error:
        locale === "he"
          ? "ההצעה כבר נחתמה ואי אפשר לשנות אותה."
          : "This estimate is signed and can no longer be re-priced.",
    };
  ...
  const { error } = await supabase.from("estimate_options").upsert(
    {
      ...
      sort: tierRank(values.tier) as number,
      created_by: profile.id,
    },
...
export async function deleteEstimateOption(
  optionId: string,
  estimateId: string,
): Promise<ActionResult> {
  const locale = await getLocale();
...
export async function addEstimateOptionItem(
  optionId: string,
  estimateId: string,
  formData: FormData,
): Promise<ActionResult> {
...
export async function deleteEstimateOptionItem(
  itemId: string,
  estimateId: string,
): Promise<ActionResult> {
  const locale = await getLocale();
 ...
 */
export async function chooseEstimateOption(
  estimateId: string,
  optionId: string,
): Promise<ActionResult> {
  const locale = await getLocale();
  ...
  const { data, error } = await supabase.rpc("select_estimate_option", {
    p_token: est.public_token,
    p_option: optionId,
    p_by: profile.full_name || "Office",
  });
  if (error) return { ok: false, error: error.message };
