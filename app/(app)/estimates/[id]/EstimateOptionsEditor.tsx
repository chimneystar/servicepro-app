...
import { addEstimateOption, deleteEstimateOption, addEstimateOptionItem, deleteEstimateOptionItem, chooseEstimateOption } from "../actions";
...
export type OptionItemRow = {
  id: string; option_id: string; title: string | null; description: string;
  qty_milli: number; unit_price_minor: number; cost_minor: number; taxable: boolean;
};
export type OptionRow = {
  id: string; tier: string; title: string; description: string | null;
  recommended: boolean; deposit_minor: number; total_minor: number; sort: number;
};
 ...
 */
export default function EstimateOptionsEditor({ locale, currency, estimateId, options, items, selectedOptionId, signed, discountMinor, taxRateBps, estimateDeposit }: {
  locale: Locale; currency: string; estimateId: string;
  options: OptionRow[]; items: OptionItemRow[]; selectedOptionId: string | null; signed: boolean;
  discountMinor: number; taxRateBps: number; estimateDeposit: number;
}) {
  const he = locale === "he";
  ...
  const withItems = options.map((option) => ({ ...option, items: items.filter((row) => row.option_id === option.id) }));
  const described = describeOptions(withItems, { discountMinor, taxRateBps, estimateDeposit }) as {
    id: string; tier: string; title: string; recommended: boolean; totalMinor: number; depositMinor: number; upgradeMinor: number;
  }[];
  ...
  return (
    <div style={card}>
      ...
      {signed && <div style={warnBox}>{he ? "ההצעה נחתמה — אי אפשר לשנות חלופות." : "This estimate is signed — options can no longer be changed."}</div>}

      {!signed && missingTiers.length > 0 && (
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 12 }}>
          {missingTiers.map((tier: string) => (
            <button key={tier} type="button" disabled={busy} style={btn}
              onClick={() => run(() => addEstimateOption(estimateId, { tier, title: tierLabel(tier, locale) as string }))}>
      ...
      {described.length === 0 && <div style={hint}>{he ? "עוד לא הוגדרו חלופות. ההצעה תישלח במחיר יחיד." : "No options yet — this estimate goes out at a single price."}</div>}

      {described.map((summary) => {
        const option = withItems.find((row) => row.id === summary.id)!;
        const chosen = selectedOptionId === option.id;
        return (
          <div key={option.id} style={{ border: `1px solid ${chosen ? "#15803d" : "#e2e8f0"}`, borderRadius: 12, padding: 12, marginBottom: 10, background: chosen ? "#f4fbf6" : "#fff" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
              <b>{tierLabel(option.tier, locale) as string}</b>
              {option.recommended && <span className="pill" style={{ background: "#e0ebff", color: "#1d4ed8" }}>{he ? "מומלץ" : "recommended"}</span>}
              {chosen && <span className="pill" style={{ background: "#e6f6ec", color: "#15803d" }}>{he ? "נבחר" : "chosen"}</span>}
            ...
            {!signed && (
              <form style={{ display: "flex", gap: 6, flexWrap: "wrap", marginTop: 8 }}
                ...
                <button type="submit" disabled={busy} style={btn}>{he ? "הוספת שורה" : "Add line"}</button>
              </form>
            )}

            <div style={{ display: "flex", gap: 8, marginTop: 8, flexWrap: "wrap" }}>
              {!signed && !chosen && option.items.length > 0 && (
                <button type="button" disabled={busy} style={ghost} onClick={() => run(() => chooseEstimateOption(estimateId, option.id))}>
                  {he ? "בחירה בשם הלקוח" : "Choose on the customer's behalf"}
                </button>
              )}
              {!signed && (
                <button type="button" disabled={busy} style={rm} onClick={() => run(() => deleteEstimateOption(option.id, estimateId))}>
