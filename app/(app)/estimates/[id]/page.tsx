...
export default async function EstimateDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: est } = await supabase.from("estimates")
    .select("id, number, status, discount_minor, deposit_minor, tax_rate_bps, issue_date, notes, public_token, version, sent_at, signed_at, voided_at, void_reason, reopened_at, reopen_reason, reopen_count, customers(name, address, city, phone, email), selected_option_id")
