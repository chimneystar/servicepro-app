...
export default async function ReceivingPage() {
  ...
  const { data: lines } = ids.length
    ? await supabase.from("purchase_order_items")
        .select("id, purchase_order_id, description, qty_milli, received_qty_milli, unit_cost_minor, inventory_item_id")
        .in("purchase_order_id", ids).order("sort")
    : { data: [] as PoRow["lines"] };
  ...
  const rows: PoRow[] = (orders ?? []).map((o) => ({
    ...
    vendor: (Array.isArray(o.vendors) ? o.vendors[0]?.name : (o.vendors as { name: string } | null)?.name) ?? null,
  ...
  return (
    <div style={{ maxWidth: 860 }}>
      ...
      <h1 style={{ fontSize: 24, fontWeight: 800, margin: "8px 0 4px" }}>{he ? "קבלת סחורה" : "Receiving"}</h1>
      ...
      <PurchaseOrderPanel orders={rows} inventory={(inventory ?? []) as { id: string; name: string; unit: string }[]} currency={org?.currency ?? "USD"} />

      {rows.length === 0 && (
        <div className="rempty">
          {he ? "אין הזמנות רכש פתוחות. אפשר ליצור הזמנה במסך התפעול." : "No open purchase orders. Create one on the Operations screen."}
          {" "}<Link href="/operations">{he ? "תפעול" : "Operations"}</Link>
        </div>
      )}
      <p style={{ color: "#5c6675", fontSize: 12.5, marginTop: 12 }}>
        {he ? "סה\"כ פתוח: " : "Open value: "}{money(rows.reduce((sum, r) => sum + (r.total_minor ?? 0), 0), org?.currency ?? "USD")}
      </p>
    </div>
  );
}
