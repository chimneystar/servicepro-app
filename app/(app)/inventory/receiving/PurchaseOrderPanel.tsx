          ...
          >
                  ...
                  >
                    <div style={{ flex: 1, minWidth: 160 }}>
                      ...
                      <div className="sp-text-muted">
                        ...
                        {!line.inventory_item_id && (
                          <span style={{ color: "#9a3412" }}> · not linked to stock</span>
                        )}
                      </div>
                    </div>
                    {outstanding > 0 && po.status !== "cancelled" && (
                      <button
                        ...
                        disabled={pending}
                        onClick={() =>
                          run(() => receivePurchaseOrderLine(line.id, formatQtyMilli(outstanding)))
                        }
                        style={smallBtn}
                      >
            ...
            <div style={{ display: "flex", gap: 8, marginTop: 12, flexWrap: "wrap" }}>
              {po.status === "draft" && (
                <button
                  type="button"
                  disabled={pending}
                  onClick={() => run(() => advancePurchaseOrderStatus(po.id, "ordered"))}
                  style={smallBtn}
                >
                  Mark ordered
                </button>
              ...
              <button
                type="button"
                disabled={pending}
                onClick={() => setAddingTo(addingTo === po.id ? null : po.id)}
                style={{ ...smallBtn, background: "#eef2f8", color: "#2563eb" }}
              >
                Add line
              </button>
              <button
                ...
                disabled={pending}
                onClick={() => {
                  if (confirm(`Cancel ${po.po_number}?`))
                    run(() => advancePurchaseOrderStatus(po.id, "cancelled"));
                }}
            ...
            {addingTo === po.id && (
              ...
              >
                ...
                <button type="submit" disabled={pending} style={{ ...smallBtn, marginTop: 10 }}>
