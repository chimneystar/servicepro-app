...
export default function VisitClient({
  ...
  const answered =
    appointment.confirmation === "confirmed" || appointment.confirmation === "declined";
  ...
  async function respond(response: "confirmed" | "declined") {
    ...
    try {
      ...
      if (rpcError || !result?.ok) {
        setError(
          result?.error === "link_expired"
            ? he
              ? "הקישור פג תוקף. אנא התקשרו אלינו."
              : "This link has expired. Please call us."
            : result?.error === "appointment_closed"
              ? he
                ? "הפגישה כבר נסגרה."
                : "This appointment is already closed."
              : he
                ? "לא הצלחנו לשמור את התשובה. נסו שוב."
      ...
      <div style={{ padding: "22px 24px" }}>
        ...
        <div style={{ fontSize: "0.875rem", color: "#5c6675", marginTop: 2 }}>
          ...
          {appointment.technician
            ? ` · ${he ? "טכנאי" : "Technician"}: ${appointment.technician}`
            : ""}
        </div>
          ...
          <div style={{ marginTop: 18 }}>
            ...
            {asking && (
              <textarea
                ...
                rows={2}
                placeholder={
                  he ? "מתי כן יתאים לכם? (לא חובה)" : "When would suit you better? (optional)"
                }
                aria-label={
                  he ? "מתי כן יתאים לכם? (לא חובה)" : "When would suit you better? (optional)"
                }
        ...
        {appointment.org?.phone && (
          <div style={{ marginTop: 16, textAlign: "center", fontSize: "0.875rem" }}>
            <a
              href={`tel:${String(appointment.org.phone).replace(/[^0-9+]/g, "")}`}
